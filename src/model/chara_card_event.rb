...
module Unlight
  class CharaCardEvent < BaseEvent
    ...
    # 13の目シリーズ
    THIRTEEN_EYES = [FEAT_THIRTEEN_EYES, FEAT_EX_THIRTEEN_EYES, FEAT_THIRTEEN_EYES_R].freeze
    ...
    # レンジ定数
    AI_RANGE_ALL = [1, 2, 3].freeze
    AI_RANGE_ARROW = [2, 3].freeze
    AI_RANGE_SWORD = [1].freeze
    ...
    def initialize(c, cc)
      ...
      wp&.each do |p|
        ...
        @passives[pno] = p unless @passives.key?(pno)
      end
      @feats_enable = {}
      @used_feats = {}
      @passives_enable = {}
    ...
    def attribute_damage(attribute, target, d = 0)
      ...
      # 完全無効化の類
      if (target.current_chara_card.special_status[SPECIAL_STATE_SHARPEN_EDGE][1]).positive? && d.positive?
        case attribute
        when ATTRIBUTE_DEATH, ATTRIBUTE_DIFF
          ...
          target.current_chara_card.finish_sharpen_edge_feat if (@cc.special_status[SPECIAL_STATE_SHARPEN_EDGE][1]).zero?
      ...
      when ATTRIBUTE_CONSTANT
        if target.invincible
          ...
          ret = 0
        elsif (target.current_chara_card.special_status[SPECIAL_STATE_CONST_COUNTER][1]).positive? && target.current_chara_card.special_status[SPECIAL_STATE_CONST_COUNTER][0] >= d
          owner.damaged_event(attribute_damage(ATTRIBUTE_REFLECTION, owner, d))
          ret = 0
        elsif (target.current_chara_card.status[STATE_UNDEAD2][1]).positive?
          ret = 0
        else
          ret = d
        end
      ...
      when ATTRIBUTE_DYING
        ...
        ret = 0 if ret.negative?

        # 赤い石榴の対BOSS用特殊効果  ダメージの期待値が即死技とおよそ同等になるよう調整。HP99への固定は99pt回復に変更。
      when ATTRIBUTE_ZAKURO
        ret = if d == 99
                instant_kill_guard?(target) ? d : d - target.hit_point
              else
                instant_kill_guard?(target) ? target.instant_kill_damage * (8 - d) : target.hit_point - d
              end
    ...
    def attribute_party_damage(target, indexies, damage, attribute = ATTRIBUTE_CONSTANT, type = TARGET_TYPE_SINGLE, attack_times = 1, is_not_hostile = false) # rubocop:disable Metrics/ParameterLists
      idxs = indexies.is_a?(Array) ? indexies : [indexies]

      return if idxs.size.zero?
    ...
    def damage_to_index(target, index, damage, attribute, type, is_not_hostile)
      ...
      if index == target.current_chara_card_no
        ...
        target.party_damaged_event(index, damage, is_not_hostile) unless (target.chara_cards[index].status[STATE_UNDEAD2][1]).positive?
      end
    end
    ...
    def check_additional_draw_passive
      check_and_on_passive(PASSIVE_ADDITIONAL_DRAW) if @cc.using
    end
    ...
    def check_indomitable_mind_passive
      check_and_on_passive(PASSIVE_INDOMITABLE_MIND) if @cc.using && !owner.initiative && @cc.owner.distance != 2 && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_indomitable_mind_passive_damage
      if @passives_enable[PASSIVE_INDOMITABLE_MIND] && (@cc.owner.hit_point - duel.tmp_damage < PassiveSkill.pow(@passives[PASSIVE_INDOMITABLE_MIND]))
        duel.tmp_damage = @cc.owner.hit_point - PassiveSkill.pow(@passives[PASSIVE_INDOMITABLE_MIND])
        duel.tmp_damage = 0 if duel.tmp_damage.negative?
      end
    end
    ...
    def check_drain_soul_passive
      check_and_on_passive(PASSIVE_DRAIN_SOUL) if @cc.using
    end
    ...
    def finish_drain_soul_passive
      if @passives_enable[PASSIVE_DRAIN_SOUL]
        ...
        if foe.cards.size.positive? && drain_cond
    ...
    def check_sealing_attack_passive
      check_and_on_passive(PASSIVE_SEALING_ATTACK) if @cc.using && owner.initiative && @cc.owner.distance == 1 && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def check_rage_against_only_first_turn_passive
      check_rage_against_passive if duel.turn.zero?
    end
    regist_event CheckRageAgainstPassiveStartEvent

    # レイジ発動。攻撃時に基本攻撃力を二倍にする
    def finish_rage_against_passive
      owner.tmp_power += @cc.ap if @passives_enable[PASSIVE_RAGE_AGAINST] && owner.tmp_power.positive?
    end
    ...
    def check_bounce_back_passive
      check_and_on_passive(PASSIVE_BOUNCE_BACK) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_bounce_back_passive_damage
      @cc.owner.special_dealed_event(duel.deck.draw_cards_event(duel.tmp_damage + PassiveSkill.pow(@passives[PASSIVE_BOUNCE_BACK])).each { |c| @cc.owner.dealed_event(c) }) if @passives_enable[PASSIVE_BOUNCE_BACK] && duel.tmp_damage.positive?
    end
    ...
    def use_liberation_passive
      if @cc.using && (rand(99) < 5)
        force_on_passive(PASSIVE_LIBERATION)
        if (@cc.status[STATE_SEAL][1]).positive?
    ...
    def check_harden_passive
      if @cc.using
        min = Time.now.min
        force_on_passive(PASSIVE_HARDEN) if (10..19).cover?(min) || (40..49).cover?(min)
      end
    end
    ...
    def check_absorp_passive
      if @cc.using
        min = Time.now.min
        force_on_passive(PASSIVE_ABSORP) if ((20..29).cover?(min) || (50..59).cover?(min)) && !(@passives_enable[PASSIVE_HARDEN])
      end
    end
    ...
    def check_moondog_passive
      check_and_on_passive(PASSIVE_MOONDOG) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_moondog_passive
      owner.tmp_power += PassiveSkill.pow(@passives[PASSIVE_MOONDOG]) * owner.distance * owner.get_type_table_count(ActionCard::DEF) if @passives_enable[PASSIVE_MOONDOG]
    end
    ...
    def jump_passive_restore
      ...
      @jump_passive_pre_status.each_index do |i|
        if @cc.status[i][0] != @jump_passive_pre_status[i][0] || @cc.status[i][1] != @jump_passive_pre_status[i][1]
          # 追加されたものを消す
          if (@jump_passive_pre_status[i][1]).zero?
    ...
    def jump_passive_card_reset
      owner.cards.each { |card| discard(owner, card) }
    end
    ...
    def check_protection_aim_passive
      check_and_on_passive(PASSIVE_PROTECTION_AIM) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_protection_aim_passive
      if @passives_enable[PASSIVE_PROTECTION_AIM]
        ...
        owner.tmp_power += arrow_ap + 3 if arrow_ap.positive?
      end
    end
    ...
    def use_mistake_passive_damage
      if @passives_enable[PASSIVE_MISTAKE] && duel.tmp_damage >= @cc.owner.hit_point && @cc.status[STATE_UNDEAD][1] <= 0
    ...
    def check_senkou_passive
      force_on_passive(PASSIVE_SENKOU) if @cc.using && owner.current_hit_point_max * 3 / 5 >= owner.hit_point && owner.hit_point > owner.current_hit_point_max * 2 / 5
    end
    regist_event CheckSenkouPassiveEvent

    # 発動する
    def use_senkou_passive
      owner.tmp_power += PassiveSkill.pow(@passives[PASSIVE_SENKOU]) if @passives_enable[PASSIVE_SENKOU]
    end
    ...
    def is_senkou?
      @passives_enable[PASSIVE_SENKOU]
    end
    ...
    def check_hate_passive
      force_on_passive(PASSIVE_HATE) if @cc.using && ((PassiveSkill.pow(@passives[PASSIVE_HATE]) == 7 && owner.current_hit_point_max * 2 / 5 >= owner.hit_point) || (PassiveSkill.pow(@passives[PASSIVE_HATE]) == 5 && owner.current_hit_point_max / 2 >= owner.hit_point && foe.current_hit_point_max / 2 >= foe.hit_point))
    end
    regist_event CheckHatePassiveEvent

    # 発動する
    def use_hate_passive
      owner.tmp_power += PassiveSkill.pow(@passives[PASSIVE_HATE]) if @passives_enable[PASSIVE_HATE]
    end
    regist_event UseHatePassiveEvent

    # 発動する
    def use_hate_passive_damage
      duel.tmp_damage = duel.tmp_damage * 2 if @passives_enable[PASSIVE_HATE] && @cc && @cc.index == owner.current_chara_card_no && !owner.initiative
    end
    ...
    def check_little_princess_passive
      check_and_on_passive(PASSIVE_LITTLE_PRINCESS) if @cc.using && owner.hit_point >= 5 && !@passives_enable[PASSIVE_CRIMSON_WITCH]
    end
    ...
    def use_little_princess_passive
      if @passives_enable[PASSIVE_LITTLE_PRINCESS] && !owner.initiative && duel.tmp_damage.positive?
    ...
    def check_crimson_witch_passive
      check_and_on_passive(PASSIVE_CRIMSON_WITCH) if @cc.using && owner.hit_point < 5 && !@passives_enable[PASSIVE_LITTLE_PRINCESS]
    end
    ...
    def use_crimson_witch_passive
      if @passives_enable[PASSIVE_CRIMSON_WITCH] && owner.initiative && duel.tmp_damage.positive?
        trigger = @feats_enable[FEAT_ATEMWENDE] ? 20 * Feat.pow(@feats[FEAT_ATEMWENDE]) : 20
        r = rand(100)
        if trigger > r
          duel.tmp_damage *= 2
    ...
    def check_aegis_passive
      check_and_on_passive(PASSIVE_AEGIS) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_aegis_passive
      if @passives_enable[PASSIVE_AEGIS] && duel.tmp_damage.positive?
        duel.tmp_damage -= 2
        duel.tmp_damage = 0 if duel.tmp_damage.negative?
      end
    end
    ...
    def check_ocean_passive
      if @cc.using && (duel.turn % PassiveSkill.pow(@passives[PASSIVE_OCEAN])).zero?
    ...
    def finish_ocean_passive
      if @passives_enable[PASSIVE_OCEAN]
        owner.healed_event(1) if (duel.turn % PassiveSkill.pow(@passives[PASSIVE_OCEAN])).zero? && owner.hit_point.positive?
    ...
    def use_night_fog_passive
      owner.magnification_hurt_const_damage = 1 if @passives_enable[PASSIVE_NIGHT_FOG]
    end
    regist_event UseNightFogPassiveEvent

    # 発動する
    def use_night_fog_passive_damage
      duel.tmp_damage = PassiveSkill.pow(@passives[PASSIVE_NIGHT_FOG]) if @passives_enable[PASSIVE_NIGHT_FOG] && @cc && @cc.index == owner.current_chara_card_no && !owner.initiative && (duel.tmp_damage > PassiveSkill.pow(@passives[PASSIVE_NIGHT_FOG]))
    end
    ...
    def check_double_boddy_passive
      check_and_on_passive(PASSIVE_DOUBLE_BODDY) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_double_boddy_passive
      attribute_party_damage(foe, get_hps(foe), (duel.tmp_damage / 2).to_i, ATTRIBUTE_REFLECTION, TARGET_TYPE_RANDOM) if @passives_enable[PASSIVE_DOUBLE_BODDY] && !owner.initiative && duel.tmp_damage.positive?
    end
    ...
    def check_wit_passive
      check_and_on_passive(PASSIVE_WIT) if @cc.using
    end
    regist_event CheckWitPassiveEvent

    # イベカをドロー
    WIT_PASSIVE_RATES = [[0, 3, 6, 15], [1, 5, 11, 35], [9, 19, 34, 69], [19, 39, 59, 79]].freeze
    def use_wit_passive_draw
      if @passives_enable[PASSIVE_WIT] && (owner.hit_point <= 4)
        # 残りHPに応じて入手率を操作する
        ...
        2.times do
          ...
          @cc.owner.special_event_card_dealed_event(duel.get_event_deck(owner).draw_cards_event(1).each { |c| @cc.owner.dealed_event(c) }) if ret.positive?
        end
    ...
    def finish_wit_passive
      off_passive_event(true, PASSIVE_WIT) if @passives_enable[PASSIVE_WIT]
    ...
    def check_curse_care_passive
      if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    ...
    def recover_curse_care_passive
      if @passives_enable[PASSIVE_CURSE_CARE] && !owner.initiative && !@healed && owner.hit_point < 5
        owner.healed_event(3) if owner.hit_point.positive?
        @healed = true
    ...
    def use_curse_care_passive
      debuff_curse_care_passive if @passives_enable[PASSIVE_CURSE_CARE] && !owner.initiative && @healed && !@debuffed
    end
    ...
    def debuff_curse_care_passive
      ...
      if buffed
        on_buff_event(false,
                      ...
                      foe.current_chara_card.status[STATE_ATK_DOWN][1])
      end
      ...
      if buffed
        on_buff_event(false,
                      ...
                      foe.current_chara_card.status[STATE_DEF_DOWN][1])
      end
      ...
      if buffed
        on_buff_event(false,
                      ...
                      foe.current_chara_card.status[STATE_MOVE_DOWN][1])
      end
    end
    ...
    def check_white_light_passive
      check_and_on_passive(PASSIVE_WHITE_LIGHT) if @cc.using
    end
    ...
    def check_carapace_break_passive
      check_and_on_passive(PASSIVE_CARAPACE_BREAKE) if @cc.using && TURTLES_ID.include?(foe.current_chara_card.id)
    end
    ...
    def use_carapace_break_passive
      duel.tmp_damage = (duel.tmp_damage * get_carapace_multi_num).to_i if @passives_enable[PASSIVE_CARAPACE_BREAKE] && owner.initiative && duel.tmp_damage.positive? && TURTLES_ID.include?(foe.current_chara_card.id)
    end
    ...
    def check_carapace_passive
      force_on_passive(PASSIVE_CARAPACE) if @cc.using && owner.current_hit_point_max / 3 >= owner.hit_point
    end
    regist_event CheckCarapacePassiveEvent

    # 発動する
    def use_carapace_passive
      owner.tmp_power += PassiveSkill.pow(@passives[PASSIVE_CARAPACE]) if @passives_enable[PASSIVE_CARAPACE]
    end
    ...
    def check_chara_resist_kamuy_passive
      favor_chara = 10
      @cc.status[STATE_SEAL][2] = 75 if @cc.using && foe.current_chara_card.charactor_id == favor_chara
    end
    regist_event CheckCharaResistKamuyPassiveEvent

    def restore_resist_kamuy_passive
      favor_chara = 10
      @cc.status[STATE_SEAL][2] = 92 if @cc.using && foe.current_chara_card.charactor_id == favor_chara
    end
    ...
    def check_revisers_passive
      force_on_passive(PASSIVE_REVISERS) if @cc.using && contains_deck_charactor(owner, NOICHROME_ID)
    end
    ...
    def check_resist_wall_passive
      check_resist_wall_passive_move if @cc.using
    end
    ...
    def check_curse_sign_passive
      force_on_passive(PASSIVE_CURSE_SIGN) if @cc.using
    end
    ...
    def check_loyalty_passive
      check_and_on_passive(PASSIVE_LOYALTY) if @cc.using
    end
    ...
    def check_harvest_passive
      check_and_on_passive(PASSIVE_HARVEST) if @cc.using && owner.current_hit_point_max / 2 >= owner.hit_point
    end
    ...
    def finish_harvest_passive
      if @passives_enable[PASSIVE_HARVEST]
        ...
        owner.special_dealed_event(duel.deck.draw_cards_event(num).each do |c|
                                     dealed_cards << c
                                     owner.dealed_event(c)
                                   end)
        off_passive_event(true, PASSIVE_HARVEST)
        dealed_cards.each do |c|
          foe.current_chara_card.steal_deal(c) if c.u_type != ActionCard::ARW && c.b_type != ActionCard::ARW
        end
      end
    ...
    def check_td_passive
      check_and_on_passive(PASSIVE_TD) if @cc.using
    end
    ...
    def check_moon_shine_passive
      check_and_on_passive(PASSIVE_MOON_SHINE) if @cc.using && owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def finish_moon_shine_passive
      if @passives_enable[PASSIVE_MOON_SHINE] && owner.tmp_power.positive?
        # 与えるダメージ
        dmg = 0
        # 特殊カード
        aca = []
        ...
        PassiveSkill.pow(@passives[PASSIVE_MOON_SHINE]).times do |a|
          dmg += discard(foe, aca[a]) if aca[a]
        end
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, dmg)) if dmg.positive?
    ...
    def check_fertility_passive
      force_on_passive(PASSIVE_FERTILITY) if @cc.using
    end
    regist_event CheckFertilityPassiveEvent

    # 豊穣を発動終了する
    def finish_fertility_passive_pre
      @fertility_reserved_cards = owner.cards.dup if @passives_enable[PASSIVE_FERTILITY]
    end
    ...
    def use_awcs_passive_damage
      duel.tmp_damage = duel.tmp_damage * 2 if @passives_enable[PASSIVE_AWCS] && @cc && @cc.index == owner.current_chara_card_no && !owner.initiative && !@awcs.include?(owner.distance)
    end
    ...
    def use_lonsbrough_event_passive_damage
      duel.tmp_damage = (duel.tmp_damage * 1.5).ceil if @passives_enable[PASSIVE_LONSBROUGH_EVENT] && @cc && @cc.index == owner.current_chara_card_no && !owner.initiative
    end
    ...
    def check_projection_passive
      if @cc.using && owner.hit_point.positive? && foe.hit_point.positive? && foe.hit_point > owner.hit_point
    ...
    def finish_projection_passive
      force_off_passive(PASSIVE_PROJECTION) if @passives_enable[PASSIVE_PROJECTION]
    end
    regist_event CheckProjectionPassiveDeadChangeEvent
    regist_event FinishProjectionPassiveEvent

    def damage_multiplier; end
    ...
    def check_damage_multiplier_passive
      check_and_on_passive(PASSIVE_DAMAGE_MULTIPLIER) if @cc.using && gen_target_charactor_id(PassiveSkill.pow(@passives[PASSIVE_DAMAGE_MULTIPLIER])) == foe.current_chara_card.unlight_charactor_id
    end
    ...
    def use_damage_multiplier_passive
      if @passives_enable[PASSIVE_DAMAGE_MULTIPLIER] && owner.initiative && duel.tmp_damage.positive? && gen_target_charactor_id(PassiveSkill.pow(@passives[PASSIVE_DAMAGE_MULTIPLIER])) == foe.current_chara_card.unlight_charactor_id
    ...
    def use_ev201606_passive
      foe.tmp_power += PassiveSkill.pow(@passives[PASSIVE_EV201606]) if @passives_enable[PASSIVE_EV201606] && !owner.initiative && foe.tmp_power.positive?
    end
    ...
    def check_cooly_passive
      if @cc.using && !(@passives_enable[PASSIVE_COOLY])
        init_cooly
    ...
    def use_cooly_passive_move
      if @passives_enable[PASSIVE_COOLY]
        owner.battle_table.clone.each do |c|
          @passive_cooly_card_list << c if (c.u_type == ActionCard::DEF || c.b_type == ActionCard::DEF) && (1..60).cover?(c.id)
        end
      end
    end
    ...
    def check_burning_embers_passive
      if @cc.using && !(@passives_enable[PASSIVE_BURNING_EMBERS])
        init_burning_embers
    ...
    def use_burning_embers_passive_move
      if @passives_enable[PASSIVE_BURNING_EMBERS]
        owner.battle_table.clone.each do |c|
          @passive_burning_embers_card_list << c if (c.u_type == ActionCard::SWD || c.b_type == ActionCard::SWD) && (1..60).cover?(c.id)
        end
      end
    end
    ...
    def use_ev201609_passive
      foe.tmp_power += PassiveSkill.pow(@passives[PASSIVE_EV201609]) if @passives_enable[PASSIVE_EV201609] && !owner.initiative && foe.tmp_power.positive?
    end
    ...
    def check_disaster_flame_passive
      check_and_on_passive(PASSIVE_DISASTER_FLAME) if @cc.using && owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def finish_disaster_flame_passive
      if @passives_enable[PASSIVE_DISASTER_FLAME] && owner.tmp_power.positive?
        # 与えるダメージ
        dmg = 0
        # 特殊カード
        aca = []
    ...
    def check_awakening_one_passive
      check_and_on_passive(PASSIVE_AWAKENING_ONE) if @cc.using && owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_servo_skull_passive
      if @passives_enable[PASSIVE_SERVO_SKULL] && @cc.using && owner.initiative && owner.current_hit_point_max / 2 >= owner.hit_point
        pow = PassiveSkill.pow(@passives[PASSIVE_SERVO_SKULL])
        owner.tmp_power += pow * 2
      end
    end
    regist_event UseServoSkullPassiveEvent

    # 2020-10-08 - Issue#186
    def use_servo_skull_defense_passive
      if @passives_enable[PASSIVE_SERVO_SKULL]
        pow = PassiveSkill.pow(@passives[PASSIVE_SERVO_SKULL])
        owner.tmp_power += pow * 2 if @cc.using && owner.current_hit_point_max / 2 < owner.hit_point
      end
    end
    regist_event UseServoSkullDefensePassiveEvent
    ...
    def check_high_protection_passive
      check_and_on_passive(PASSIVE_HIGH_PROTECTION) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def finish_high_protection_passive
      if @passives_enable[PASSIVE_HIGH_PROTECTION] && @high_protection_def_card_point.positive?
    ...
    def check_puppet_master_passive
      check_and_on_passive(PASSIVE_PUPPET_MASTER) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_puppet_master_passive
      if @passives_enable[PASSIVE_PUPPET_MASTER]
        return if (foe.current_chara_card.status[STATE_BIND][1]).zero?

        # 相手の手札を奪う
        PassiveSkill.pow(@passives[PASSIVE_PUPPET_MASTER]).times do
          break if foe.cards.size.zero?
        ...
        # 相手の使用カードを奪う
        PassiveSkill.pow(@passives[PASSIVE_PUPPET_MASTER]).times do
          return if foe.battle_table.size.zero?
    ...
    def check_aiming_plus_passive
      check_and_on_passive(PASSIVE_AIMING_PLUS) if @cc.using && owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_aiming_plus_passive
      @cc.owner.tmp_power += PassiveSkill.pow(@passives[PASSIVE_AIMING_PLUS]) if @passives_enable[PASSIVE_AIMING_PLUS] && @feats_enable[FEAT_AIMING]
    end
    ...
    def check_rock_crusher_passive
      check_and_on_passive(PASSIVE_ROCK_CRUSHER) if @cc.using && ROCK_SPIRITS_ID.include?(foe.current_chara_card.id)
    end
    ...
    def use_rock_crusher_passive
      duel.tmp_damage = (duel.tmp_damage * get_rock_crusher_multi_num).to_i if @passives_enable[PASSIVE_ROCK_CRUSHER] && owner.initiative && duel.tmp_damage.positive? && ROCK_SPIRITS_ID.include?(foe.current_chara_card.id)
    end
    ...
    def check_brambles_card_passive
      force_on_passive(PASSIVE_BRAMBLES_CARD) if @cc.using
    end
    ...
    def check_ogre_arm_passive
      check_and_on_passive(PASSIVE_OGRE_ARM) if @cc.using && owner.initiative && (@cc.status[STATE_STOP][1]).zero? && owner.distance == 1
    end
    ...
    def use_ogre_arm_passive
      duel.tmp_damage = (duel.tmp_damage * PassiveSkill.pow(@passives[PASSIVE_OGRE_ARM])).to_i if @passives_enable[PASSIVE_OGRE_ARM] && owner.initiative && duel.tmp_damage.positive?
    end
    ...
    def check_crimson_will_passive
      check_and_on_passive(PASSIVE_CRIMSON_WILL) if @cc.using && !owner.initiative && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_crimson_will_passive_damage
      if @passives_enable[PASSIVE_CRIMSON_WILL] && duel.tmp_damage >= 1 && duel.tmp_damage <= PassiveSkill.pow(@passives[PASSIVE_CRIMSON_WILL])
    ...
    def check_guardian_of_life_passive
      check_and_on_passive(PASSIVE_GUARDIAN_OF_LIFE) if @cc.using && (@cc.status[STATE_STOP][1]).zero?
    end
    ...
    def use_guardian_of_life_passive_defense
      if @passives_enable[PASSIVE_GUARDIAN_OF_LIFE] && !owner.initiative && duel.tmp_damage.positive?
    ...
    def use_guardian_of_life_passive_attack
      if @passives_enable[PASSIVE_GUARDIAN_OF_LIFE] && owner.initiative && (duel.tmp_damage <= 0)
        if (@cc.status[STATE_STIGMATA][1]).positive?
          @cc.status[STATE_STIGMATA][1] += PassiveSkill.pow(@passives[PASSIVE_GUARDIAN_OF_LIFE])
          @cc.status[STATE_STIGMATA][1] = 9 if @cc.status[STATE_STIGMATA][1] > 9
          on_buff_event(true, owner.current_chara_card_no, STATE_STIGMATA, @cc.status[STATE_STIGMATA][0], @cc.status[STATE_STIGMATA][1])
    ...
    def check_poison_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_POISON][1]).positive?
      end
    end
    ...
    def finish_poison_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_POISON][1]).positive?
        owner.damaged_event(1, IS_NOT_HOSTILE_DAMAGE) if owner.hit_point.positive?
    ...
    def check_poison2_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_POISON2][1]).positive?
      end
    end
    ...
    def finish_poison2_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_POISON2][1]).positive?
        owner.damaged_event(2, IS_NOT_HOSTILE_DAMAGE) if owner.hit_point.positive?
    ...
    def check_paralysis_state
      owner.move_point = 0 if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_PARALYSIS][1]).positive?
    end
    regist_event CheckParalysisStateEvent

    # 麻痺が終了される
    def finish_paralysis_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_PARALYSIS][1]).positive?
    ...
    def check_attack_up_state
      owner.tmp_power += @cc.status[STATE_ATK_UP][0] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_ATK_UP][1]).positive? && owner.initiative && owner.tmp_power.positive?
    end
    regist_event CheckAttackUpStateEvent

    # ATKアップが終了される
    def finish_attack_up_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_ATK_UP][1]).positive?
    ...
    def check_attack_down_state
      owner.tmp_power -= @cc.status[STATE_ATK_DOWN][0] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_ATK_DOWN][1]).positive? && owner.initiative && owner.tmp_power.positive?
    end
    regist_event CheckAttackDownStateEvent

    # ATKダウンが終了される
    def finish_attack_down_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_ATK_DOWN][1]).positive?
    ...
    def check_deffence_up_state
      owner.tmp_power += @cc.status[STATE_DEF_UP][0] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DEF_UP][1]).positive? && !owner.initiative
    end
    regist_event CheckDeffenceUpStateEvent

    # DEFアップが終了される
    def finish_deffence_up_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DEF_UP][1]).positive?
    ...
    def check_deffence_down_state
      owner.tmp_power -= @cc.status[STATE_DEF_DOWN][0] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DEF_DOWN][1]).positive? && !owner.initiative
    end
    regist_event CheckDeffenceDownStateEvent

    # DEFダウンが終了される
    def finish_deffence_down_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DEF_DOWN][1]).positive?
    ...
    def check_move_up_state
      owner.tmp_power += @cc.status[STATE_MOVE_UP][0] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_MOVE_UP][1]).positive?
    end
    regist_event CheckMoveUpStateEvent

    # DEFアップが終了される
    def finish_move_up_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_MOVE_UP][1]).positive?
    ...
    def check_move_down_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_MOVE_DOWN][1]).positive?
        if owner.tmp_power.positive?
          owner.tmp_power -= @cc.status[STATE_MOVE_DOWN][0]
          owner.tmp_power = 0 if owner.tmp_power.negative?
        elsif owner.tmp_power.negative?
          owner.tmp_power += @cc.status[STATE_MOVE_DOWN][0]
          owner.tmp_power = 0 if owner.tmp_power.positive?
        end
      end
    end
    ...
    def check_berserk_state
      duel.tmp_damage = duel.tmp_damage * 2 if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_BERSERK][1]).positive?
    end
    regist_event CheckBerserkStateEvent

    # バーサークが終了される
    def finish_berserk_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_BERSERK][1]).positive?
    ...
    def check_attack_stop_state
      if @cc && @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_STOP][1]).positive?
    ...
    def finish_stop_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_STOP][1]).positive?
    ...
    def check_seal_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_SEAL][1]).positive?
      end
    end
    ...
    def check_dead_count_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DEAD_COUNT][1]).positive?
      end
    end
    ...
    def check_control_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_CONTROL][1]).positive?
      end
    end
    ...
    def finish_control_state
      if @cc && (@cc.status[STATE_CONTROL][1]).positive? && !Charactor.attribute(@cc.charactor_id).include?('revisers')
    ...
    def check_undead_state
      duel.tmp_damage = 0 if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_UNDEAD][1]).positive? && !owner.initiative
    end
    regist_event CheckUndeadStateEvent

    # 不死が終了される
    def finish_undead_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_UNDEAD][1]).positive?
    ...
    def check_undead2_state
      duel.tmp_damage = 0 if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_UNDEAD2][1]).positive? && !owner.initiative
    end
    regist_event CheckUndead2StateEvent

    # 不死が終了される
    def finish_undead2_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_UNDEAD2][1]).positive?
    ...
    def check_stone_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_STONE][1]).positive?
    ...
    # リジェネ状態かどうかのチェック
    def check_regene_state; end
    regist_event CheckRegeneStateEvent

    # リジェネが終了される
    def finish_regene_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_REGENE][1]).positive?
    ...
    def check_bind_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_BIND][1]).positive?
        dmg = owner.move_point_abs > 2 ? 2 : owner.move_point_abs
    ...
    def check_chaos_state
      owner.tmp_power *= 2 if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_CHAOS][1]).positive?
    end
    ...
    def finish_chaos_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_CHAOS][1]).positive?
    ...
    def check_stigmata_state
      owner.tmp_power += @cc.status[STATE_STIGMATA][1] if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_STIGMATA][1]).positive? && owner.tmp_power.positive?
    end
    regist_event CheckStigmataAttackStateEvent
    regist_event CheckStigmataDefenceStateEvent

    # 聖痕が終了される
    def finish_stigmata_state; end
    ...
    def check_state_down_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_STATE_DOWN][1]).positive? && owner.tmp_power.positive?
    ...
    # 能力低下が終了される
    def finish_state_down_state; end
    ...
    def check_curse_attack_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_CURSE][1]).positive? && owner.initiative && duel.tmp_damage.positive?
    ...
    def check_bless_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_BLESS][1]).positive? && owner.tmp_power.positive?
    ...
    def check_barrier_state
      duel.tmp_damage = 0 if owner.invincible && !owner.initiative && duel.tmp_damage.positive?
    end
    ...
    def check_attack_stick_state
      owner.tmp_power *= 2 if @cc && @cc.index == owner.current_chara_card_no && @cc.status[STATE_STICK][0] == 1 && (@cc.status[STATE_STICK][1]).positive? && owner.initiative && owner.tmp_power.positive?
    end
    regist_event CheckAttackStickStateEvent

    # 棍術が使用されたかのチェック
    def check_deffence_stick_state
      owner.tmp_power *= 2 if @cc && @cc.index == owner.current_chara_card_no && @cc.status[STATE_STICK][0] == 2 && (@cc.status[STATE_STICK][1]).positive? && !owner.initiative
    end
    regist_event CheckDeffenceStickStateEvent

    # 棍術が終了される
    def finish_stick_state; end
    regist_event FinishStickStateEvent

    # 棍術を入れ替える
    def change_stick_state
      case @cc.status[STATE_STICK][0]
      when 1
    ...
    def check_cat_state_attack
      owner.tmp_power = 13 if @cc && @cc.index == owner.current_chara_card_no && (@cc.special_status[SPECIAL_STATE_CAT][1]).positive? && owner.initiative && owner.tmp_power.positive?
    end
    regist_event CheckCatStateAttackEvent

    # 防御力上書き
    def check_cat_state_defence
      owner.tmp_power = 7 if @cc && @cc.index == owner.current_chara_card_no && (@cc.special_status[SPECIAL_STATE_CAT][1]).positive? && !owner.initiative
    end
    regist_event CheckCatStateDefenceEvent

    # 猫状態の終了
    def finish_cat_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.special_status[SPECIAL_STATE_CAT][1]).positive?
        @cc.special_status[SPECIAL_STATE_CAT][1] -= 1 if @cc.status_update
        if (@cc.special_status[SPECIAL_STATE_CAT][1]).zero?
          off_transform_sequence(true)
        else
    ...
    def finish_doll_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DOLL][1]).positive?
    ...
    def finish_dark_state
      if @cc && @cc.index == owner.current_chara_card_no && (@cc.status[STATE_DARK][1]).positive?
    ...
    def is_dark?
      (@cc.status[STATE_DARK][1]).positive?
    end
    ...
    def finish_antiseptic_state
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_ANTISEPTIC][1]).positive?

        @cc.special_status[SPECIAL_STATE_ANTISEPTIC][1] -= 1 if @cc.status_update
        if (@cc.special_status[SPECIAL_STATE_ANTISEPTIC][1]).positive?
    ...
    def finish_magnetic_field_state
      @cc.special_status[SPECIAL_STATE_MAGNETIC_FIELD][1] -= 1 if @cc && @cc.index == owner.current_chara_card_no && (@cc.special_status[SPECIAL_STATE_MAGNETIC_FIELD][1]).positive? && @cc.status_update
    end
    regist_event FinishMagneticFieldStateEvent

    def is_magnetic?
      (@cc.special_status[SPECIAL_STATE_MAGNETIC_FIELD][1]).positive?
    end
    ...
    def use_sharpen_edge_state_damage
      if @cc && @cc.index == owner.current_chara_card_no && !owner.initiative &&
         (@cc.special_status[SPECIAL_STATE_SHARPEN_EDGE][1]).positive? &&
         @feats_enable[FEAT_SHARPEN_EDGE] && duel.tmp_damage.positive?
        ...
        finish_sharpen_edge_feat if (@cc.special_status[SPECIAL_STATE_SHARPEN_EDGE][1]).zero?
      end
    end
    ...
    def finish_sharpen_edge_state
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_SHARPEN_EDGE][1]).positive?
        @cc.special_status[SPECIAL_STATE_SHARPEN_EDGE][1] = 0
    ...
    def check_dealing_restriction_state
      foe.cards_max = foe.cards_max - @cc.special_status[SPECIAL_STATE_DEALING_RESTRICTION][0] if @cc&.using && (@cc.special_status[SPECIAL_STATE_DEALING_RESTRICTION][1]).positive?
    end
    regist_event CheckDealingRestrictionStateEvent

    def finish_dealing_restriction_state
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_DEALING_RESTRICTION][1]).positive?
    ...
    CONSTRAINT_BACKWARD = 2
    CONSTRAINT_STAY = 4
    CONSTRAINT_CHARA_CHANGE = 8
    # ターン開始
    def check_constraint_state
      owner.constraint_event(@cc.special_status[SPECIAL_STATE_CONSTRAINT][0]) if @cc&.using && (@cc.special_status[SPECIAL_STATE_CONSTRAINT][1]).positive?
    end
    regist_event CheckConstraintStateEvent

    # 終了
    def finish_constraint_state
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_CONSTRAINT][1]).positive?
    ...
    def forbidden_direction?(dir)
      ...
      return false if (@cc.special_status[SPECIAL_STATE_CONSTRAINT][1]).zero?
    ...
    def use_damage_insurance_damage
      if @cc && @cc.index == owner.current_chara_card_no && owner.initiative &&
         (@cc.special_status[SPECIAL_STATE_DAMAGE_INSURANCE][1]).positive? &&
         @feats_enable[FEAT_MEXTLI]

        if owner.tmp_power.positive? && duel.tmp_damage <= 0
          dmg = 0
          if owner.distance == 1
    ...
    def finish_override_skill_state
      if @cc && (@cc.special_status[SPECIAL_STATE_OVERRIDE_SKILL][1]).positive?
        @cc.special_status[SPECIAL_STATE_OVERRIDE_SKILL][1] -= 1 if @cc.status_update
        reset_override_feats if @cc.special_status[SPECIAL_STATE_OVERRIDE_SKILL][1] <= 0
      end
    end
    ...
    def reset_override_feats
      # 相手
      foe.chara_cards[@override_feats[3][:foe_index]].reset_override_my_feats if @override_feats&.key?(3) && @override_feats[3][:foe_index]
      # 自分自身
      owner.chara_cards[@override_feats[3][:owner_index]].reset_override_my_feats if @override_feats&.key?(3)
      @override_feats = {}
    end
    ...
    def check_stuffed_toys_state_damage
      if @cc.using && (@cc.special_status[SPECIAL_STATE_STUFFED_TOYS][1]).positive? && owner.initiative && owner.tmp_power.positive?
        owner.duel_message_event(DUEL_MSGDLG_DOLL_ATK)
        @cc.special_status[SPECIAL_STATE_STUFFED_TOYS][1].times do |_i|
    ...
    def finish_stuffed_toys_state_damage
      if @cc.using && !owner.initiative && (@cc.special_status[SPECIAL_STATE_STUFFED_TOYS][1]).positive? && duel.tmp_damage.positive?
    ...
    def use_monitoring_state_before_check
      @monitoring_feat_before_hp = owner.hit_points[owner.current_chara_card_no] if @cc&.using && (@cc.special_status[SPECIAL_STATE_MONITORING][1]).positive?
    end
    ...
    def use_monitoring_state_after_check
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_MONITORING][1]).positive? && (@monitoring_feat_before_hp < owner.hit_points[owner.current_chara_card_no])
        heal_pt = owner.hit_points[owner.current_chara_card_no] - @monitoring_feat_before_hp
        foe.healed_event(heal_pt) if heal_pt.positive?
      end
    end
    ...
    def finish_monitoring_state
      if @cc&.using && (@cc.special_status[SPECIAL_STATE_MONITORING][1]).positive?
    ...
    def finish_time_lag_draw_state
      if @cc && (@cc.special_status[SPECIAL_STATE_TIME_LAG_DROW][1]).positive?
        @cc.special_status[SPECIAL_STATE_TIME_LAG_DROW][1] -= 1 if @cc.status_update
        owner.special_dealed_event(duel.deck.draw_cards_event(4).each { |c| owner.dealed_event(c) }) if owner.initiative
      end
    end
    ...
    def finish_time_lag_buff_state
      if @cc && (@cc.special_status[SPECIAL_STATE_TIME_LAG_BUFF][1]).positive?
    ...
    def check_machine_cell_state
      if @cc && (@cc.special_status[SPECIAL_STATE_MACHINE_CELL][1]).positive? && (owner.tmp_power < 1)
        # 攻撃していない場合効果終了
    ...
    def use_machine_cell_state
      duel.tmp_damage += 2 if @cc && (@cc.special_status[SPECIAL_STATE_MACHINE_CELL][1]).positive? && owner.initiative && (owner.distance > 1)
    end
    regist_event UseMachineCellStateEvent

    # スキルが終了される
    def finish_machine_cell_state
      @cc.special_status[SPECIAL_STATE_MACHINE_CELL][1] -= 1 if @cc && (@cc.special_status[SPECIAL_STATE_MACHINE_CELL][1]).positive?
    end
    ...
    def check_ax_guard_state
      if @cc && (@cc.special_status[SPECIAL_STATE_AX_GUARD][1]).positive? && (owner.tmp_power < 1)
        # 攻撃していない場合効果終了
    ...
    def use_ax_guard_state
      duel.tmp_damage = (duel.tmp_damage / 2).to_i if @cc && (@cc.special_status[SPECIAL_STATE_AX_GUARD][1]).positive? && !owner.initiative
    end
    regist_event UseAxGuardStateEvent

    # スキルが終了される
    def finish_ax_guard_state
      @cc.special_status[SPECIAL_STATE_AX_GUARD][1] -= 1 if @cc && (@cc.special_status[SPECIAL_STATE_AX_GUARD][1]).positive?
    end
    ...
    def update_buff(player, id, value, idx = -1, turn = -1)
      ...
      index = idx.negative? ? target.current_chara_card_no : idx
    ...
    def state_adjust(player, index, id, value, turn)
      ...
      # ターン延長処理
      if target.chara_cards[index].event.passives_enable[PASSIVE_CURSE_SIGN] && turn.positive?
        case id
        ...
        when STATE_STOP, STATE_STIGMATA, STATE_STATE_DOWN, STATE_STICK, STATE_CURSE, STATE_BLESS, STATE_TARGET
          ...
          target.chara_cards[index].status[id][1] = tmp_turn if tmp_turn > turn
        end
      end
    ...
    # 共存不可能な状態異常を消す
    def check_exclusive_state(player, index, id, _value, _turn)
      target = player ? owner : foe
      case id
      when STATE_POISON
        if (target.chara_cards[index].status[STATE_POISON2][1]).positive?
      ...
      when STATE_POISON2
        if (target.chara_cards[index].status[STATE_POISON][1]).positive?
    ...
    def check_field_status_finish_turn
      if @cc&.using
        owner.field_status.each_with_index do |val, i|
          next unless (val[1]).positive?
    ...
    # 強打が使用される
    def use_smash_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SMASH]) if @feats_enable[FEAT_SMASH]
    end
    ...
    def use_bloody_howl_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLOODY_HOWL]) if @feats_enable[FEAT_BLOODY_HOWL]
    end
    regist_event UseBloodyHowlFeatEvent

    # ブラッディハウルが使用終了される
    def finish_bloody_howl_feat
      use_feat_event(@feats[FEAT_BLOODY_HOWL]) if @feats_enable[FEAT_BLOODY_HOWL]
    end
    ...
    # 精密射撃が使用される
    def use_aiming_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_AIMING]) if @feats_enable[FEAT_AIMING]
    end
    ...
    def finish_precision_fire_feat
      use_feat_event(@feats[FEAT_PRECISION_FIRE]) if @feats_enable[FEAT_PRECISION_FIRE]
    end
    ...
    def use_precision_fire_feat_damage
      if @feats_enable[FEAT_PRECISION_FIRE]
        ...
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, @precision_fire_arrow_const - 2)) if foe.distance == 3 && @precision_fire_arrow_const > 2
      end
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_strike_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_STRIKE]) if @feats_enable[FEAT_STRIKE]
    end
    regist_event UseStrikeFeatEvent

    # 精密射撃が使用終了
    def finish_strike_feat
      use_feat_event(@feats[FEAT_STRIKE]) if @feats_enable[FEAT_STRIKE]
    end
    ...
    def use_combo_feat
      if @feats_enable[FEAT_COMBO]
        # 2020-10-08 - Issue#191
        pow = Feat.pow(@feats[FEAT_COMBO])
        @cc.owner.tmp_power += pow

        # Ex
        if pow == 6
          sword_card_set = owner.get_card_points_set(ActionCard::SWD)
          @cc.owner.tmp_power += pow if sword_card_set[1] # SWD == 2
          @cc.owner.tmp_power += pow * 2 if sword_card_set[4] # SWD == 5
        end
      end
    end
    ...
    def finish_sword_dance_feat
      use_feat_event(@feats[FEAT_SWORD_DANCE]) if @feats_enable[FEAT_SWORD_DANCE]
    end
    ...
    def use_sword_dance_feat_damage
      if @feats_enable[FEAT_SWORD_DANCE]
        ...
        @sword_dance_card_set.each_with_index do |a, i|
          ...
          foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, i)) if i.positive?
        end
    ...
    # 有効の場合必殺技IDを返す
    def use_thorn_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_THORN]) if @feats_enable[FEAT_THORN]
    end
    regist_event UseThornFeatEvent

    # 茨の森が使用終了
    def finish_thorn_feat
      use_feat_event(@feats[FEAT_THORN]) if @feats_enable[FEAT_THORN]
    end
    ...
    def use_thorn_feat_damage
      if @feats_enable[FEAT_THORN]
        ...
        duel.first_entrant.damaged_event(attribute_damage(ATTRIBUTE_COUNTER, foe, duel.tmp_dice_heads_def - duel.tmp_dice_heads_atk)) if duel.tmp_damage <= 0 && foe.tmp_power.positive? && (duel.tmp_dice_heads_def - duel.tmp_dice_heads_atk).positive?
    ...
    def use_purple_lightning_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PURPLE_LIGHTNING]) if @feats_enable[FEAT_PURPLE_LIGHTNING]
    end
    regist_event UsePurpleLightningFeatEvent

    # 紫電が使用終了
    def finish_purple_lightning_feat
      use_feat_event(@feats[FEAT_PURPLE_LIGHTNING]) if @feats_enable[FEAT_PURPLE_LIGHTNING]
    end
    ...
    def use_purple_lightning_feat_damage
      if @feats_enable[FEAT_PURPLE_LIGHTNING]
        if duel.tmp_damage.positive?
          ...
          if foe.current_chara_card.kind != CC_KIND_MONSTAR &&
             foe.current_chara_card.kind != CC_KIND_BOSS_MONSTAR &&
             foe.current_chara_card.kind != CC_KIND_PROFOUND_BOSS && (duel.tmp_damage > foe_cards_cnt)
            deal_count = foe_cards_cnt
          end
    ...
    # 有効の場合必殺技IDを返す
    def use_charge_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CHARGE]) if @feats_enable[FEAT_CHARGE]
    end
    ...
    def use_charged_thrust_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CHARGED_THRUST]) if @feats_enable[FEAT_CHARGED_THRUST]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_mirage_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MIRAGE]) if @feats_enable[FEAT_MIRAGE] && @cc.owner.initiative == false
    end
    ...
    def use_frenzy_eyes_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_FRENZY_EYES]
    end
    regist_event UseFrenzyEyesFeatEvent

    # 狂気の眼窩が使用終了
    def finish_frenzy_eyes_feat
      use_feat_event(@feats[FEAT_FRENZY_EYES]) if @feats_enable[FEAT_FRENZY_EYES]
    end
    ...
    def use_abyss_feat
      @feat_abyss_bornus_damage = 0 if @feats_enable[FEAT_ABYSS]
    end
    ...
    def finish_abyss_feat
      if @feats_enable[FEAT_ABYSS]
        ...
        duel.first_entrant.healed_event(d) if Feat.pow(@feats[FEAT_ABYSS]).positive?
    ...
    def use_rapid_sword_feat
      if @feats_enable[FEAT_RAPID_SWORD]
        # 2020-10-08 - Issue#175
        @cc.owner.tmp_power += if Feat.pow(@feats[FEAT_RAPID_SWORD]) == 2
                                 2 + ((@cc.owner.get_battle_table_point(ActionCard::SWD) + 1) / 2).to_i
                               else
                                 4 + @cc.owner.get_battle_table_point(ActionCard::SWD)
                               end
      end
    end
    ...
    def use_rapid_sword_r2_feat
      @cc.owner.tmp_power += @cc.owner.get_battle_table_point(ActionCard::SWD) + Feat.pow(@feats[FEAT_RAPID_SWORD_R2]) if @feats_enable[FEAT_RAPID_SWORD_R2]
    end
    ...
    def finish_power_stock_feat
      if @feats_enable[FEAT_POWER_STOCK]
        ...
        # 2020-10-08 - Issue#193
        pow = Feat.pow(@feats[FEAT_POWER_STOCK])
        keep_round = pow == 9 ? 2 : 1
        set_state(@cc.status[STATE_ATK_UP], pow, keep_round)
        on_buff_event(true, owner.current_chara_card_no, STATE_ATK_UP, @cc.status[STATE_ATK_UP][0], @cc.status[STATE_ATK_UP][1])
      end
    end
    ...
    def finish_mortal_style_feat
      if @feats_enable[FEAT_MORTAL_STYLE]
        ...
        r = rand(1..2)
        @cc.status[STATE_ATK_UP][0] = (@cc.status[STATE_ATK_UP][1]).positive? ? @cc.status[STATE_ATK_UP][0] + r : Feat.pow(@feats[FEAT_MORTAL_STYLE])
        @cc.status[STATE_DEF_UP][0] = (@cc.status[STATE_DEF_UP][1]).positive? ? @cc.status[STATE_DEF_UP][0] + r : Feat.pow(@feats[FEAT_MORTAL_STYLE])

        if @cc.status[STATE_ATK_UP][0] > 9 && (@cc.status[STATE_ATK_DOWN][1]).positive?
        ...
        if @cc.status[STATE_DEF_UP][0] > 9 && (@cc.status[STATE_DEF_DOWN][1]).positive?
    ...
    def finish_shadow_shot_feat
      use_feat_event(@feats[FEAT_SHADOW_SHOT]) if @feats_enable[FEAT_SHADOW_SHOT]
    end
    ...
    def use_red_fang_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RED_FANG]) if @feats_enable[FEAT_RED_FANG]
    end
    regist_event UseRedFangFeatEvent

    # 赫い牙が使用される
    def finish_red_fang_feat
      use_feat_event(@feats[FEAT_RED_FANG]) if @feats_enable[FEAT_RED_FANG]
    end
    ...
    def use_blessing_blood_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLESSING_BLOOD]) if @feats_enable[FEAT_BLESSING_BLOOD]
    end
    regist_event UseBlessingBloodFeatEvent

    # 血の恵みが使用終了
    def finish_blessing_blood_feat
      use_feat_event(@feats[FEAT_BLESSING_BLOOD]) if @feats_enable[FEAT_BLESSING_BLOOD]
    end
    ...
    def use_blessing_blood_feat_damage
      if @feats_enable[FEAT_BLESSING_BLOOD]
        # 2020-10-08 - Issue#195
        @cc.owner.healed_event(1) if Feat.pow(@feats[FEAT_BLESSING_BLOOD]) == 10

        # ダメージがマイナス（ダイスの結果防御点の方が上回った場合）
        if duel.tmp_damage <= 0 && foe.tmp_power.positive? && (duel.tmp_dice_heads_def - duel.tmp_dice_heads_atk).positive?
          # 回復処理
          @cc.owner.healed_event(duel.tmp_dice_heads_def - duel.tmp_dice_heads_atk)
        end
    ...
    def finish_counter_preparation_feat
      use_feat_event(@feats[FEAT_COUNTER_PREPARATION]) if @feats_enable[FEAT_COUNTER_PREPARATION]
    end
    ...
    def use_counter_preparation_feat_damage
      if @feats_enable[FEAT_COUNTER_PREPARATION]
        @cc.owner.special_dealed_event(duel.deck.draw_cards_event(duel.tmp_damage + Feat.pow(@feats[FEAT_COUNTER_PREPARATION])).each { |c| @cc.owner.dealed_event(c) }) if duel.tmp_damage.positive?
    ...
    def use_karmic_time_feat
      if @feats_enable[FEAT_KARMIC_TIME]
        ...
        foe.battle_table = []
        if Feat.pow(@feats[FEAT_KARMIC_TIME]).positive?
    ...
    def finish_karmic_ring_feat
      @feats_enable[FEAT_KARMIC_RING] = false if @feats_enable[FEAT_KARMIC_RING]
    end
    ...
    def use_karmic_string_feat
      if @feats_enable[FEAT_KARMIC_STRING]
        ...
        Feat.pow(@feats[FEAT_KARMIC_STRING]).times do
          steal_deal(foe.cards[rand(foe.cards.size)]) if foe.cards.size.positive?
        end
    ...
    def use_hi_smash_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_HI_SMASH]
    end
    ...
    def use_hi_aiming_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_HI_AIMING]
    end
    ...
    def use_hi_rapid_sword_feat
      @cc.owner.tmp_power += ((@cc.owner.get_battle_table_point(ActionCard::SWD) + 1) * 0.75).to_i if @feats_enable[FEAT_HI_RAPID_SWORD]
    end
    ...
    def use_hi_frenzy_eyes_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_HI_FRENZY_EYES]
    end
    regist_event UseHiFrenzyEyesFeatEvent

    # 狂気の眼窩2が使用終了
    def finish_hi_frenzy_eyes_feat
      use_feat_event(@feats[FEAT_HI_FRENZY_EYES]) if @feats_enable[FEAT_HI_FRENZY_EYES]
    end
    ...
    def use_hi_shadow_shot_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_HI_SHADOW_SHOT]
    end
    regist_event UseHiShadowShotFeatEvent

    # 影撃ち2が使用される
    def finish_hi_shadow_shot_feat
      use_feat_event(@feats[FEAT_HI_SHADOW_SHOT]) if @feats_enable[FEAT_HI_SHADOW_SHOT]
    end
    ...
    def use_desperado_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DESPERADO]) if @feats_enable[FEAT_DESPERADO]
    end
    ...
    def use_reject_sword_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_REJECT_SWORD]) if @feats_enable[FEAT_REJECT_SWORD]
    end
    ...
    def use_counter_guard_feat
      if @feats_enable[FEAT_COUNTER_GUARD] || @feats_enable[FEAT_EX_COUNTER_GUARD]
        if @lock
          ...
          f_id = @feats[FEAT_COUNTER_GUARD] || @feats[FEAT_EX_COUNTER_GUARD]
          @cc.owner.tmp_power = foe.tmp_power + (2 - @cc.owner.distance) * Feat.pow(f_id)
          @cc.owner.tmp_power = 0 if @cc.owner.tmp_power.negative?
          @lock = true
    ...
    def use_seal_chain_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SEAL_CHAIN]) if @feats_enable[FEAT_SEAL_CHAIN]
    end
    regist_event UseSealChainFeatEvent

    # 封印の鎖が使用される
    def finish_seal_chain_feat
      use_feat_event(@feats[FEAT_SEAL_CHAIN]) if @feats_enable[FEAT_SEAL_CHAIN]
    end
    ...
    def use_purification_light_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PURIFICATION_LIGHT]) if @feats_enable[FEAT_PURIFICATION_LIGHT]
    end
    regist_event UsePurificationLightFeatEvent

    # 降魔の光が使用される
    def finish_purification_light_feat
      use_feat_event(@feats[FEAT_PURIFICATION_LIGHT]) if @feats_enable[FEAT_PURIFICATION_LIGHT]
    end
    ...
    def use_purification_light_feat_damage
      if @feats_enable[FEAT_PURIFICATION_LIGHT]
        if duel.tmp_damage.positive? && !instant_kill_guard?(foe)
    ...
    def use_reject_blade_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_REJECT_BLADE]
    end
    ...
    def use_spell_chain_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_SPELL_CHAIN]
    end
    regist_event UseSpellChainFeatEvent

    # 封印の鎖が使用される
    def finish_spell_chain_feat
      use_feat_event(@feats[FEAT_SPELL_CHAIN]) if @feats_enable[FEAT_SPELL_CHAIN]
    end
    ...
    def use_indomitable_mind_feat
      owner.is_indomitable = true if @feats_enable[FEAT_INDOMITABLE_MIND] && Feat.pow(@feats[FEAT_INDOMITABLE_MIND]) > 1
    end
    ...
    def finish_indomitable_mind_feat
      if @feats_enable[FEAT_INDOMITABLE_MIND]
        owner.is_indomitable = false if Feat.pow(@feats[FEAT_INDOMITABLE_MIND]) > 1
        owner.cured_event # 2020-10-08 - Issue#174
    ...
    def use_drain_soul_feat
      if @feats_enable[FEAT_DRAIN_SOUL]
        ...
        # 相手のカードを奪う
        # 2020-10-08 - Issue#194
        if foe.cards.size.positive?
          tmp_cards = foe.cards.dup.sort_by { rand }
          tmp_count = 0
          tmp_cards.each do |c|
            if c.u_type == ActionCard::SPC || c.b_type == ActionCard::SPC
    ...
    def check_back_stab_feat
      if (foe.current_chara_card.status[STATE_PARALYSIS][1]).positive?
        # カードをON情報をリセットしてから
    ...
    def use_back_stab_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BACK_STAB]) if @feats_enable[FEAT_BACK_STAB] && (foe.current_chara_card.status[STATE_PARALYSIS][1]).positive?
    end
    ...
    # 2020-10-08 - Issue#192
    def check_enlightened_point_feat
      @enlightened_seal_more = owner.get_battle_table_point(ActionCard::SPC) >= 5 if @feats_enable[FEAT_ENLIGHTENED]
    end
    regist_event CheckEnlightenedPointFeatEvent
    ...
    def use_enlightened_feat
      if @feats_enable[FEAT_ENLIGHTENED]
        ...
        # 2020-10-08 - Issue#192
        pow = Feat.pow(@feats[FEAT_ENLIGHTENED])
        pow = 1 unless @enlightened_seal_more
        buffed = set_state(foe.current_chara_card.status[STATE_SEAL], 1, pow)
        on_buff_event(false, foe.current_chara_card_no, STATE_SEAL, foe.current_chara_card.status[STATE_SEAL][0], foe.current_chara_card.status[STATE_SEAL][1]) if buffed
    ...
    def use_dark_whirlpool_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DARK_WHIRLPOOL]) if @feats_enable[FEAT_DARK_WHIRLPOOL]
    end
    ...
    def use_karmic_phantom_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_KARMIC_PHANTOM]) if @feats_enable[FEAT_KARMIC_PHANTOM]
    end
    ...
    def finish_recovery_wave_feat
      if @feats_enable[FEAT_RECOVERY_WAVE]
        ...
        @cc.owner.hit_points.each_index do |i|
          @cc.owner.party_healed_event(i, Feat.pow(@feats[FEAT_RECOVERY_WAVE])) if @cc.owner.hit_points[i].positive?
        end
      end
    end
    ...
    def use_deffence_shooting_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::ARW) + Feat.pow(@feats[FEAT_DEFFENCE_SHOOTING])) if @feats_enable[FEAT_DEFFENCE_SHOOTING]
    end
    ...
    def use_shadow_attack_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SHADOW_ATTACK]) if @feats_enable[FEAT_SHADOW_ATTACK]
    end
    ...
    def use_suicidal_tendencies_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SUICIDAL_TENDENCIES]) * @cc.owner.get_battle_table_point(ActionCard::SPC) if @feats_enable[FEAT_SUICIDAL_TENDENCIES]
    end
    ...
    # ミスフィットが使用される
    # 有効の場合必殺技IDを返す
    def use_misfit_feat; end
    regist_event UseMisfitFeatEvent

    # ミスフィットが使用終了
    def finish_misfit_feat; end
    ...
    def use_lets_knife_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LETS_KNIFE]) * @cc.owner.battle_table.count if @feats_enable[FEAT_LETS_KNIFE]
    end
    ...
    def use_single_heart_feat
      if @feats_enable[FEAT_SINGLE_HEART]
        ...
        Feat.pow(@feats[FEAT_SINGLE_HEART]).times do
          next unless foe.cards.size.positive?
    ...
    # ２つの身体が使用される
    # 有効の場合必殺技IDを返す
    def use_double_body_feat; end
    regist_event UseDoubleBodyFeatEvent

    # ２つの身体が使用終了
    def finish_double_body_feat
      use_feat_event(@feats[FEAT_DOUBLE_BODY]) if @feats_enable[FEAT_DOUBLE_BODY]
    end
      ...
      if @feats_enable[FEAT_DOUBLE_BODY]
        ...
        if duel.tmp_damage.positive?
          ...
          duel.first_entrant.hit_points.each_index do |i|
            hps << i if duel.first_entrant.hit_points[i].positive?
          end
          # 相手のカードを回転する
          if Feat.pow(@feats[FEAT_DOUBLE_BODY]).positive?
    ...
    def use_nine_soul_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_NINE_SOUL]) if @feats_enable[FEAT_NINE_SOUL]
    end
    ...
    def finish_thirteen_eyes_feat
      if @feats_enable[FEAT_THIRTEEN_EYES] || @feats_enable[FEAT_EX_THIRTEEN_EYES]
        f_id = @feats[FEAT_THIRTEEN_EYES] || @feats[FEAT_EX_THIRTEEN_EYES]
        use_feat_event(f_id)
        owner.tmp_power = 13
        foe.tmp_power = 0
        owner.point_rewrite_event
    ...
    def use_thirteen_eyes_feat_damage
      if @feats_enable[FEAT_THIRTEEN_EYES] || @feats_enable[FEAT_EX_THIRTEEN_EYES]
        f_no = @feats[FEAT_THIRTEEN_EYES] ? FEAT_THIRTEEN_EYES : FEAT_EX_THIRTEEN_EYES
        @feats_enable[f_no] = false
        if Feat.pow(@feats[f_no]) > 13 && (duel.tmp_damage < 4)
          rec_damage = duel.tmp_damage
          @cc.owner.dice_roll_event(duel.battle_result)
    ...
    def use_life_drain_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LIFE_DRAIN]) if @feats_enable[FEAT_LIFE_DRAIN]
    end
    regist_event UseLifeDrainFeatEvent

    # ライフドレインが使用される
    def finish_life_drain_feat
      use_feat_event(@feats[FEAT_LIFE_DRAIN]) if @feats_enable[FEAT_LIFE_DRAIN]
    end
    ...
    # 必殺技の状態
    def use_random_curse_feat; end
    regist_event UseRandomCurseFeatEvent

    # ランダムカースが使用される
    def finish_random_curse_feat
      use_feat_event(@feats[FEAT_RANDOM_CURSE]) if @feats_enable[FEAT_RANDOM_CURSE]
    end
    ...
    def use_double_attack_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DOUBLE_ATTACK]) if @feats_enable[FEAT_DOUBLE_ATTACK]
    end
    ...
    def finish_party_damage_feat
      if @feats_enable[FEAT_PARTY_DAMAGE]
        ...
        duel.second_entrant.hit_points.each_index do |i|
          hps << i if duel.second_entrant.hit_points[i].positive?
        end
    ...
    def finish_guard_feat
      use_feat_event(@feats[FEAT_GUARD]) if @feats_enable[FEAT_GUARD]
    end
    ...
    def use_death_control_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DEATH_CONTROL]) if @feats_enable[FEAT_DEATH_CONTROL]
    end
    regist_event UseDeathControlFeatEvent

    # 自壊攻撃が使用される
    def finish_death_control_feat; end
    ...
    def use_death_control_feat_damage
      if @feats_enable[FEAT_DEATH_CONTROL]
        ...
        if duel.tmp_damage.positive?
          ...
          if Feat.pow(@feats[FEAT_DEATH_CONTROL]) != 15 || foe_dead_count_num.zero?
            ...
            if own_dead_count_num.positive? && own_dead_count_num < 9
    ...
    # 必殺技の状態
    def use_wit_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_WIT]) if @feats_enable[FEAT_WIT]
    end
    ...
    def use_thorn_care_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::MOVE) * Feat.pow(@feats[FEAT_THORN_CARE])) if @feats_enable[FEAT_THORN_CARE]
    end
    regist_event UseThornCareFeatEvent

    # 茨の構えが使用終了
    def finish_thorn_care_feat
      use_feat_event(@feats[FEAT_THORN_CARE]) if @feats_enable[FEAT_THORN_CARE]
    end
    ...
    def use_liberating_sword_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LIBERATING_SWORD]) if @feats_enable[FEAT_LIBERATING_SWORD]
    end
    regist_event UseLiberatingSwordFeatEvent

    # 解放剣が使用される
    def finish_liberating_sword_feat
      use_feat_event(@feats[FEAT_LIBERATING_SWORD]) if @feats_enable[FEAT_LIBERATING_SWORD]
    end
    ...
    # 必殺技の状態
    def use_one_slash_feat; end
    regist_event UseOneSlashFeatEvent

    # 一閃が使用される
    def finish_one_slash_feat
      use_feat_event(@feats[FEAT_ONE_SLASH]) if @feats_enable[FEAT_ONE_SLASH]
    end
    ...
    def use_one_slash_feat_damage
      if @feats_enable[FEAT_ONE_SLASH]
        # 2020-10-08 - Issue #176
        pow = Feat.pow(@feats[FEAT_ONE_SLASH])
        if pow == 7
          set_state(@cc.status[STATE_ATK_UP], pow, 3)
        else
          set_state(@cc.status[STATE_ATK_UP], pow, 2)
        end
        on_buff_event(true, owner.current_chara_card_no, STATE_ATK_UP, @cc.status[STATE_ATK_UP][0], @cc.status[STATE_ATK_UP][1])
    ...
    def finish_ten_slash_feat
      if @feats_enable[FEAT_TEN_SLASH]
        ...
        pow = Feat.pow(@feats[FEAT_TEN_SLASH])
        # 2020-10-08 - Issue#179
        if pow == 7
          duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, 2))
          set_state(@cc.status[STATE_DEF_UP], pow, 3)
        else
          duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, 1))
          set_state(@cc.status[STATE_DEF_UP], pow, 2)
        end
    ...
    def use_handled_slash_feat
      @cc.owner.tmp_power += 6 if @feats_enable[FEAT_HANDLED_SLASH]
    end
    regist_event UseHandledSlashFeatEvent

    # 百閃が使用される
    def finish_handled_slash_feat
      use_feat_event(@feats[FEAT_HANDLED_SLASH]) if @feats_enable[FEAT_HANDLED_SLASH]
    end
    ...
    def use_handled_slash_feat_damage
      if @feats_enable[FEAT_HANDLED_SLASH]
        if (@cc.status[STATE_ATK_UP][1]).positive? && (@cc.status[STATE_DEF_UP][1]).positive?
          tmp_dice_atk = duel.tmp_dice_heads_atk
          tmp_dice_def = duel.tmp_dice_heads_def
          (Feat.pow(@feats[FEAT_HANDLED_SLASH]) - 1).times.each do |_i|
            # １回目のダイスを振ってダメージを保存
            rec_damage = duel.tmp_damage
            # ダメージ計算をもう１度実行
            @cc.owner.dice_roll_event(duel.battle_result)
    ...
    def use_handled_slash_r_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_HANDLED_SLASH_R]
    end
    regist_event UseHandledSlashRFeatEvent

    # 百閃が使用される
    def finish_handled_slash_r_feat
      use_feat_event(@feats[FEAT_HANDLED_SLASH_R]) if @feats_enable[FEAT_HANDLED_SLASH_R]
    end
    ...
    def use_handled_slash_r_feat_damage
      if @feats_enable[FEAT_HANDLED_SLASH_R]
        ...
        elsif (@cc.status[STATE_ATK_UP][1]).positive? || (@cc.status[STATE_DEF_UP][1]).positive?
          add_roll = rand(1..2)
        end

        tmp_dice_atk = duel.tmp_dice_heads_atk
        tmp_dice_def = duel.tmp_dice_heads_def
        add_roll.times.each do |_i|
          # １回目のダイスを振ってダメージを保存
          rec_damage = duel.tmp_damage
          # ダメージ計算をもう１度実行
          @cc.owner.dice_roll_event(duel.battle_result)
    ...
    def use_curse_care_feat_heal1
      if @feats_enable[FEAT_CURSE_CARE]
        ...
        if curse_care_hp_check
          curse_care_healing if @curse_care_is_ex # 2020-10-08 - Issue#180
    ...
    def use_curse_care_feat_damage
      if @feats_enable[FEAT_CURSE_CARE]
        ...
        damage = duel.tmp_damage.negative? ? 0 : duel.tmp_damage
        curse_care_healing if curse_care_hp_check(damage) && @curse_care_is_ex
      end
    end
    ...
    def use_curse_care_feat_heal2
      curse_care_healing if @feats_enable[FEAT_CURSE_CARE] && curse_care_hp_check && @curse_care_is_ex
    end
    ...
    def reset_flags
      ...
      # 2020-10-08 - Issue#180
      @curse_care_dead_line = Feat.pow(@feats[FEAT_CURSE_CARE]) == 7 ? 4 : 3   # HPがdead_line以下ならば発動
      @curse_care_is_ex = Feat.pow(@feats[FEAT_CURSE_CARE]) == 7               # Ex技？
    end

    def curse_care_play_cutin
      unless @curse_care_cutin_played
    ...
    def curse_care_hp_check(dmg = 0)
      if @cc.owner.hit_point <= @curse_care_dead_line + dmg
        ...
        #  2020-10-08 - Issue#180 - remove `if dmg > 0`
        @curse_care_actuate_normal = true
        true
      else
        false
      end
    ...
    def curse_care_healing
      if @curse_care_actuate_ex && !@curse_care_healed && @cc.owner.hit_point.positive?
    ...
    def use_rakshasa_stance_feat
      use_feat_event(@feats[FEAT_RAKSHASA_STANCE]) if @feats_enable[FEAT_RAKSHASA_STANCE]
    end
    ...
    def on_rakshasa_stance_feat
      add_singlton_method_rakshasa_stance_feat if @cc&.using && @feats_enable[FEAT_RAKSHASA_STANCE]
    end
    ...
    def off_rakshasa_stance_feat
      remove_singlton_method_rakshasa_stance_feat if @cc&.using && @feats_enable[FEAT_RAKSHASA_STANCE]
    end
    regist_event OffRakshasaStanceFeatEvent

    def finish_rakshasa_stance_feat
      if @cc&.using && @feats_enable[FEAT_RAKSHASA_STANCE]
    ...
    def use_moon_shine_feat
      @cc.owner.tmp_power += 4 if @feats_enable[FEAT_MOON_SHINE]
    end
    ...
    def use_rapture_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RAPTURE]) if @feats_enable[FEAT_RAPTURE]
    end
    regist_event UseRaptureFeatEvent

    # ラプチュアが使用終了
    def finish_rapture_feat
      use_feat_event(@feats[FEAT_RAPTURE]) if @feats_enable[FEAT_RAPTURE]
    end
    ...
    def use_moving_one_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MOVING_ONE]) if @feats_enable[FEAT_MOVING_ONE] && (@cc.level > foe.current_chara_card.level)
    end
    ...
    def use_arrogant_one_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ARROGANT_ONE]) if @feats_enable[FEAT_ARROGANT_ONE]
    end
    ...
    def use_eating_one_feat
      if @feats_enable[FEAT_EATING_ONE]
        bonus = 0
        bonus = 2 if Feat.pow(@feats[FEAT_EATING_ONE]) == 6 && (foe.current_chara_card.status[STATE_STONE][1]).positive?
    ...
    def finish_reviving_one_feat
      if @feats_enable[FEAT_REVIVING_ONE]
        ...
        if Feat.pow(@feats[FEAT_REVIVING_ONE]) > 3
          ...
          on_buff_event(true, owner.current_chara_card_no, STATE_DEF_UP, @cc.status[STATE_DEF_UP][0], @cc.status[STATE_DEF_UP][1])
          owner.healed_event(1) if owner.hit_point.positive?
        end
    ...
    def finish_white_light_feat
      if @feats_enable[FEAT_WHITE_LIGHT]
        ...
        # 2020-10-08 - Issue#169
        p *= 2 if foe.hit_point >= (foe.current_chara_card.hp - 2)
    ...
    def use_crystal_shield_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CRYSTAL_SHIELD]) if @feats_enable[FEAT_CRYSTAL_SHIELD]
    end
    ...
    def use_snow_balling_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SNOW_BALLING]) if @feats_enable[FEAT_SNOW_BALLING]
    end
    regist_event UseSnowBallingFeatEvent

    # 精密射撃が使用終了
    def finish_snow_balling_feat
      use_feat_event(@feats[FEAT_SNOW_BALLING]) if @feats_enable[FEAT_SNOW_BALLING]
    end
    ...
    def use_snow_balling_feat_damage
      if @feats_enable[FEAT_SNOW_BALLING] && duel.tmp_damage.positive?
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << i if duel.second_entrant.hit_points[i].positive?
        end
    ...
    def use_snow_balling_feat_const_damage
      if @feats_enable[FEAT_SNOW_BALLING]
        # ダメージがプラスなら
        attribute_party_damage(foe, get_hps(foe), 2, ATTRIBUTE_CONSTANT, TARGET_TYPE_RANDOM) if @snow_balling_const_actuated
    ...
    def use_obituary_feat
      if @feats_enable[FEAT_OBITUARY]
        ...
        foe.hit_points.each_index do |i|
          hps << [i] if foe.hit_points[i].zero?
        end

        owner.hit_points.each_index do |i|
          hps << [i] if i != owner.current_chara_card_no && owner.hit_points[i].zero?
        end

        # 2020-10-08 - Issue#172
        @cc.owner.tmp_power += 4 + Feat.pow(@feats[FEAT_OBITUARY]) * hps.size
      end
    end
    ...
    def finish_obituary_feat
      use_feat_event(@feats[FEAT_OBITUARY]) if @feats_enable[FEAT_OBITUARY]
    end
    ...
    def use_obituary_feat_damage
      if @feats_enable[FEAT_OBITUARY]
        # ダメージがプラスなら
        duel.get_event_deck(foe).freez_event_cards(1) if duel.tmp_damage.positive?
    ...
    def finish_solvent_rain_feat
      if @feats_enable[FEAT_SOLVENT_RAIN]
        ...
        if Feat.pow(@feats[FEAT_SOLVENT_RAIN]) == 7
          ...
          hps = []
          foe.hit_points.each_index do |i|
            hps << [false, i] if foe.hit_points[i].positive?
          end

          owner.hit_points.each_index do |i|
            hps << [true, i] if i != owner.current_chara_card_no && owner.hit_points[i].positive?
          end

          # 自分にかかる確率は3/4
          own_buff = rand(4) != 0

          hps = if hps.size == 1 || own_buff
                  [[true, owner.current_chara_card_no]] + hps.shuffle
                else
                  hps.shuffle
                end
          debuff_times.times do |i|
            ...
            on_buff_event(player,
                          ...
                          target.chara_cards[hps[i][1]].status[STATE_DEAD_COUNT][1])
          end
        end
      end
    end
    ...
    def use_razors_edge_feat
      foe.tmp_power = foe.tmp_power / 2 if @feats_enable[FEAT_RAZORS_EDGE] || @feats_enable[FEAT_EX_RAZORS_EDGE]
    end
    ...
    def finish_razors_edge_feat
      if @feats_enable[FEAT_RAZORS_EDGE] || @feats_enable[FEAT_EX_RAZORS_EDGE]
        ...
        @feats_enable[f_no] = false
        # オーバードウズ
        owner.healed_event(Feat.pow(@feats[f_no])) if @feats_enable[FEAT_OVER_DOSE] || @feats_enable[FEAT_THUNDER_STRUCK]
      end
    end
    ...
    def finish_hells_bell_feat
      if @feats_enable[FEAT_HELLS_BELL]
        ...
        foe.hit_points.each_with_index do |v, i|
          hps_f << i if v.positive?
        end
        attribute_party_damage(foe, hps_f, Feat.pow(@feats[FEAT_HELLS_BELL]), ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if hps_f.size.positive?

        # 自身を除く自デッキ全体にダメージ
        hps_o = []
        owner.hit_points.each_with_index do |v, i|
          hps_o << i if v.positive? && i != owner.current_chara_card_no
        end
        attribute_party_damage(owner, hps_o, Feat.pow(@feats[FEAT_HELLS_BELL]), ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL, 1, IS_NOT_HOSTILE_DAMAGE) if hps_o.size.positive?

        # オーバードウズ
        owner.healed_event(Feat.pow(@feats[FEAT_HELLS_BELL])) if @feats_enable[FEAT_OVER_DOSE] || @feats_enable[FEAT_THUNDER_STRUCK]
      end
    end
    ...
    def use_poison_skin_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_POISON_SKIN]) if @feats_enable[FEAT_POISON_SKIN]
    end
    regist_event UsePoisonSkinFeatEvent

    # 毒竜燐が使用終了
    def finish_poison_skin_feat
      use_feat_event(@feats[FEAT_POISON_SKIN]) if @feats_enable[FEAT_POISON_SKIN]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_roar_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ROAR]) if @feats_enable[FEAT_ROAR]
    end
    ...
    def use_whirl_wind_feat
      @cc.owner.tmp_power += (owner.distance * Feat.pow(@feats[FEAT_WHIRL_WIND])) if @feats_enable[FEAT_WHIRL_WIND]
    end
    ...
    def use_active_armor_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::DEF) * Feat.pow(@feats[FEAT_ACTIVE_ARMOR])) if @feats_enable[FEAT_ACTIVE_ARMOR]
    end
    ...
    def set_active_armor_feat_sealing_state
      ...
      if @active_armor_feat_state_initialized
        if (@cc.status[STATE_SEAL][1]).positive? != @active_armor_feat_state_sealing
          @active_armor_feat_state_sealing = (@cc.status[STATE_SEAL][1]).positive?
    ...
    def check_seal_active_armor_feat
      ...
      if (@cc.status[STATE_SEAL][1]).positive? && @cc.owner.hit_point.positive? && @active_armor_feat_state_changed
    ...
    def check_unseal_active_armor_feat
      set_active_armor_feat_sealing_state
      if (@cc.status[STATE_SEAL][1]).zero? && @cc.owner.hit_point.positive? && @active_armor_feat_state_changed
        off_transform_sequence(true)
        @active_armor_feat_state_changed = false
      end
    end
    ...
    def finish_scolor_attack_feat
      use_feat_event(@feats[FEAT_SCOLOR_ATTACK]) if @feats_enable[FEAT_SCOLOR_ATTACK]
    end
    ...
    def use_scolor_attack_feat_damage
      if @feats_enable[FEAT_SCOLOR_ATTACK]
        duel.tmp_damage += duel.tmp_damage * rand(Feat.pow(@feats[FEAT_SCOLOR_ATTACK])) if duel.tmp_damage.positive?
    ...
    def use_heat_seeker_feat
      if @feats_enable[FEAT_HEAT_SEEKER]
        # 2020-12-18 Issue#246 Remove constraint
      end
    end
    ...
    def finish_heat_seeker_feat
      use_feat_event(@feats[FEAT_HEAT_SEEKER]) if @feats_enable[FEAT_HEAT_SEEKER]
    end
    ...
    def use_heat_seeker_feat_damage
      if @feats_enable[FEAT_HEAT_SEEKER]
        if Feat.pow(@feats[FEAT_HEAT_SEEKER]) == 9 && (@cc.status[STATE_PARALYSIS][1]).positive?
    ...
    def use_high_hand_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_HIGH_HAND]) * @cc.foe.battle_table.count if @feats_enable[FEAT_HIGH_HAND]
    end
    ...
    def finish_low_ball_feat
      use_feat_event(@feats[FEAT_LOW_BALL]) if @feats_enable[FEAT_LOW_BALL]
    end
    ...
    def finish_gamble_feat
      use_feat_event(@feats[FEAT_GAMBLE]) if @feats_enable[FEAT_GAMBLE]
    end
    ...
    def use_gamble_feat_damage
      if @feats_enable[FEAT_GAMBLE]
        duel.tmp_damage = attribute_damage(ATTRIBUTE_DEATH, foe) if duel.tmp_damage == 1 || (Feat.pow(@feats[FEAT_GAMBLE]).zero? && duel.tmp_damage == 2)
    ...
    def use_hanging_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_HANGING]) if @feats_enable[FEAT_HANGING]
    end
    ...
    def finish_hanging_feat
      if @feats_enable[FEAT_HANGING]
        ...
        d += 1 if d.positive? && Feat.pow(@feats[FEAT_HANGING]) > 5
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, d))
      end
    end
    ...
    # C.T.Lが使用終了
    def finish_ctl_feat
      use_feat_event(@feats[FEAT_CTL]) if @feats_enable[FEAT_CTL]
    ...
    # 有効の場合必殺技IDを返す
    def use_bpa_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BPA]) if @feats_enable[FEAT_BPA]
    end
    ...
    def finish_bpa_feat
      if @feats_enable[FEAT_BPA]
        ...
        dmg += 1 if Feat.pow(@feats[FEAT_BPA]) == 5 && dmg.positive?
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, dmg)) if dmg.positive?
      end
    ...
    def finish_lar_feat
      if @feats_enable[FEAT_LAR]
        heal_pt = Feat.pow(@feats[FEAT_LAR]) > 1 && (@cc.status[STATE_CHAOS][1]).positive? ? 2 : 1
    ...
    def use_lar_feat_damage
      if @feats_enable[FEAT_LAR]
        @cc.owner.healed_event(Feat.pow(@feats[FEAT_LAR])) if duel.tmp_damage.positive?
    ...
    def use_counter_rush_feat
      @cc.owner.tmp_power = foe.tmp_power + Feat.pow(@feats[FEAT_COUNTER_RUSH]) if @feats_enable[FEAT_COUNTER_RUSH]
    end
    ...
    def use_disaster_flame_feat
      @cc.owner.tmp_power += 3 if @feats_enable[FEAT_DISASTER_FLAME]
    end
    ...
    def use_hell_fire_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_HELL_FIRE]) if @feats_enable[FEAT_HELL_FIRE]
    end
    regist_event UseHellFireFeatEvent

    # 精密射撃が使用終了
    def finish_hell_fire_feat
      use_feat_event(@feats[FEAT_HELL_FIRE]) if @feats_enable[FEAT_HELL_FIRE]
    end
    ...
    def use_hell_fire_feat_damage
      if @feats_enable[FEAT_HELL_FIRE] && duel.tmp_damage.positive?
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << i if duel.second_entrant.hit_points[i].positive?
        end
    ...
    def use_hell_fire_feat_const_damage
      if @feats_enable[FEAT_HELL_FIRE]
        ...
        if @hell_fire_feat_const_damage&.positive?
          hps = []
          foe.hit_points.each_index do |i|
            hps << i if duel.second_entrant.hit_points[i].positive?
          end
    ...
    def use_blindness_feat1
      owner.tmp_power += Feat.pow(@feats[FEAT_BLINDNESS]) if @feats_enable[FEAT_BLINDNESS]
    end
    ...
    def use_blindness_feat2
      if @feats_enable[FEAT_BLINDNESS] && (foe.current_chara_card.get_enable_feats(PHASE_ATTACK).keys & THIRTEEN_EYES).size <= 0
        hands_limit = Feat.pow(@feats[FEAT_BLINDNESS]) > 5 ? 1 : 0
    ...
    # 焼滅が使用される
    # 有効の場合必殺技IDを返す
    def use_fire_disappear_feat; end
    ...
    def use_dark_hole_feat
      owner.tmp_power = Feat.pow(@feats[FEAT_DARK_HOLE]) if @feats_enable[FEAT_DARK_HOLE]
    end
    ...
    def finish_tannhauser_gate_feat
      @feats_enable[FEAT_TANNHAUSER_GATE] = false if @feats_enable[FEAT_TANNHAUSER_GATE]
    end
    ...
    def use_schwar_blitz_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SCHWAR_BLITZ]) if @feats_enable[FEAT_SCHWAR_BLITZ]
    end
    regist_event UseSchwarBlitzFeatEvent

    # シュバルトブリッツが使用される
    def finish_schwar_blitz_feat
      use_feat_event(@feats[FEAT_SCHWAR_BLITZ]) if @feats_enable[FEAT_SCHWAR_BLITZ]
    end
    ...
    def use_hi_rounder_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_HI_ROUNDER]) if @feats_enable[FEAT_HI_ROUNDER]
    end
    regist_event UseHiRounderFeatEvent

    # ハイランダーが使用終了
    def finish_hi_rounder_feat
      use_feat_event(@feats[FEAT_HI_ROUNDER]) if @feats_enable[FEAT_HI_ROUNDER]
    end
    ...
    # ブラッドレッティングが使用される
    # 有効の場合必殺技IDを返す
    def use_blood_retting_feat; end
    regist_event UseBloodRettingFeatEvent

    # ブラッドレッティングが使用終了
    def finish_blood_retting_feat
      use_feat_event(@feats[FEAT_BLOOD_RETTING]) if @feats_enable[FEAT_BLOOD_RETTING]
    end
    ...
    def use_blood_retting_feat_damage
      if @feats_enable[FEAT_BLOOD_RETTING]
        # ダメージがマイナス（ダイスの結果防御点の方が上回った場合）
        if duel.tmp_damage.positive? && foe.tmp_power.positive?
          @cc.owner.hit_points.each_index do |i|
            @cc.owner.party_healed_event(i, duel.tmp_damage) if i != @cc.owner.current_chara_card_no && @cc.owner.hit_points[i].positive?
          end
        end
    ...
    def finish_acupuncture_feat
      if @feats_enable[FEAT_ACUPUNCTURE]
        ...
        on_buff_event(false, foe.current_chara_card_no, STATE_MOVE_DOWN, foe.current_chara_card.status[STATE_MOVE_DOWN][0], foe.current_chara_card.status[STATE_MOVE_DOWN][1]) if buffed
        # 移動方向を制御
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, 2)) if Feat.pow(@feats[FEAT_ACUPUNCTURE]) > 1 && (foe.direction == Entrant::DIRECTION_STAY || foe.direction == Entrant::DIRECTION_CHARA_CHANGE)
        foe.set_direction(Entrant::DIRECTION_PEND) if foe.get_direction.zero?
      end
    end
    ...
    def use_dissection_feat_guard
      if @feats_enable[FEAT_DISSECTION]
        ...
        if Feat.pow(@feats[FEAT_DISSECTION]) > 1 && (duel.tmp_damage < 1)
          # 2020-10-08 - Issue#188
          p = Feat.pow(@feats[FEAT_DISSECTION])
          @cc.owner.hit_points.each_index do |i|
            @cc.owner.party_healed_event(i, p) if i != @cc.owner.current_chara_card_no && @cc.owner.hit_points[i].positive?
          end
        end
      end
    end
    ...
    def use_dissection_feat_damage
      if @feats_enable[FEAT_DISSECTION]
        # HPがマイナスで1度だけ発動
        owner.cards_max = owner.cards_max + 1 if duel.tmp_damage >= @cc.owner.hit_point
    ...
    def use_euthanasia_feat
      @cc.owner.tmp_power = 0 if @feats_enable[FEAT_EUTHANASIA]
    end
    regist_event UseEuthanasiaFeatEvent

    # ユーサネイジアが使用される
    def finish_euthanasia_feat
      use_feat_event(@feats[FEAT_EUTHANASIA]) if @feats_enable[FEAT_EUTHANASIA]
    end
    ...
    def finish_calm_back_feat
      use_feat_event(@feats[FEAT_CALM_BACK]) if @feats_enable[FEAT_CALM_BACK]
    end
    ...
    def use_calm_back_feat_damage
      if @feats_enable[FEAT_CALM_BACK]
        if (@cc.status[STATE_STIGMATA][1]).positive?
          ...
          @cc.status[STATE_STIGMATA][1] = 0 if (@cc.status[STATE_STIGMATA][1]).negative?
    ...
    def use_blue_eyes_feat
      @cc.owner.tmp_power = 0 if @feats_enable[FEAT_BLUE_EYES]
    end
    ...
    def finish_wolf_fang_feat
      if @feats_enable[FEAT_WOLF_FANG]
        ...
        stigma_pow += Feat.pow(@feats[FEAT_WOLF_FANG]) if Feat.pow(@feats[FEAT_WOLF_FANG]) > 1 && stigma_pow.positive?
    ...
    # 葉隠れが使用される
    # 有効の場合必殺技IDを返す
    def use_hagakure_feat; end
    regist_event UseHagakureFeatEvent

    # 葉隠れが使用終了
    def finish_hagakure_feat
      use_feat_event(@feats[FEAT_HAGAKURE]) if @feats_enable[FEAT_HAGAKURE]
    end
    ...
    def use_hagakure_feat_damage
      if @feats_enable[FEAT_HAGAKURE]
        ...
        if duel.tmp_damage.positive?
          # 相手のカードを回転する
          duel.tmp_damage = if Feat.pow(@feats[FEAT_HAGAKURE]).positive?
                              ((duel.tmp_damage + 1) / 2).to_i
                            else
                              (duel.tmp_damage / 2).to_i
                            end
        end
    ...
    # 有効の場合必殺技IDを返す
    def use_reppu_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_REPPU]) if @feats_enable[FEAT_REPPU]
    end
    ...
    def finish_effect_reppu_feat
      if @feats_enable[FEAT_REPPU]
        ...
        ret = []
        foe.hit_points.each_index do |i|
          ret << i if foe.hit_points[i].positive? && foe.current_chara_card_no != i
        end
    ...
    def finish_turn_reppu_feat
      @feats_enable[FEAT_REPPU] = false if @feats_enable[FEAT_REPPU]
    end
    ...
    # 必殺技の状態
    def use_enpi_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ENPI]) if @feats_enable[FEAT_ENPI]
    end
    regist_event UseEnpiFeatEvent

    # 燕飛が使用される
    def finish_enpi_feat
      use_feat_event(@feats[FEAT_ENPI]) if @feats_enable[FEAT_ENPI]
    end
    ...
    def use_mikazuki_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MIKAZUKI]) if @feats_enable[FEAT_MIKAZUKI]
    end
    ...
    def finish_mikazuki_feat
      if @feats_enable[FEAT_MIKAZUKI]
        ...
        # 全ての状態をコピーする
        fs.each_index do |i|
          next if Feat.pow(@feats[FEAT_MIKAZUKI]) == 10 && !EX_MIKAZUKI_STATE_LIST.include?(i)

          if (fs[i][1]).positive? && i != STATE_CONTROL
        ...
        # 状態初期化
        foe.cured_event if Feat.pow(@feats[FEAT_MIKAZUKI]) != 10
    ...
    def use_rhodesia_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RHODESIA]) if @feats_enable[FEAT_RHODESIA]
    end
    regist_event UseRhodesiaFeatEvent

    # ローデシアの海が使用終了
    def finish_rhodesia_feat
      use_feat_event(@feats[FEAT_RHODESIA]) if @feats_enable[FEAT_RHODESIA]
    end
    ...
    def finish_madripool_feat
      if @feats_enable[FEAT_MADRIPOOL]
        ...
        elsif hps.count.positive?
          attribute_party_damage(foe, hps, 1, ATTRIBUTE_CONSTANT, TARGET_TYPE_RANDOM, Feat.pow(@feats[FEAT_MADRIPOOL]))
        end
      end
    end
    ...
    def finish_asia_feat
      if @feats_enable[FEAT_ASIA]
        ...
        attribute_party_damage(foe, hps, Feat.pow(@feats[FEAT_ASIA]), ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if hps.size.positive?
    ...
    def use_demonic_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DEMONIC]) if @feats_enable[FEAT_DEMONIC]
    end
    regist_event UseDemonicFeatEvent

    # デモニックが使用される
    def finish_demonic_feat
      use_feat_event(@feats[FEAT_DEMONIC]) if @feats_enable[FEAT_DEMONIC]
    end
    ...
    def use_shadow_sword_feat
      @cc.owner.tmp_power = 0 if @feats_enable[FEAT_SHADOW_SWORD] && (Feat.pow(@feats[FEAT_SHADOW_SWORD]) == 1)
    end
    ...
    # パーフェクトデッドが使用される
    # 有効の場合必殺技IDを返す
    def use_perfect_dead_feat; end
    regist_event UsePerfectDeadFeatEvent

    # パーフェクトデッドが使用終了
    def finish_perfect_dead_feat
      use_feat_event(@feats[FEAT_PERFECT_DEAD]) if @feats_enable[FEAT_PERFECT_DEAD]
    end
    ...
    def use_perfect_dead_feat_damage
      if @feats_enable[FEAT_PERFECT_DEAD]
        # ダメージがマイナス（ダイスの結果防御点の方が上回った場合）
        duel.first_entrant.damaged_event(attribute_damage(ATTRIBUTE_REFLECTION, foe, duel.tmp_damage)) if owner.hit_point <= duel.tmp_damage
    ...
    # 破壊の歯車が使用される
    # 有効の場合必殺技IDを返す
    def use_destruct_gear_feat; end
    regist_event UseDestructGearFeatEvent

    # 精密射撃が使用終了
    def finish_destruct_gear_feat
      use_feat_event(@feats[FEAT_DESTRUCT_GEAR]) if @feats_enable[FEAT_DESTRUCT_GEAR]
    end
    ...
    def use_destruct_gear_feat_damage
      if @feats_enable[FEAT_DESTRUCT_GEAR]
        duel.tmp_damage = duel.tmp_damage * Feat.pow(@feats[FEAT_DESTRUCT_GEAR]) if duel.tmp_damage.positive?
    ...
    # パワーシフトが使用される
    # 有効の場合必殺技IDを返す
    def use_power_shift_feat; end
    regist_event UsePowerShiftFeatEvent

    # パワーシフトが使用終了
    def finish_power_shift_feat
      use_feat_event(@feats[FEAT_POWER_SHIFT]) if @feats_enable[FEAT_POWER_SHIFT]
    end
    ...
    def use_power_shift_feat_damage
      if @feats_enable[FEAT_POWER_SHIFT]
        ...
        if duel.tmp_damage.positive?
          hps = []
          duel.second_entrant.hit_points.each_index do |i|
            hps << i if i != duel.second_entrant.current_chara_card_no && duel.second_entrant.hit_points[i].positive?
          end
          if hps.size.positive?
    ...
    def use_kill_shot_feat
      @cc.owner.tmp_power = foe.tmp_power if @feats_enable[FEAT_KILL_SHOT]
    end
    regist_event UseKillShotFeatEvent

    # 精密射撃が使用終了
    def finish_kill_shot_feat
      use_feat_event(@feats[FEAT_KILL_SHOT]) if @feats_enable[FEAT_KILL_SHOT]
    end
    ...
    def use_kill_shot_feat_damage
      if @feats_enable[FEAT_KILL_SHOT]
        duel.tmp_damage = duel.tmp_damage * Feat.pow(@feats[FEAT_KILL_SHOT]) if duel.tmp_damage.positive?
    ...
    # ディフレクトが使用される
    # 有効の場合必殺技IDを返す
    def use_defrect_feat; end
    regist_event UseDefrectFeatEvent

    # ディフレクトが使用終了
    def finish_defrect_feat
      use_feat_event(@feats[FEAT_DEFRECT]) if @feats_enable[FEAT_DEFRECT]
    end
    ...
    def use_defrect_feat_damage
      if @feats_enable[FEAT_DEFRECT]
        # ダメージがマイナス（ダイスの結果防御点の方が上回った場合）
        if duel.tmp_damage.positive? && (duel.tmp_damage > Feat.pow(@feats[FEAT_DEFRECT]))
          # 相手のカードを回転する
    ...
    def use_flame_offering_feat
      if @feats_enable[FEAT_FLAME_OFFERING]
        ...
        # 相手のカードを奪う
        if owner.cards.size.positive?
          ct = 0
          tmp_cards = owner.cards.dup
          tmp_cards.each do |c|
            ct += discard(owner, c) if c.u_type == ActionCard::ARW || c.b_type == ActionCard::ARW
          end
          @cc.owner.special_dealed_event(duel.deck.draw_cards_event(ct).each { |c| @cc.owner.dealed_event(c) }) if ct.positive?
        end
    ...
    def use_drain_hand_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DRAIN_HAND]) if @feats_enable[FEAT_DRAIN_HAND]
    end
    regist_event UseDrainHandFeatEvent

    # 黄金の手が使用終了
    def finish_drain_hand_feat
      use_feat_event(@feats[FEAT_DRAIN_HAND]) if @feats_enable[FEAT_DRAIN_HAND]
    end
    ...
    def use_drain_hand_feat_damage
      if @feats_enable[FEAT_DRAIN_HAND]
        # ダメージがマイナス
        @cc.owner.healed_event(duel.tmp_damage) if duel.tmp_damage.positive?
    ...
    def use_dead_guard_feat_damage
      if @feats_enable[FEAT_DEAD_GUARD]
        duel.tmp_damage = 0 if duel.tmp_damage >= @cc.owner.hit_point + Feat.pow(@feats[FEAT_DEAD_GUARD])
    ...
    def finish_dead_guard_feat
      use_feat_event(@feats[FEAT_DEAD_GUARD]) if @feats_enable[FEAT_DEAD_GUARD]
    end
    ...
    # 奇数即死が使用される
    # 有効の場合必殺技IDを返す
    def use_dead_blue_feat; end
    regist_event UseDeadBlueFeatEvent

    # 奇数即死が使用終了
    def finish_dead_blue_feat
      use_feat_event(@feats[FEAT_DEAD_BLUE]) if @feats_enable[FEAT_DEAD_BLUE]
    end
    ...
    def use_dead_blue_feat_damage
      if @feats_enable[FEAT_DEAD_BLUE]
        # レベルが奇数なら即死
        duel.first_entrant.damaged_event(attribute_damage(ATTRIBUTE_DEATH, foe)) if foe.current_chara_card.level.odd?
    ...
    def use_evil_guard_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_EVIL_GUARD]) if @feats_enable[FEAT_EVIL_GUARD]
    end
    regist_event UseEvilGuardFeatEvent

    # 善悪の彼岸が使用終了
    def finish_evil_guard_feat
      use_feat_event(@feats[FEAT_EVIL_GUARD]) if @feats_enable[FEAT_EVIL_GUARD]
    end
    ...
    def use_evil_guard_feat_damage
      if @feats_enable[FEAT_EVIL_GUARD]
        # ダメージがマイナス（ダイスの結果防御点の方が上回った場合）
        if duel.tmp_damage.negative? && foe.tmp_power.positive?
          hps = []
          duel.first_entrant.hit_points.each_index do |i|
            hps << i if duel.first_entrant.hit_points[i].positive?
          end
    ...
    def finish_abyss_eyes_feat
      use_feat_event(@feats[FEAT_ABYSS_EYES]) if @feats_enable[FEAT_ABYSS_EYES]
    end
    ...
    def use_abyss_eyes_feat_damage
      if @feats_enable[FEAT_ABYSS_EYES]
        # HPがマイナスで1度だけ発動
        duel.first_entrant.damaged_event(attribute_damage(ATTRIBUTE_DYING, foe, 1)) if duel.tmp_damage >= @cc.owner.hit_point
    ...
    def finish_dead_red_feat
      if @feats_enable[FEAT_DEAD_RED]
        ...
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_DEATH, foe)) if foe.current_chara_card.level.even?
      end
    end
    ...
    def use_night_ghost_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_NIGHT_GHOST]) if @feats_enable[FEAT_NIGHT_GHOST]
    end
    regist_event UseNightGhostFeatEvent

    # 精密射撃が使用終了
    def finish_night_ghost_feat
      use_feat_event(@feats[FEAT_NIGHT_GHOST]) if @feats_enable[FEAT_NIGHT_GHOST]
    end
    ...
    def use_night_ghost_feat_damage
      if @feats_enable[FEAT_NIGHT_GHOST]
        # ダメージがプラスなら
        owner.cards_max = owner.cards_max + 1 if duel.tmp_damage.positive?
    ...
    # 必殺技の状態
    def use_avatar_war_feat; end
    regist_event UseAvatarWarFeatEvent

    # 人形の軍勢が使用される
    def finish_avatar_war_feat
      use_feat_event(@feats[FEAT_AVATAR_WAR]) if @feats_enable[FEAT_AVATAR_WAR]
    end
    ...
    # 混沌の渦が使用される
    # 有効の場合必殺技IDを返す
    def use_confuse_pool_feat; end
    regist_event UseConfusePoolFeatEvent

    # 混沌の渦が使用終了
    def finish_confuse_pool_feat
      use_feat_event(@feats[FEAT_CONFUSE_POOL]) if @feats_enable[FEAT_CONFUSE_POOL]
    end
    ...
    def use_confuse_pool_feat_damage
      if @feats_enable[FEAT_CONFUSE_POOL]
        ...
        aca.count.times do |i|
          dmg += discard(owner, aca[i]) if aca[i]
        end
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, dmg))
    ...
    def use_prominence_feat
      if @feats_enable[FEAT_PROMINENCE]
        ...
        owner.healed_event(Feat.pow(@feats[FEAT_PROMINENCE])) if owner.hit_point.positive?
      end
    end
    ...
    def finish_prominence_feat
      @cc.owner.special_dealed_event(duel.deck.draw_cards_event(Feat.pow(@feats[FEAT_PROMINENCE])).each { |c| @cc.owner.dealed_event(c) }) if @feats_enable[FEAT_PROMINENCE]
    end
    ...
    def use_battle_axe_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BATTLE_AXE]) if @feats_enable[FEAT_BATTLE_AXE]
    end
    regist_event UseBattleAxeFeatEvent

    # バトルアックスが使用される
    def finish_battle_axe_feat
      use_feat_event(@feats[FEAT_BATTLE_AXE]) if @feats_enable[FEAT_BATTLE_AXE]
    end
    ...
    def use_battle_axe_feat_damage
      if @feats_enable[FEAT_BATTLE_AXE]
        duel.tmp_damage = duel.tmp_damage * 2 if (@cc.status[STATE_PARALYSIS][1]).positive?
    ...
    def finish_blue_rose_feat
      use_feat_event(@feats[FEAT_BLUE_ROSE]) if @feats_enable[FEAT_BLUE_ROSE]
    end
    ...
    def finish_white_crow_feat
      if @feats_enable[FEAT_WHITE_CROW]
        ...
        @cc.status.each_with_index do |c, i|
          point += 1 if (c[1]).positive? && i != STATE_CONTROL
        end
    ...
    def use_red_moon_feat
      if @feats_enable[FEAT_RED_MOON] || @feats_enable[FEAT_EX_RED_MOON]
        if @lock
          ...
          f_id = @feats[FEAT_RED_MOON] || @feats[FEAT_EX_RED_MOON]
    ...
    def use_red_moon_feat_damage
      if @feats_enable[FEAT_RED_MOON] || @feats_enable[FEAT_EX_RED_MOON]
        # ダメージが1以上
        @cc.owner.healed_event((duel.tmp_damage / 2).to_i) if duel.tmp_damage > 1
        f_no = @feats[FEAT_RED_MOON] ? FEAT_RED_MOON : FEAT_EX_RED_MOON
        @feats_enable[f_no] = false
      end
    end
    ...
    def finish_black_sun_feat
      if @feats_enable[FEAT_BLACK_SUN]
        ...
        if Feat.pow(@feats[FEAT_BLACK_SUN]) >= 1 # Ex Black Sun
          guard_hp = 1 # Origin = 8
          if owner.hit_point > guard_hp
            sp_dmg = @cc.owner.get_battle_table_point(ActionCard::SPC) + 1 # Origin +4
        ...
        elsif @cc.owner.get_battle_table_point(ActionCard::SPC) >= owner.hit_point
    ...
    def use_girasole_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_GIRASOLE]) if @feats_enable[FEAT_GIRASOLE]
    end
    regist_event UseGirasoleFeatEvent

    # 精密射撃が使用終了
    def finish_girasole_feat
      use_feat_event(@feats[FEAT_GIRASOLE]) if @feats_enable[FEAT_GIRASOLE]
    end
    ...
    def use_girasole_feat_damage
      if @feats_enable[FEAT_GIRASOLE] && duel.tmp_damage.positive?
    ...
    def finish_violetta_feat
      if @feats_enable[FEAT_VIOLETTA]
        ...
        buffs.pop if Feat.pow(@feats[FEAT_VIOLETTA]) > 2 && (foe.current_chara_card.status[STATE_DEAD_COUNT][1]).positive?
        sbuffs = buffs.shuffle
        Feat.pow(@feats[FEAT_VIOLETTA]).times do |_i|
          st = sbuffs.pop
    ...
    def finish_digitale_feat
      use_feat_event(@feats[FEAT_DIGITALE]) if @feats_enable[FEAT_DIGITALE]
    end
    ...
    def use_digitale_feat_damage
      if @feats_enable[FEAT_DIGITALE]
        foe.current_chara_card.status.each_index do |i|
          ...
          next unless (foe.current_chara_card.status[i][1]).positive?

          add_num = Feat.pow(@feats[FEAT_DIGITALE]) == 9 ? 3 : 2
          add_num = (BLESS_MAX - foe.current_chara_card.status[i][1]) if i == STATE_BLESS && (add_num + foe.current_chara_card.status[i][1]) > BLESS_MAX
    ...
    def finish_hachiyou_feat
      if @feats_enable[FEAT_HACHIYOU]
        ...
        mhp_criteria = []
        mhp_criteria = if Feat.pow(@feats[FEAT_HACHIYOU]) == 4
                         [1, 20, 27, 34, 40]
                       else
                         [20, 27, 34, 40]
                       end
    ...
    def use_stone_care_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_STONE_CARE]) if @feats_enable[FEAT_STONE_CARE]
    end
    ...
    def finish_stone_care_feat
      if @feats_enable[FEAT_STONE_CARE]
        ...
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << [i, duel.second_entrant.hit_points[i]] if i != duel.second_entrant.current_chara_card_no && duel.second_entrant.hit_points[i].positive?
        end
        duel.second_entrant.party_healed_event(hps.min_by { |a| a[1] }[0], 3) if hps.size.positive?
      end
    end
    ...
    def use_dust_sword_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DUST_SWORD]) if @feats_enable[FEAT_DUST_SWORD]
    end
    regist_event UseDustSwordFeatEvent

    # 絶塵剣が使用終了
    def finish_dust_sword_feat
      use_feat_event(@feats[FEAT_DUST_SWORD]) if @feats_enable[FEAT_DUST_SWORD]
    end
    ...
    def use_dust_sword_feat_damage
      if @feats_enable[FEAT_DUST_SWORD]
        ...
        if duel.tmp_damage > 1
          duel.first_entrant.hit_points.each_index do |i|
            @cc.owner.party_healed_event(i, (duel.tmp_damage / 2).to_i) if i != @cc.owner.current_chara_card_no && @cc.owner.hit_points[i].positive?
          end
        end
    ...
    def finish_illusion_feat
      use_feat_event(@feats[FEAT_ILLUSION]) if @feats_enable[FEAT_ILLUSION]
    end
    ...
    def use_illusion_feat_damage
      if @feats_enable[FEAT_ILLUSION]
        ...
        attribute_party_damage(owner, hps, 99, ATTRIBUTE_DEATH, TARGET_TYPE_HP_MIN, 1, IS_HOSTILE_DAMAGE) if hps.size.positive?
    ...
    def use_darkness_song_feat
      foe.tmp_power = (foe.tmp_power / Feat.pow(@feats[FEAT_DARKNESS_SONG])).to_i if @feats_enable[FEAT_DARKNESS_SONG]
    end
    ...
    def use_slaughter_organ_feat
      @cc.owner.tmp_power += @cc.owner.tmp_power if @feats_enable[FEAT_SLAUGHTER_ORGAN]
    end
    ...
    def use_fools_hand_feat
      @cc.owner.tmp_power += foe.hit_points.count(&:positive?) * Feat.pow(@feats[FEAT_FOOLS_HAND]) if @feats_enable[FEAT_FOOLS_HAND]
    end
    regist_event UseFoolsHandFeatEvent

    # 愚者の手が使用終了
    def finish_fools_hand_feat
      use_feat_event(@feats[FEAT_FOOLS_HAND]) if @feats_enable[FEAT_FOOLS_HAND]
    end
    ...
    def use_fools_hand_feat_damage
      if @feats_enable[FEAT_FOOLS_HAND]
        owner.damaged_event(foe.hit_points.count(&:positive?), IS_NOT_HOSTILE_DAMAGE) if @feats_enable[FEAT_SLAUGHTER_ORGAN]
    ...
    def finish_time_seed_feat
      if @feats_enable[FEAT_TIME_SEED]
        ...
        duel.second_entrant.healed_event(h) if owner.hit_point.positive?
      end
    end
    ...
    def finish_irongate_of_fate_feat
      use_feat_event(@feats[FEAT_IRONGATE_OF_FATE]) if @feats_enable[FEAT_IRONGATE_OF_FATE]
    end
    ...
    def use_irongate_of_fate_feat_damage
      if @feats_enable[FEAT_IRONGATE_OF_FATE]
        ...
        hps_s = []
        owner.hit_points.each_index do |i|
          hps_f << [i, owner.hit_points[i]] if owner.current_chara_card_no != i && owner.hit_points[i].positive?
        end
        foe.hit_points.each_index do |i|
          hps_s << [i, foe.hit_points[i]] if foe.hit_points[i].positive?
        end
        # 同率HPの場合、自パーティを優先して倒す
        if hps_f.size.positive? && hps_f.min_by { |a| a[1] }[1] <= hps_s.min_by { |a| a[1] }[1]
          attribute_party_damage(owner, hps_f.min_by { |a| a[1] }[0], 99, ATTRIBUTE_DEATH, TARGET_TYPE_SINGLE, 1, IS_NOT_HOSTILE_DAMAGE) if hps_f.size.positive?
        elsif hps_s.size.positive?
          foe.party_damaged_event(hps_s.min_by { |a| a[1] }[0], attribute_damage(ATTRIBUTE_DEATH, foe))
        end
        # 殺戮状態で回復
        # owner.healed_event(Feat.pow(@feats[FEAT_IRONGATE_OF_FATE])) if (@feats_enable[FEAT_SLAUGHTER_ORGAN] && owner.hit_point > 0)
        owner.damaged_event(Feat.pow(@feats[FEAT_IRONGATE_OF_FATE])) if @feats_enable[FEAT_SLAUGHTER_ORGAN] && owner.hit_point.positive?
    ...
    def use_gatherer_feat
      if @feats_enable[FEAT_GATHERER]
        ...
        owner.healed_event(Feat.pow(@feats[FEAT_GATHERER])) if owner.hit_point.positive?
    ...
    def finish_dead_gatherer_feat
      if @feats_enable[FEAT_GATHERER]
        ...
        # カードをきちんと墓場に戻す場合
        @gatherer_tmp_table&.each { |c| c.throw }
    ...
    # 有効の場合必殺技IDを返す
    def use_judge_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_JUDGE]) if @feats_enable[FEAT_JUDGE]
    end
    ...
    def finish_judge_feat
      if @feats_enable[FEAT_JUDGE]
        ...
        foe.damaged_event(attribute_damage(ATTRIBUTE_COUNTER, foe, (foe.tmp_power * 0.1).to_i)) if foe.tmp_power >= 10
    ...
    # 有効の場合必殺技IDを返す
    def use_dream_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DREAM]) * foe.battle_table.count if @feats_enable[FEAT_DREAM]
    end
    regist_event UseDreamFeatEvent

    # ザ・ドリームが使用終了
    def finish_dream_feat
      use_feat_event(@feats[FEAT_DREAM]) if @feats_enable[FEAT_DREAM]
    end
    ...
    def use_one_above_all_feat
      if @feats_enable[FEAT_ONE_ABOVE_ALL]
        ...
        # 相手のカードを奪う
        99.times do
          steal_deal(foe.cards[foe.cards.size - 1]) if foe.cards.size.positive?
        end
    ...
    def use_antiseptic_feat
      @antiseptic_sp_count = owner.get_battle_table_point(ActionCard::SPC) if @feats_enable[FEAT_ANTISEPTIC] && Feat.pow(@feats[FEAT_ANTISEPTIC]).positive?
    end
    ...
    def finish_antiseptic_feat
      if @feats_enable[FEAT_ANTISEPTIC]
        ...
        if Feat.pow(@feats[FEAT_ANTISEPTIC]).positive?

          set_state(@cc.special_status[SPECIAL_STATE_ANTISEPTIC], 1, 3) if @antiseptic_sp_count >= 2
    ...
    def finish_turn_antiseptic_feat
      if @feats_enable[FEAT_ANTISEPTIC]
        ...
        owner.cured_event
        @cc.owner.hit_points.each_index do |i|
          @cc.owner.party_healed_event(i, 1) if @cc.owner.hit_points[i].positive?
        end
      end
    end
    ...
    def use_silver_machine_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SILVER_MACHINE]) if @feats_enable[FEAT_SILVER_MACHINE]
    end
    regist_event UseSilverMachineFeatEvent

    # シルバーマシンが使用終了
    def finish_silver_machine_feat; end
    ...
    def finish_turn_silver_machine_feat
      if @feats_enable[FEAT_SILVER_MACHINE]
        ...
        foe.hit_points.each_index do |i|
          ...
          attribute_party_damage(foe, i, dmg) if dmg.positive?
        end
    ...
    def use_atom_heart_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ATOM_HEART]) if @feats_enable[FEAT_ATOM_HEART]
    end
    ...
    def disable_atom_heart_feat
      @feats_enable[FEAT_ATOM_HEART] = false if @feats_enable[FEAT_ATOM_HEART]
    end
    ...
    def use_electric_surgery_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ELECTRIC_SURGERY]) if @feats_enable[FEAT_ELECTRIC_SURGERY]
    end
    regist_event UseElectricSurgeryFeatEvent

    # エレクトロサージェリーが使用される
    def finish_electric_surgery_feat
      use_feat_event(@feats[FEAT_ELECTRIC_SURGERY]) if @feats_enable[FEAT_ELECTRIC_SURGERY]
    end
    ...
    def finish_used_acid_eater_feat
      @acid_eater_used = true if @feats_enable[FEAT_ACID_EATER]
    end
    ...
    def use_dead_lock_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DEAD_LOCK]) if @feats_enable[FEAT_DEAD_LOCK]
    end
    regist_event UseDeadLockFeatEvent

    # デッドロックが使用終了
    def finish_dead_lock_feat
      use_feat_event(@feats[FEAT_DEAD_LOCK]) if @feats_enable[FEAT_DEAD_LOCK]
    end
    ...
    def use_beggars_banquet_feat
      if @feats_enable[FEAT_BEGGARS_BANQUET]
        ...
        hps = []
        foe.hit_points.each_index do |i|
          hps << i if foe.hit_points[i].positive?
        end
        d = Feat.pow(@feats[FEAT_BEGGARS_BANQUET]) == 50 ? 5 : Feat.pow(@feats[FEAT_BEGGARS_BANQUET])
        attribute_party_damage(foe, hps, 1, ATTRIBUTE_CONSTANT, TARGET_TYPE_RANDOM, d) if hps.count.positive?
      end
    end
    ...
    def ex_beggars_banquet_tmp_feat
      if @feats_enable[FEAT_BEGGARS_BANQUET] && Feat.pow(@feats[FEAT_BEGGARS_BANQUET]) == 50
        # 提出したカードを回収
        @beggars_keep_card = owner.battle_table.clone.sort_by(&:get_value_max).pop
        owner.battle_table = []
      end
    end
    ...
    def use_sorrow_song_feat
      if @feats_enable[FEAT_SORROW_SONG] && (foe.current_chara_card.get_enable_feats(PHASE_ATTACK).keys & THIRTEEN_EYES).size <= 0
    ...
    def finish_sorrow_song_feat
      if @feats_enable[FEAT_SORROW_SONG]
        ...
        return if Feat.pow(@feats[FEAT_SORROW_SONG]).zero?
    ...
    def finish_ex_sorrow_song_feat
      if @feats_enable[FEAT_SORROW_SONG]
        @feats_enable[FEAT_SORROW_SONG] = false
        # 自身を能力低下
        if Feat.pow(@feats[FEAT_SORROW_SONG]).zero? && duel.tmp_damage <= 0
          if (@cc.status[STATE_STATE_DOWN][1]).positive?
            @cc.status[STATE_STATE_DOWN][1] += 1
            @cc.status[STATE_STATE_DOWN][1] = 9 if @cc.status[STATE_STATE_DOWN][1] > 9
            on_buff_event(true, owner.current_chara_card_no, STATE_STATE_DOWN, @cc.status[STATE_STATE_DOWN][0], @cc.status[STATE_STATE_DOWN][1])
    ...
    def use_red_wheel_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RED_WHEEL]) if @feats_enable[FEAT_RED_WHEEL]
    end
    regist_event UseRedWheelFeatEvent

    # 紅蓮の車輪が使用終了
    def finish_red_wheel_feat
      use_feat_event(@feats[FEAT_RED_WHEEL]) if @feats_enable[FEAT_RED_WHEEL]
    end
    ...
    def finish_red_pomegranate_feat
      if @feats_enable[FEAT_RED_POMEGRANATE]
        ...
        # ラダンム効果を発揮
        send(effects[no][0], effects[no][1])
    ...
    def random_damage_all(v = 0)
      ...
      attribute_party_damage(owner, hps, v, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL, 1, IS_NOT_HOSTILE_DAMAGE) if hps.size.positive?

      hps = []
      foe.hit_points.each_with_index do |v, i|
        hps << i if v.positive?
      end
      attribute_party_damage(foe, hps, v, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if hps.size.positive?
    end

    # 全てを回復
    def random_heal_all(v = 0)
      owner.hit_points.each_index do |i|
        owner.party_healed_event(i, v) if owner.hit_points[i].positive?
      end
      foe.hit_points.each_index do |i|
        foe.party_healed_event(i, v) if foe.hit_points[i].positive?
      end
    end
    ...
    def use_ex_time_hunt_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_TIME_HUNT]) if @feats_enable[FEAT_TIME_HUNT] && Feat.pow(@feats[FEAT_TIME_HUNT]) == 5
    end
    ...
    def use_time_hunt_feat
      if @feats_enable[FEAT_TIME_HUNT] && (foe.current_chara_card.get_enable_feats(PHASE_ATTACK).keys & THIRTEEN_EYES).size <= 0
        # 2020-12-18 @ Issue#248
        foe.tmp_power -= [10, duel.turn].min
        foe.tmp_power = 0 if foe.tmp_power.negative?
        foe.point_rewrite_event
      end
    end
    ...
    def finish_time_bomb_feat
      if @feats_enable[FEAT_TIME_BOMB]
        ...
        if [2, 3, 5, 7, 11, 13, 17].include?(duel.turn)
          # 2020-12-18 - Issue#249
          d = Feat.pow(@feats[FEAT_TIME_BOMB]) > 7 ? 3 : 2
          foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, d))
        end
      end
    end
    ...
    def finish_in_the_evening_feat
      if @feats_enable[FEAT_IN_THE_EVENING]
        ...
        @cc.owner.chara_cards.each_index do |i|
          @cc.owner.party_healed_event(i, ((@cc.owner.chara_cards[i].rarity - 4) / 2).to_i) if @cc.owner.hit_points[i].positive? && @cc.owner.chara_cards[i].rarity > 5
        end
      end
    end
    ...
    def use_final_waltz_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_FINAL_WALTZ]
    end
    regist_event UseFinalWaltzFeatEvent

    # 終局のワルツが使用される
    def finish_final_waltz_feat
      use_feat_event(@feats[FEAT_FINAL_WALTZ]) if @feats_enable[FEAT_FINAL_WALTZ]
    end
    ...
    def use_final_waltz_feat_damage
      if @feats_enable[FEAT_FINAL_WALTZ]
        if (foe.current_chara_card.status[STATE_DEAD_COUNT][1]).positive?
    ...
    def use_desperate_sonata_feat
      @cc.owner.tmp_power += 1 if @feats_enable[FEAT_DESPERATE_SONATA]
    end
    ...
    def finish_turn_desperate_sonata_feat
      @feats_enable[FEAT_DESPERATE_SONATA] = false if @feats_enable[FEAT_DESPERATE_SONATA]
    end
    ...
    def use_gladiator_march_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_GLADIATOR_MARCH]) if @feats_enable[FEAT_GLADIATOR_MARCH]
    end
    ...
    def finish_gladiator_march_feat
      if @feats_enable[FEAT_GLADIATOR_MARCH]
        ...
        @cc.owner.hit_points.each_index do |i|
          @cc.owner.party_healed_event(i, heal) if @cc.owner.hit_points[i].positive?
        end
        foe.hit_points.each_index do |i|
          foe.party_healed_event(i, 1) if foe.hit_points[i].positive?
        end
      end
    end
    ...
    def use_requiem_of_revenge_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_REQUIEM_OF_REVENGE]
    end
    ...
    def finish_requiem_of_revenge_feat
      if @feats_enable[FEAT_REQUIEM_OF_REVENGE]
        ...
        attribute_party_damage(foe, get_hps(foe), Feat.pow(@feats[FEAT_REQUIEM_OF_REVENGE]), ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if duel.turn == BATTLE_TIMEOUT_TURN
      end
    end
    ...
    def use_delicious_milk_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DELICIOUS_MILK]) if @feats_enable[FEAT_DELICIOUS_MILK]
    end
    regist_event UseDeliciousMilkFeatEvent

    # おいしいミルクの効果が発揮される
    def use_ex_delicious_milk_feat
      @cc.owner.tmp_power += 4 if @feats_enable[FEAT_DELICIOUS_MILK] && Feat.pow(@feats[FEAT_DELICIOUS_MILK]) > 10
    end
    regist_event UseExDeliciousMilkFeatEvent

    # おいしいミルクを使用
    def finish_change_delicious_milk_feat
      if @feats_enable[FEAT_DELICIOUS_MILK] && owner.hit_points.count(&:positive?) <= 1 && owner.direction == Entrant::DIRECTION_CHARA_CHANGE
        # 自分ひとりでキャラチェンジしたとき移動方向を制御
        owner.set_direction(Entrant::DIRECTION_STAY)
      end
    end
    ...
    def use_easy_injection_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_EASY_INJECTION]) if @feats_enable[FEAT_EASY_INJECTION]
    end
    ...
    def use_blood_collecting_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLOOD_COLLECTING]) * @cc.owner.get_battle_table_point(ActionCard::SPC) if @feats_enable[FEAT_BLOOD_COLLECTING]
    end
    ...
    def finish_blood_collecting_feat
      if @feats_enable[FEAT_BLOOD_COLLECTING]
        ...
        hps = []
        owner.hit_points.each_index do |i|
          hps << [i, owner.hit_points[i]] if i != owner.current_chara_card_no && owner.hit_points[i].positive?
        end
        if hps.size.positive?
          hps.shuffle! if hps.size == 2 && hps[0][1] == hps[1][1] # 残り二人のhpが同じ値ならばランダムに入れ替える
          # オーナー側は自傷ダメージとして取り扱う
    ...
    # ひみつのお薬が使用
    def use_secret_medicine_feat; end
    ...
    def use_ice_gate_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ICE_GATE]) if @feats_enable[FEAT_ICE_GATE]
    end
    ...
    def finish_ice_gate_feat
      if @feats_enable[FEAT_ICE_GATE]
        ...
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, Feat.pow(@feats[FEAT_ICE_GATE]))) if foe.current_chara_card.level.odd? && duel.turn.odd?
      end
    end
    ...
    def use_fire_gate_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FIRE_GATE]) if @feats_enable[FEAT_FIRE_GATE]
    end
    ...
    def finish_fire_gate_feat
      if @feats_enable[FEAT_FIRE_GATE]
        ...
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, Feat.pow(@feats[FEAT_FIRE_GATE]))) if foe.current_chara_card.level.even? && duel.turn.even?
      end
    end
    ...
    def finish_break_gate_feat
      if @feats_enable[FEAT_BREAK_GATE]
        ...
        @cc.owner.move_action(mp)
        @cc.foe.move_action(mp)
        owner.damaged_event(Feat.pow(@feats[FEAT_BREAK_GATE]), IS_NOT_HOSTILE_DAMAGE) if (duel.turn % 5).zero?
      end
    end
    ...
    # 叫ぶ門が使用される
    # 有効の場合必殺技IDを返す
    def use_shout_of_gate_feat; end
    regist_event UseShoutOfGateFeatEvent

    # 叫ぶ門が使用終了
    def finish_shout_of_gate_feat
      use_feat_event(@feats[FEAT_SHOUT_OF_GATE]) if @feats_enable[FEAT_SHOUT_OF_GATE]
    end
    ...
    def use_shout_of_gate_feat_damage
      if @feats_enable[FEAT_SHOUT_OF_GATE]
        # ダメージが0以下でダメージ
        attribute_party_damage(foe, get_hps(foe), Feat.pow(@feats[FEAT_SHOUT_OF_GATE]), ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if duel.tmp_damage <= 0
    ...
    def use_ferreous_anger_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FERREOUS_ANGER]) if @feats_enable[FEAT_FERREOUS_ANGER]
    end
      ...
      if @feats_enable[FEAT_FERREOUS_ANGER]
        ...
        if (@cc.status[STATE_STICK][1]).positive?
          if @cc.status[STATE_STICK][0] == 1
            foe.cured_event
          elsif Feat.pow(@feats[FEAT_FERREOUS_ANGER]) == 5 && @cc.status[STATE_STICK][0] == 2
            @cc.status.each_with_index do |s, i|
              if (s[1]).positive? && i != STATE_STICK && !CharaCard::IRREMEDIABLE_STATE.include?(i)
                s[1] = 0
    ...
    def use_name_of_charity_feat
      if @feats_enable[FEAT_NAME_OF_CHARITY]
        ...
        if foe.current_chara_card.kind == CC_KIND_MONSTAR ||
           ...
           (foe.current_chara_card.special_status[SPECIAL_STATE_CAT][1]).positive?
    ...
    def use_good_will_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_GOOD_WILL]) + 1 if @feats_enable[FEAT_GOOD_WILL]
    end
    ...
    def finish_good_will_feat
      if @feats_enable[FEAT_GOOD_WILL]
        ...
        if (@cc.status[STATE_STICK][1]).positive?
          owner.healed_event(Feat.pow(@feats[FEAT_GOOD_WILL])) if @cc.status[STATE_STICK][0] == 2
          change_stick_state
          on_buff_event(true, owner.current_chara_card_no, STATE_STICK, @cc.status[STATE_STICK][0], @cc.status[STATE_STICK][1])
        end
      end
    ...
    def use_great_vengeance_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_GREAT_VENGEANCE]) if @feats_enable[FEAT_GREAT_VENGEANCE]
    end
    ...
    def use_great_vengeance_feat_damage
      if @feats_enable[FEAT_GREAT_VENGEANCE]
        ...
        if (@cc.status[STATE_STICK][1]).positive?
          @great_vengence_const_damage = duel.tmp_damage if @cc.status[STATE_STICK][0] == 1 && duel.tmp_damage.positive?
          change_stick_state
          on_buff_event(true, owner.current_chara_card_no, STATE_STICK, @cc.status[STATE_STICK][0], @cc.status[STATE_STICK][1])
        end
      end
    ...
    def use_great_vengeance_feat_const_damage
      if @feats_enable[FEAT_GREAT_VENGEANCE]
        attribute_party_damage(foe, get_hps(foe), @great_vengence_const_damage, ATTRIBUTE_CONSTANT, TARGET_TYPE_RANDOM) if @great_vengence_const_damage&.positive?
    ...
    def use_infallible_deed_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_INFALLIBLE_DEED]) if @feats_enable[FEAT_INFALLIBLE_DEED]
    end
    regist_event UseInfallibleDeedFeatEvent

    # 無謬の行いを使用
    def finish_infallible_deed_feat
      if @feats_enable[FEAT_INFALLIBLE_DEED] && (owner.move_point > foe.move_point)
        # 移動方向を制御
        foe.set_direction(Entrant::DIRECTION_CHARA_CHANGE)
      end
    end
    ...
    def finish_effect_infallible_deed_feat
      use_feat_event(@feats[FEAT_INFALLIBLE_DEED]) if @feats_enable[FEAT_INFALLIBLE_DEED]
    end
    ...
    def finish_change_infallible_deed_feat
      if @feats_enable[FEAT_INFALLIBLE_DEED] && (owner.move_point > foe.move_point)
        hps = []
        foe.hit_points.each_index do |i|
          hps << [i, foe.hit_points[i]] if foe.hit_points[i].positive?
        end
        foe.chara_change_index = hps.min_by { |a| a[1] }[0]
    ...
    def finish_turn_infallible_deed_feat
      @feats_enable[FEAT_INFALLIBLE_DEED] = false if @feats_enable[FEAT_INFALLIBLE_DEED]
    end
    ...
    def use_idle_fate_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_IDLE_FATE]) if @feats_enable[FEAT_IDLE_FATE]
    end
    regist_event UseIdleFateFeatEvent

    # 無為の運命が使用終了
    def finish_idle_fate_feat
      use_feat_event(@feats[FEAT_IDLE_FATE]) if @feats_enable[FEAT_IDLE_FATE]
    end
    ...
    def use_idle_fate_feat_damage
      if @feats_enable[FEAT_IDLE_FATE]
        foe.cards_max = foe.cards_max - 1 if duel.tmp_damage >= foe.hit_point
    ...
    # 無念の裁きが使用される
    # 有効の場合必殺技IDを返す
    def use_regrettable_judgment_feat; end
    regist_event UseRegrettableJudgmentFeatEvent

    # 無念の裁きが使用終了
    def finish_regrettable_judgment_feat
      use_feat_event(@feats[FEAT_REGRETTABLE_JUDGMENT]) if @feats_enable[FEAT_REGRETTABLE_JUDGMENT]
    end
    ...
    def use_regrettable_judgment_feat_damage
      if @feats_enable[FEAT_REGRETTABLE_JUDGMENT] && (duel.tmp_damage >= @cc.owner.hit_point)
    ...
    def use_sin_wriggle_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SIN_WRIGGLE]) if @feats_enable[FEAT_SIN_WRIGGLE]
    end
    regist_event UseSinWriggleFeatEvent

    # 罪業の蠢きが使用終了
    def finish_sin_wriggle_feat
      use_feat_event(@feats[FEAT_SIN_WRIGGLE]) if @feats_enable[FEAT_SIN_WRIGGLE]
    end
    ...
    def use_sin_wriggle_feat_damage
      if @feats_enable[FEAT_SIN_WRIGGLE]
        ...
        if duel.tmp_damage.positive?
          # 対戦相手のイベントカードを腐らせる
          # 2020-12-18 - Issue #245
          amount = duel.tmp_damage.to_f / 3
          amount = Feat.pow(@feats[FEAT_SIN_WRIGGLE]) > 4 ? amount.ceil : amount.floor
          duel.get_event_deck(foe).replace_event_cards(USELESS_EVENT_CARD_ID, amount)
        end
    ...
    def use_idle_groan_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_IDLE_GROAN]) if @feats_enable[FEAT_IDLE_GROAN]
    end
    regist_event UseIdleGroanFeatEvent

    # 懶惰の呻きが使用される
    def finish_idle_groan_feat
      use_feat_event(@feats[FEAT_IDLE_GROAN]) if @feats_enable[FEAT_IDLE_GROAN]
    end
    ...
    def finish_turn_idle_groan_feat
      @feats_enable[FEAT_IDLE_GROAN] = false if @feats_enable[FEAT_IDLE_GROAN]
    end
    ...
    # 狂気の眼窩が使用される
    # 有効の場合必殺技IDを返す
    def use_failure_groan_feat; end
    ...
    def use_cathedral_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CATHEDRAL]) if @feats_enable[FEAT_CATHEDRAL]
    end
    ...
    def use_winter_dream_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_WINTER_DREAM]) if @feats_enable[FEAT_WINTER_DREAM]
    end
    regist_event UseWinterDreamFeatEvent

    # 冬の夢が使用される
    def finish_winter_dream_feat
      use_feat_event(@feats[FEAT_WINTER_DREAM]) if @feats_enable[FEAT_WINTER_DREAM]
    end
    ...
    def use_rud_num_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RUD_NUM]) if @feats_enable[FEAT_RUD_NUM]
    end
    ...
    def use_von_num_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_VON_NUM]) if @feats_enable[FEAT_VON_NUM]
    end
    regist_event UseVonNumFeatEvent

    # von541が使用終了
    def finish_von_num_feat
      use_feat_event(@feats[FEAT_VON_NUM]) if @feats_enable[FEAT_VON_NUM]
    end
    ...
    def use_chr_num_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CHR_NUM]) if @feats_enable[FEAT_CHR_NUM]
    end
    ...
    DRAIN_STATE_SET = [[STATE_ATK_UP, STATE_ATK_DOWN, 3],
                       ...
                       [STATE_MOVE_UP, STATE_MOVE_DOWN, 1]].freeze
    ...
    # 狂気の眼窩が使用される
    # 有効の場合必殺技IDを返す
    def use_wil_num_feat; end
    regist_event UseWilNumFeatEvent
    # 吸収するステータス
    POWER_UP_STATE_SET = [STATE_ATK_UP, STATE_ATK_DOWN, STATE_DEF_UP, STATE_DEF_DOWN, STATE_MOVE_UP, STATE_MOVE_DOWN].freeze
    ...
    def finish_wil_num_feat
      if @feats_enable[FEAT_WIL_NUM]
        ...
        POWER_UP_STATE_SET.each do |p|
          if (owner.current_chara_card.status[p][1]).positive?
          ...
          if (foe.current_chara_card.status[p][1]).positive?
    ...
    def use_kutunesirka_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_KUTUNESIRKA]) if @feats_enable[FEAT_KUTUNESIRKA]
    end
    regist_event UseKutunesirkaFeatEvent

    # クトネシリカが使用終了
    def finish_kutunesirka_feat
      use_feat_event(@feats[FEAT_KUTUNESIRKA]) if @feats_enable[FEAT_KUTUNESIRKA]
    end
    ...
    def use_feet_of_hermes_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FEET_OF_HERMES]) if @feats_enable[FEAT_FEET_OF_HERMES]
    end
    ...
    def use_feet_of_hermes_feat_damage
      if @feats_enable[FEAT_FEET_OF_HERMES]
        ...
        foe.current_chara_card.status[STATE_BIND][0] += 1 if Feat.pow(@feats[FEAT_FEET_OF_HERMES]) == 9 && (foe.current_chara_card.status[STATE_BIND][1]).positive?
        @cc.owner.move_action(-3)
        @cc.foe.move_action(-3)
        foe.current_chara_card.status[STATE_BIND][0] -= 1 if Feat.pow(@feats[FEAT_FEET_OF_HERMES]) == 9 && (foe.current_chara_card.status[STATE_BIND][1]).positive?
      end
    ...
    def use_aegis_wing_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_AEGIS_WING]) if @feats_enable[FEAT_AEGIS_WING]
    end
    ...
    def finish_aegis_wing_feat
      use_feat_event(@feats[FEAT_AEGIS_WING]) if @feats_enable[FEAT_AEGIS_WING]
    end
    ...
    def use_rampancy_feat_damage
      use_feat_event(@feats[FEAT_RAMPANCY]) if @feats_enable[FEAT_RAMPANCY]
    end
    ...
    def finish_rampancy_feat
      if @feats_enable[FEAT_RAMPANCY]
        ...
        dmg = 0 if dmg.negative?
        if (@cc.status[STATE_UNDEAD][1]).positive?
          foe.damaged_event(attribute_damage(ATTRIBUTE_COUNTER, foe, ((dmg + Feat.pow(@feats[FEAT_RAMPANCY])) / 2).to_i))
        else
          duel.tmp_damage = foe.tmp_power.positive? ? dmg : owner.hit_point - 1
        end
    ...
    def use_sacrifice_of_soul_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SACRIFICE_OF_SOUL]) if @feats_enable[FEAT_SACRIFICE_OF_SOUL]
    end
    regist_event UseSacrificeOfSoulFeatEvent

    def use_sacrifice_of_soul_feat_heal
      @cc.owner.healed_event(2) if @feats_enable[FEAT_SACRIFICE_OF_SOUL] && (@cc.status[STATE_UNDEAD][1] < 1)
    end
    ...
    def finish_silver_bullet_feat
      if @feats_enable[FEAT_SILVER_BULLET]
        ...
        owner.damaged_event(4, IS_NOT_HOSTILE_DAMAGE)
    ...
    def use_pumpkin_drop_feat
      if @feats_enable[FEAT_PUMPKIN_DROP]
        ...
        # 素数ターンで攻撃力増加
        @cc.owner.tmp_power += duel.turn if [2, 3, 5, 7, 11, 13, 17].include?(duel.turn)
      end
    end
    ...
    def finish_pumpkin_drop_feat
      use_feat_event(@feats[FEAT_PUMPKIN_DROP]) if @feats_enable[FEAT_PUMPKIN_DROP]
    end
    ...
    def use_pumpkin_drop_feat_damage
      if @feats_enable[FEAT_PUMPKIN_DROP]
        # 素数ターンでPTダメージ
        pumpkin_drop_const_actuated = [2, 3, 5, 7, 11, 13, 17].include?(duel.turn) && duel.tmp_damage.positive?
        @pumpkin_drop_const_damage = if pumpkin_drop_const_actuated
                                       (((duel.second_entrant.current_hit_point < duel.tmp_damage ? duel.second_entrant.current_hit_point : duel.tmp_damage) + 1) / 2).to_i
                                     else
                                       0
                                     end
      end
    ...
    def use_pumpkin_drop_feat_const_damage
      if @feats_enable[FEAT_PUMPKIN_DROP]
        attribute_party_damage(foe, get_hps(foe), @pumpkin_drop_const_damage, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if @pumpkin_drop_const_damage.positive?
    ...
    def use_wandering_feather_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_WANDERING_FEATHER]) if @feats_enable[FEAT_WANDERING_FEATHER]
    end
    ...
    def cutin_wandering_feather_feat
      if @feats_enable[FEAT_WANDERING_FEATHER]
        ...
        if rand_num < Feat.pow(@feats[FEAT_WANDERING_FEATHER]) * 2
          multi_num = 0
          multi_num = 0.5 if Feat.pow(@feats[FEAT_WANDERING_FEATHER]) == 5 # Ex Wandering Feather
        elsif rand_num < Feat.pow(@feats[FEAT_WANDERING_FEATHER]) * 8
          multi_num = 2
        end
        owner.tmp_power = (owner.tmp_power.to_f * multi_num).floor
        case multi_num
        when 0
          owner.special_message_event(:DREAM_STICK_X0)
        when 0.5
          owner.special_message_event(:DREAM_STICK_X0_5)
        when 2
    ...
    def finish_sheep_song_feat
      if @feats_enable[FEAT_SHEEP_SONG]
        ...
        if rand_num < Feat.pow(@feats[FEAT_SHEEP_SONG]) * 2
          ...
          foe.tmp_power = 0
          owner.healed_event(owner.current_hit_point_max - owner.hit_point) if Feat.pow(@feats[FEAT_SHEEP_SONG]) > 4
    ...
    def use_marys_sheep_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MARYS_SHEEP]) if @feats_enable[FEAT_MARYS_SHEEP]
    end
    regist_event UseMarysSheepFeatEvent

    # メリーズシープが使用終了
    def finish_marys_sheep_feat
      use_feat_event(@feats[FEAT_MARYS_SHEEP]) if @feats_enable[FEAT_MARYS_SHEEP]
    end
    ...
    def use_marys_sheep_feat_damage
      if @feats_enable[FEAT_MARYS_SHEEP]
        ...
        damage_point += 1 if Feat.pow(@feats[FEAT_MARYS_SHEEP]) > 4
    ...
    def use_evil_eye_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_EVIL_EYE]) if @feats_enable[FEAT_EVIL_EYE]
    end
    ...
    def finish_black_arts_feat
      if @feats_enable[FEAT_BLACK_ARTS]
        ...
        rand_num = rand(100)
        foe.set_direction(Entrant::DIRECTION_PEND) if foe.direction == 3 && (rand_num < 50 + alpha * foe.current_chara_card.status[STATE_CURSE][1])
      end
    end
    ...
    def use_blasphemy_curse_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLASPHEMY_CURSE]) if @feats_enable[FEAT_BLASPHEMY_CURSE]
    end
    regist_event UseBlasphemyCurseFeatEvent

    # 冒涜する呪詛が使用終了
    def finish_blasphemy_curse_feat
      use_feat_event(@feats[FEAT_BLASPHEMY_CURSE]) if @feats_enable[FEAT_BLASPHEMY_CURSE]
    end
    ...
    def use_end_of_end_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_END_OF_END]) + foe.current_chara_card.status[STATE_CURSE][1] * 5 if @feats_enable[FEAT_END_OF_END]
    end
    regist_event UseEndOfEndFeatEvent

    # 終焉の果てが使用終了
    def finish_end_of_end_feat
      use_feat_event(@feats[FEAT_END_OF_END]) if @feats_enable[FEAT_END_OF_END]
    end
    ...
    def use_end_of_end_feat_damage
      @feats_enable[FEAT_END_OF_END] = false if @feats_enable[FEAT_END_OF_END]
    end
    ...
    def use_thrones_gate_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_THRONES_GATE]) if @feats_enable[FEAT_THRONES_GATE]
    end
    regist_event UseThronesGateFeatEvent

    # 玉座の凱旋門が使用終了
    def finish_thrones_gate_feat
      use_feat_event(@feats[FEAT_THRONES_GATE]) if @feats_enable[FEAT_THRONES_GATE]
    end
    ...
    def use_thrones_gate_feat_damage
      if @feats_enable[FEAT_THRONES_GATE]
        ...
        foe.damaged_event(attribute_damage(ATTRIBUTE_DEATH, foe)) if (duel.turn % foe.current_chara_card.level).zero?
      end
    end
    ...
    def use_ghost_resentment_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_GHOST_RESENTMENT]) if @feats_enable[FEAT_GHOST_RESENTMENT]
    end
    regist_event UseGhostResentmentFeatEvent

    # 精密射撃が使用終了
    def finish_ghost_resentment_feat
      use_feat_event(@feats[FEAT_GHOST_RESENTMENT]) if @feats_enable[FEAT_GHOST_RESENTMENT]
    end
    ...
    def use_sword_avoid_feat_damage
      if @feats_enable[FEAT_SWORD_AVOID] && duel.tmp_damage.positive?
    ...
    def use_curse_sword_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CURSE_SWORD]) if @feats_enable[FEAT_CURSE_SWORD]
    end
    ...
    def use_curse_sword_feat_damage
      if @feats_enable[FEAT_CURSE_SWORD]
        if duel.tmp_damage.positive?
          hps = []
          duel.second_entrant.hit_points.each_index do |i|
            if duel.second_entrant.hit_points[i].positive?
    ...
    def use_volition_deflect_feat_damage(d)
      rand_num = rand(100)
      owner.damaged_event(attribute_damage(ATTRIBUTE_REFLECTION, owner, d)) if rand_num < 33
    end
    ...
    def use_shadow_shot_r_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SHAROW_SHOT_R]) * @cc.owner.distance if @feats_enable[FEAT_SHAROW_SHOT_R]
    end
    regist_event UseShadowShotRFeatEvent

    # 影撃ちが使用される
    def finish_shadow_shot_r_feat
      use_feat_event(@feats[FEAT_SHAROW_SHOT_R]) if @feats_enable[FEAT_SHAROW_SHOT_R]
    end
    ...
    def use_burning_tail_feat
      @burning_tail_mode_pow = @cc.owner.greater_check_of_type(ActionCard::SWD, 1) if @feats_enable[FEAT_BURNING_TAIL]
    end
    ...
    def finish_burning_tail_feat
      if @feats_enable[FEAT_BURNING_TAIL]
        ...
        if foe.battle_table.size.positive?
          ...
          foe.battle_table.each_with_index do |c, _i|
          ...
          now_on_feats = foe.current_chara_card.get_enable_feats
          attack_phase_feats.each do |f|
            foe.current_chara_card.off_feat_event(f) unless now_on_feats.include?(f)
          end
        end
    ...
    def finish_quake_walk_feat
      if @feats_enable[FEAT_QUAKE_WALK]
        ...
        foe.set_direction(Entrant::DIRECTION_PEND) if foe.get_direction == Entrant::DIRECTION_FORWARD || foe.get_direction == Entrant::DIRECTION_BACKWARD
      end
    end
    ...
    def use_drainage_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DRAINAGE]) if @feats_enable[FEAT_DRAINAGE]
    end
    regist_event UseDrainageFeatEvent

    # ドレナージが使用される
    def finish_drainage_feat
      use_feat_event(@feats[FEAT_DRAINAGE]) if @feats_enable[FEAT_DRAINAGE]
    end
    ...
    def use_drainage_feat_damage
      if @feats_enable[FEAT_DRAINAGE]
        ...
        if @cc.owner.current_chara_card.status
          ...
          # 全ての状態をコピーする
          os.each_index do |i|
            next unless (os[i][1]).positive?

            # 能力低下, 詛呪以外を相手に移す
            next unless i != STATE_STATE_DOWN && i != STATE_CURSE
    ...
    # 必殺技の状態
    def use_smile_feat
      @cc.owner.tmp_power = 0 if @feats_enable[FEAT_SMILE]
    end
    regist_event UseSmileFeatEvent

    #  やさしい微笑みを使用
    def finish_smile_feat
      use_feat_event(@feats[FEAT_SMILE]) if @feats_enable[FEAT_SMILE]
    end
    ...
    def use_blutkontamina_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLUTKONTAMINA]) if @feats_enable[FEAT_BLUTKONTAMINA]
    end
    regist_event UseBlutkontaminaFeatEvent

    # 血統汚染が使用される
    def finish_blutkontamina_feat
      use_feat_event(@feats[FEAT_BLUTKONTAMINA]) if @feats_enable[FEAT_BLUTKONTAMINA]
    end
    ...
    def use_blutkontamina_feat_damage
      if @feats_enable[FEAT_BLUTKONTAMINA]
        if duel.tmp_damage.positive?
          ...
          if foe_dead_count_num.zero?
            buffed = set_state(foe.current_chara_card.status[STATE_DEAD_COUNT], 1, 5)
            on_buff_event(false, foe.current_chara_card_no, STATE_DEAD_COUNT, foe.current_chara_card.status[STATE_DEAD_COUNT][0], foe.current_chara_card.status[STATE_DEAD_COUNT][1]) if buffed

          elsif foe.current_chara_card.status[STATE_DEAD_COUNT][1] <= 2
    ...
    def finish_cold_eyes_feat
      use_feat_event(@feats[FEAT_COLD_EYES]) if @feats_enable[FEAT_COLD_EYES]
    end
      ...
      if @feats_enable[FEAT_FEAT1]
        ...
        foe.current_chara_card.status.each_index do |i|
          ...
          while foe.current_chara_card.status[i][1] > min_turn && reduce_count < reduce_num
            ...
            if (foe.current_chara_card.status[i][1]).zero?
              off_buff_event(false, foe.current_chara_card_no, i, foe.current_chara_card.status[i][0])
              foe.damaged_event(attribute_damage(ATTRIBUTE_DEATH, foe)) if [STATE_DEAD_COUNT, STATE_CONTROL].include?(i)
            end
    ...
    def use_feat1_feat_damage
      if @feats_enable[FEAT_FEAT1]
        if duel.tmp_damage.positive? && @feats_enable[FEAT_FEAT4]

          if (@cc.status[STATE_BLESS][1]).positive?
    ...
    # 有効の場合必殺技IDを返す
    def use_feat2_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FEAT2]) if @feats_enable[FEAT_FEAT2]
    end
    ...
    def finish_feat2_feat
      if @feats_enable[FEAT_FEAT2]
        ...
        duel.tmp_damage = 0 if filtering.any? { |div| (duel.tmp_damage % div).zero? }
      end
    end
    ...
    # 必殺技の状態
    def use_feat3_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FEAT3]) if @feats_enable[FEAT_FEAT3]
    end
    ...
    def finish_feat3_feat
      if @feats_enable[FEAT_FEAT3]
        ...
        if @feats_enable[FEAT_FEAT4]
          ...
          @cc.owner.hit_points.each_index do |i|
            @cc.owner.party_healed_event(i, 2) if @cc.owner.hit_points[i].positive?
          end

          if (@cc.status[STATE_BLESS][1]).positive?

            @cc.status[STATE_BLESS][1] -= 1
            if (@cc.status[STATE_BLESS][1]).zero?
    ...
    def check_bp_feat4_feat
      owner.tmp_power *= 2 if @feats_enable[FEAT_FEAT4] && @cc && @cc.index == owner.current_chara_card_no && @over_drive_five
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_weasel_feat
      @cc.owner.tmp_power += 2 if @feats_enable[FEAT_WEASEL]
    end
    regist_event UseWeaselFeatEvent

    def use_weasel_feat_deal
      if @feat_weasel_deff_card_list && duel.turn > 1 && @feat_weasel_deff_card_list[(duel.turn - 1).to_s]
        owner.battle_table = []
        deal_list = []
        @feat_weasel_deff_card_list[(duel.turn - 1).to_s].each do |c|
    ...
    def check_table_weasel_feat_move
      return if Feat.pow(@feats[FEAT_WEASEL]).zero?

      init_weasel unless @feat_weasel_deff_card_list
      keep_list = @feat_weasel_deff_card_list[duel.turn.to_s]
      keep_list = [] if keep_list.nil?
      foe.battle_table.clone.each do |c|
        keep_list << c if c.u_type == ActionCard::DEF || c.b_type == ActionCard::DEF
      end
      @feat_weasel_deff_card_list[duel.turn.to_s] = keep_list
    end
    regist_event CheckTableWeaselFeatMoveEvent

    # 見えざる白群の鼬が使用終了
    def check_table_weasel_feat_battle
      return if Feat.pow(@feats[FEAT_WEASEL]).zero? && owner.initiative
    ...
    def finish_weasel_feat
      use_feat_event(@feats[FEAT_WEASEL]) if @feats_enable[FEAT_WEASEL] && !owner.initiative
    end
    ...
    def use_weasel_feat_damage
      if @feats_enable[FEAT_WEASEL] && !owner.initiative
        ...
        delete_num.times do |i|
          ...
          keep_list << aca[i].clone if Feat.pow(@feats[FEAT_WEASEL]).zero? && (aca[i].u_type == ActionCard::DEF || aca[i].b_type == ActionCard::DEF) && result == 1
        end
        @feat_weasel_deff_card_list[duel.turn.to_s] = keep_list
      end
    end
    ...
    def use_dark_profound_feat_bornus
      @feat_abyss_bornus_damage = 1 if @feats_enable[FEAT_DARK_PROFOUND] && @feats_enable[FEAT_ABYSS]
    end
    ...
    def check_point_karmic_dor_feat
      @karmic_dor_eight_and_more = owner.table_point_check(ActionCard::SPC) >= 8 if @feats_enable[FEAT_KARMIC_DOR]
    end
    ...
    def check_batafly_atk_feat
      ...
      if @feats_enable[FEAT_BATAFLY_ATK]
        owner.table_cards_lock = (true)
        min_val = 3
        min_val = 2 if @easing_feat_list&.key?(@feats[FEAT_BATAFLY_ATK])
    ...
    def check_batafly_def_feat
      ...
      if @feats_enable[FEAT_BATAFLY_DEF]
        ...
        owner.table_cards_lock = (false) unless @feats_enable[FEAT_BATAFLY_SLD]
    ...
    def use_batafly_def_feat
      if @feats_enable[FEAT_BATAFLY_DEF]
        extra_def = owner.table_count * 2
        extra_def = 8 if extra_def > 8
        owner.tmp_power += extra_def - owner.table_point_check(ActionCard::DEF)
      end
    end
    ...
    def check_batafly_sld_feat
      ...
      if @feats_enable[FEAT_BATAFLY_SLD]
        ...
        owner.table_cards_lock = (false) unless @feats[FEAT_BATAFLY_DEF]
    ...
    def use_batafly_sld_feat
      owner.tmp_power += 2 if @feats_enable[FEAT_BATAFLY_SLD]
    end
    ...
    def use_grace_cocktail_feat
      @cc.owner.tmp_power += 4 if @feats_enable[FEAT_GRACE_COCKTAIL]
    end
    ...
    def finish_grace_cocktail_feat
      if @feats_enable[FEAT_GRACE_COCKTAIL]
        ...
        status_list.each do |s|
          ...
          # 有効リストに無ければ消す
          elsif (@cc.owner.current_chara_card.status[s][1]).positive?
    ...
    def use_napalm_death_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_NAPALM_DEATH]) if @feats_enable[FEAT_NAPALM_DEATH]
    end
    ...
    def finish_napalm_death_feat
      if @feats_enable[FEAT_NAPALM_DEATH]
        ...
        total_cnt = @feat_grace_cocktail_cnt || 0
        used_cnt = @feat_grace_cocktail_used || 0
        new_cocktail = total_cnt - used_cnt
        if new_cocktail.positive?
    ...
    def use_suicidal_failure_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SUICIDAL_FAILURE]) * @cc.owner.get_battle_table_point(ActionCard::SPC) if @feats_enable[FEAT_SUICIDAL_FAILURE]
    end
    ...
    def use_lets_knife_r_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LETS_KNIFE_R]) * @cc.owner.battle_table.count if @feats_enable[FEAT_LETS_KNIFE_R]
    end
    ...
    def use_rumination_feat
      if @feats_enable[FEAT_RUMINATION]
        ...
        if owner.battle_table.size.positive?
        ...
        if foe.cards.size.positive?
          tmp_cards = foe.cards.dup.sort_by { rand }
          tmp_cards.each do |c|
            steal_deal(c) if (c.u_type == ActionCard::ARW || c.b_type == ActionCard::ARW || c.u_type == ActionCard::SWD || c.b_type == ActionCard::SWD) && (c.u_value >= Feat.pow(@feats[FEAT_RUMINATION]) || c.b_value >= Feat.pow(@feats[FEAT_RUMINATION]))
          end
        end
      end
    end
    ...
    def finish_rumination_feat
      if @feats_enable[FEAT_RUMINATION]
        # 相手がカードを取得
        foe.grave_dealed_event(@feat_rumination_deal_list) if @feat_rumination_deal_list
    ...
    def finish_pilum_feat
      use_feat_event(@feats[FEAT_PILUM]) if @feats_enable[FEAT_PILUM]
    end
    ...
    def use_pilum_feat_damage
      if @feats_enable[FEAT_PILUM]
        ...
        duel.tmp_damage = foe.hit_point - 1 if duel.tmp_damage >= foe.hit_point
      end
    end
    ...
    def use_road_of_underground_feat
      owner.set_direction(Entrant::DIRECTION_PEND) if foe.trap.size.positive? && foe.trap.key?(FEAT_BATAFLY_ATK.to_s)
    end
    regist_event UseRoadOfUndergroundFeatEvent

    # かからなかったら撤去する
    def use_road_of_underground_feat_finish_move
      if foe.trap.size.positive? && foe.trap.key?(FEAT_BATAFLY_ATK.to_s)
    ...
    def finish_fox_shoot_feat
      use_feat_event(@feats[FEAT_FOX_SHOOT]) if @feats_enable[FEAT_FOX_SHOOT]
    end
    ...
    def check_fox_zone_feat
      ...
      # テーブルにアクションカードがおかれている
      if owner.get_battle_table_point(ActionCard::ARW).positive?
    ...
    def use_fox_zone_feat_attack_deal
      if @feats_enable[FEAT_FOX_ZONE] && owner.initiative
        ...
        deal_arrow(deal_num, true) if deal_num.positive?
      end
    end
    ...
    def use_fox_zone_feat_defense_deal
      if @feats_enable[FEAT_FOX_ZONE]
        deal_num = @fox_zone_arrow_set.size == @fox_zone_arrow_set_size ? ((@fox_zone_arrow_set_size + 1) / 2).to_i : @fox_zone_arrow_set.size
        deal_arrow(deal_num) if deal_num.positive?
      end
    end
    ...
    def use_fox_zone_feat
      if @feats_enable[FEAT_FOX_ZONE]
        ...
        foe.tmp_power -= 1 if foe.tmp_power.positive?
      end
    end
    regist_event UseFoxZoneFeatEvent

    def finish_fox_zone_feat
      @feats_enable[FEAT_FOX_ZONE] = false if @feats_enable[FEAT_FOX_ZONE]
    end
    ...
    def deal_arrow(n, pop = false)
      ...
      n.times do
        cid = if pop
                @fox_zone_arrow_set.pop
              else
                @fox_zone_arrow_set.shift
              end
    ...
    def finish_arrow_rain_feat
      if @feats_enable[FEAT_ARROW_RAIN]
        ...
        if rain_num.positive?

          paralysis_targets = []
          poison_targets = []

          rain_num.times do |_c|
            hps = []
            duel.second_entrant.hit_points.each_index do |i|
              hps << i if duel.second_entrant.hit_points[i].positive?
            end

            next unless hps.count.positive?
            ...
            if @feats_enable[FEAT_FOX_SHOOT]
              ...
              elsif @fox_shoot_feat_poison_turn.positive?
          ...
          paralysis_targets.each do |i|
            ...
            on_buff_event(false,
                          ...
                          foe.chara_cards[i].status[STATE_PARALYSIS][1])
          end
          ...
          poison_targets.each do |i|
            ...
            on_buff_event(false,
                          ...
                          foe.chara_cards[i].status[STATE_POISON][1])
          end
    ...
    def use_fadensonnen_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FADENSONNEN]) if @feats_enable[FEAT_FADENSONNEN]
    end
    ...
    def use_lichtzwang_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LICHTZWANG]) if @feats_enable[FEAT_LICHTZWANG]
    end
    ...
    def finish_lichtzwang_feat
      if @feats_enable[FEAT_LICHTZWANG]
        ...
        atk_count.times do |_c|
          r = rand(100)
          next unless trigger > r
          ...
          if hps.count.positive?
            attribute_party_damage(foe, hps[rand(hps.size)], const_damage)
            total_const_damage += const_damage if @feats_enable[FEAT_SCHNEEPART]
          end
        end
        heal_pt = (total_const_damage / Feat.pow(@feats[FEAT_SCHNEEPART])).to_i
        owner.healed_event(heal_pt) if heal_pt.positive?
      end
    end
    ...
    def use_lichtzwang_feat_damage
      @feats_enable[FEAT_LICHTZWANG] = false if @feats_enable[FEAT_LICHTZWANG]
    end
    ...
    # 溜息の転換が使用される
    # 有効の場合必殺技IDを返す
    def use_schneepart_feat; end
    regist_event UseSchneepartFeatEvent

    # 溜息の転換が使用終了
    def finish_schneepart_feat
      use_feat_event(@feats[FEAT_SCHNEEPART]) if @feats_enable[FEAT_SCHNEEPART] && @feats_enable[FEAT_SCHNEEPART]
    end
    ...
    def use_schneepart_feat_damage
      if @feats_enable[FEAT_SCHNEEPART] && @feats_enable[FEAT_SCHNEEPART]
        # ダメージが1以上
        @cc.owner.healed_event((duel.tmp_damage / Feat.pow(FEAT_SCHNEEPART)).to_i) if duel.tmp_damage > 1 && @feats_enable[FEAT_LICHTZWANG] && owner.hit_point.positive?
    ...
    # ダメージ軽減処理
    def check_harbour
      if @cc.using && !owner.initiative && owner.is_highgate && duel.tmp_damage.positive?
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << i if duel.second_entrant.hit_points[i].positive? && duel.second_entrant.chara_cards[i].charactor_id == 4010
        end
    ...
    def use_dorfloft_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DORFLOFT]) if @feats_enable[FEAT_DORFLOFT]
    end
    ...
    def use_lumines_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LUMINES]) + owner.get_battle_table_point(ActionCard::DEF) * 4 if @feats_enable[FEAT_LUMINES]
    end
    regist_event UseLuminesFeatEvent

    # ルミネセンスが使用される
    def finish_lumines_feat
      use_feat_event(@feats[FEAT_LUMINES]) if @feats_enable[FEAT_LUMINES]
    end
    ...
    def finish_stampede_feat
      use_feat_event(@feats[FEAT_STAMPEDE]) if @feats_enable[FEAT_STAMPEDE]
    end
    ...
    def use_stampede_feat_damage
      if @feats_enable[FEAT_STAMPEDE]
        if duel.tmp_damage.positive?
          ...
          dead_count_turn = 4
          other_buff_turn = 2
          buff_list.delete(STATE_DEAD_COUNT) if (foe.current_chara_card.status[STATE_DEAD_COUNT][1]).positive?
          cnt = rand(1..2)
    ...
    def use_death_control2_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DEATH_CONTROL2]) if @feats_enable[FEAT_DEATH_CONTROL2]
    end
    ...
    def use_kengi_feat_battle_result
      if @feats_enable[FEAT_KENGI]
        ...
        duel.tmp_dice_heads_def -= fumble_cnt if duel.tmp_dice_heads_def.positive?
        ...
        duel.tmp_damage = dmg.positive? ? dmg : 0
    ...
    def finish_kengi_feat
      @feats_enable[FEAT_KENGI] = false if @feats_enable[FEAT_KENGI]
    end
    ...
    # これが俺様の本当の力だが使用
    def use_hontou_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_HONTOU]) if @feats_enable[FEAT_HONTOU]
    end
    ...
    def use_hontou_feat_battle_result
      if @feats_enable[FEAT_HONTOU]
        ...
        unless @feats_enable[FEAT_KENGI]
          # 実際に見せるダイスロール
          result = duel.battle_result
    ...
    def finish_hontou_feat
      @feats_enable[FEAT_HONTOU] = false if @feats_enable[FEAT_HONTOU]
    end
    ...
    def use_seven_wish_feat
      @seven_wish_feat_pow = @cc.owner.get_battle_table_point(ActionCard::SPC) + 1 if @feats_enable[FEAT_SEVEN_WISH]
    end
    ...
    def use_thorn_care_r_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::MOVE) * Feat.pow(@feats[FEAT_THORN_CARE_R])) if @feats_enable[FEAT_THORN_CARE_R]
    end
    regist_event UseThornCareRFeatEvent

    # 茨の構え(復活)が使用終了
    def finish_thorn_care_r_feat
      use_feat_event(@feats[FEAT_THORN_CARE_R]) if @feats_enable[FEAT_THORN_CARE_R]
    end
    ...
    def use_liberating_sword_r_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_LIBERATING_SWORD_R]) if @feats_enable[FEAT_LIBERATING_SWORD_R]
    end
    regist_event UseLiberatingSwordRFeatEvent

    # 解放剣(復活)が使用される
    def finish_liberating_sword_r_feat
      use_feat_event(@feats[FEAT_LIBERATING_SWORD_R]) if @feats_enable[FEAT_LIBERATING_SWORD_R]
    end
    ...
    def use_curse_sword_r_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CURSE_SWORD_R]) if @feats_enable[FEAT_CURSE_SWORD_R]
    end
    ...
    def finish_curse_sword_r_feat
      if @feats_enable[FEAT_CURSE_SWORD_R]
        ...
        if (@cc.status[STATE_POISON][1]).positive?
    ...
    def use_curse_sword_r_feat_damage
      if @feats_enable[FEAT_CURSE_SWORD_R]
        if duel.tmp_damage.positive?
          hps = []
          duel.second_entrant.hit_points.each_index do |i|
            next unless duel.second_entrant.hit_points[i].positive?

            poison = (@cc.status[STATE_POISON2][1]).positive? ? STATE_POISON2 : STATE_POISON
    ...
    def use_flame_ring_feat
      if @feats_enable[FEAT_FLAME_RING]
        atk = 0
        @cc.owner.tmp_power += foe.current_chara_card.status[STATE_ATK_DOWN][0] if (foe.current_chara_card.status[STATE_ATK_DOWN][1]).positive?
        @cc.owner.tmp_power += foe.current_chara_card.status[STATE_DEF_DOWN][0] if (foe.current_chara_card.status[STATE_DEF_DOWN][1]).positive?
        @cc.owner.tmp_power += foe.current_chara_card.status[STATE_STATE_DOWN][1] * 2 if (foe.current_chara_card.status[STATE_STATE_DOWN][1]).positive?
      end
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_piano_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PIANO]) if @feats_enable[FEAT_PIANO]
    end
    ...
    def finish_next_ona_ball_feat
      if @feats_enable[FEAT_ONA_BALL]
        ...
        mp_abs = move_point.negative? ? -move_point : move_point
    ...
    def finish_violent_feat_change
      @feats_enable[FEAT_VIOLENT] = false if @feats_enable[FEAT_VIOLENT]
    end
    ...
    def use_balance_life_feat
      if @feats_enable[FEAT_BALANCE_LIFE]
        hps = []
        owner.hit_points.each_index do |i|
          hps << i if owner.hit_points[i].positive?
        end
    ...
    def finish_balance_life_feat
      use_feat_event(@feats[FEAT_BALANCE_LIFE]) if @feats_enable[FEAT_BALANCE_LIFE]
    end
    ...
    def use_balance_life_feat_damage
      if @feats_enable[FEAT_BALANCE_LIFE]
        ...
        hps = []
        owner.hit_points.each_index do |i|
          hps << [i, owner.hit_points[i]] if owner.current_chara_card_no != i && owner.hit_points[i].positive?
        end
        ...
        if duel.tmp_damage.positive?
          ...
          mod = duel.tmp_damage % (hps.size + 1) if Feat.pow(@feats[FEAT_BALANCE_LIFE]) < 2
          ...
          hps.each do |h|
            h[2] = h[1] - 1 if Feat.pow(@feats[FEAT_BALANCE_LIFE]) > 2 && (h[1] <= h[2])
    ...
    def check_lifetime_sound_feat
      ...
      # テーブルにアクションカードがおかれていてかつ、距離が近距離の時
      if owner.get_battle_table_point(ActionCard::SPC).positive?
    ...
    def use_lifetime_sound_feat
      if @feats_enable[FEAT_LIFETIME_SOUND]
        hps = []
        owner.hit_points.each_index do |i|
          hps << i if owner.current_chara_card_no != i && owner.hit_points[i].positive?
        end

        @lifetime_sound_sp_count = owner.get_battle_table_point(ActionCard::SPC)
        @cc.owner.tmp_power += @lifetime_sound_sp_count * 4 if hps.size.positive?
      end
    end
    ...
    def finish_lifetime_sound_feat
      if @feats_enable[FEAT_LIFETIME_SOUND]
        ...
        # 最低限残すHP
        save_hp = Feat.pow(@feats[FEAT_LIFETIME_SOUND]).positive? ? 1 : 0
        ...
        if hps.size.positive?
          ...
          attribute_party_damage(owner, hps.min { |a, b| b[1] <=> a[1] }[0], absorb, ATTRIBUTE_CONSTANT, TARGET_TYPE_SINGLE, 1, IS_NOT_HOSTILE_DAMAGE) if absorb.positive?
        end

        owner.healed_event(absorb) if absorb.positive?
      end
    end
    regist_event FinishLifetimeSoundFeatEvent

    def finish_lifetime_sound_feat_damage
      @feats_enable[FEAT_LIFETIME_SOUND] = false if @feats_enable[FEAT_LIFETIME_SOUND]
    end
    ...
    def use_coma_white_feat
      if @feats_enable[FEAT_COMA_WHITE]
        d = 0
        owner.hit_points.each_index do |i|
          d += owner.hit_points_max[i] - owner.hit_points[i] if owner.hit_points[i].positive?
        end
    ...
    def finish_goes_to_dark_feat
      if @feats_enable[FEAT_GOES_TO_DARK] && get_hps(owner).size.positive?
        ...
        hps.each do |i|
          ...
          next if Charactor.attribute(owner.chara_cards[i].charactor_id).include?('revisers')
          ...
          on_buff_event(true,
                        ...
                        owner.chara_cards[i].status[STATE_SEAL][1])
        end
    ...
    def check_kirigakure_feat_foe
      if @feats_enable[FEAT_KIRIGAKURE]
        ...
        if foe.current_chara_card.get_enable_feats(phase).length.zero?
    ...
    def use_kirigakure_feat_calc
      if @feats_enable[FEAT_KIRIGAKURE]
        owner.seconds = owner.tmp_power.positive?
    ...
    attr_writer :check_feat_range_free
    ...
    def use_kirigakure_feat_defense_done
      if @cc&.using && foe.current_chara_card.check_feat_range_free?
        foe.current_chara_card.check_feat_range_free = false
        foe.bp_calc_range_free = false
    ...
    def recheck_battle_point(target, phase)
      ...
      target.bp_calc_unenabled = (!@target_range || @target_range.length.zero?) || !@target_range.include?(foe.distance)
    ...
    def finish_kirigakure_feat_owner_damaged
      finish_kirigakure_effect if @feats_enable[FEAT_KIRIGAKURE] && @hiding && owner.determined_damage.positive?
    end
    regist_event FinishKirigakureFeatOwnerDamagedEvent

    # 霧隠れ終了 (ダメージを与えた場合の解除)
    def finish_kirigakure_feat_do_damage
      finish_kirigakure_effect if @feats_enable[FEAT_KIRIGAKURE] && @hiding && owner.initiative && foe.determined_damage.positive?
    end
    regist_event FinishKirigakureFeatDoDamageEvent

    # 霧隠れ終了(ターンエンド)
    def finish_kirigakure_feat_finish_turn
      finish_kirigakure_effect if @feats_enable[FEAT_KIRIGAKURE] || @hiding
    end
    ...
    def finish_mikagami_feat
      if @feats_enable[FEAT_MIKAGAMI]
        ...
        foe.damaged_event(attribute_damage(ATTRIBUTE_COUNTER, foe, @mikagami_match_cnt + Feat.pow(@feats[FEAT_MIKAGAMI]))) if duel.tmp_damage < 1 && @mikagami_match_cnt.positive?
    ...
    def use_mutual_love_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MUTUAL_LOVE]) if @feats_enable[FEAT_MUTUAL_LOVE]
    end
    regist_event UseMutualLoveFeatEvent

    # 落花流水が使用終了
    def finish_mutual_love_feat
      use_feat_event(@feats[FEAT_MUTUAL_LOVE]) if @feats_enable[FEAT_MUTUAL_LOVE]
    end
    ...
    def use_mutual_love_feat_damage
      if @feats_enable[FEAT_MUTUAL_LOVE]
        # ダメージが通った場合
        if @feats_enable[FEAT_KIRIGAKURE] && duel.tmp_damage.positive?
    ...
    def use_mutual_love_feat_const_damage
      if @feats_enable[FEAT_MUTUAL_LOVE]
        @passives_enable[PASSIVE_ROCK_CRUSHER] = false if @passives_enable[PASSIVE_ROCK_CRUSHER]
        @passives_enable[PASSIVE_DAMAGE_MULTIPLIER] = false if @passives_enable[PASSIVE_DAMAGE_MULTIPLIER]
        if @mutual_love_damage&.positive?
    ...
    def finish_scapulimancy_feat
      if @feats_enable[FEAT_SCAPULIMANCY]
        ...
        @cc.owner.special_event_card_dealed_event(duel.get_event_deck(owner).draw_cards_event(ret).each { |c| @cc.owner.dealed_event(c) }) if ret.positive?

        ret = duel.get_event_deck(foe).replace_event_cards(cid, Feat.pow(@feats[FEAT_SCAPULIMANCY]), true)
        foe.special_event_card_dealed_event(duel.get_event_deck(foe).draw_cards_event(ret).each { |c| foe.dealed_event(c) }) if ret.positive?
      end
    end
    ...
    def use_soil_guard_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::DEF) * Feat.pow(@feats[FEAT_SOIL_GUARD])) if @feats_enable[FEAT_SOIL_GUARD]
    end
    ...
    # 甲羅スピンが使用される
    # 有効の場合必殺技IDを返す
    def use_carapace_spin_feat; end
    ...
    def use_vendetta_feat
      @cc.owner.tmp_power += owner.before_damage * Feat.pow(@feats[FEAT_VENDETTA]) if @feats_enable[FEAT_VENDETTA]
    end
    ...
    def use_avengers_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_AVENGERS]) if @feats_enable[FEAT_AVENGERS]
    end
    ...
    def finish_avengers_feat
      if @feats_enable[FEAT_AVENGERS]
        ...
        div = (@cc.status[STATE_CONTROL][1]).positive? ? 5 : 10
        if foe.tmp_power >= div
          ...
          hps.size.times do |i|
            pow = (owner.chara_cards[i].status[STATE_ATK_UP][1]).positive? ? owner.chara_cards[i].status[STATE_ATK_UP][0] + buff_pow : buff_pow
    ...
    def use_sharpen_edge_feat
      if @feats_enable[FEAT_SHARPEN_EDGE]
        pow = (owner.current_chara_card.status[STATE_ATK_UP][1]).positive? ? owner.current_chara_card.status[STATE_ATK_UP][0] + Feat.pow(@feats[FEAT_SHARPEN_EDGE]) : Feat.pow(@feats[FEAT_SHARPEN_EDGE])
    ...
    def finish_hacknine_feat
      if @feats_enable[FEAT_HACKNINE]
        ...
        aim_hp = 0
        if (@cc.status[STATE_CONTROL][1]).positive?
    ...
    def finish_corps_drain_feat
      use_feat_event(@feats[FEAT_CORPS_DRAIN]) if @feats_enable[FEAT_CORPS_DRAIN]
    end
    ...
    def use_corps_drain_feat_damage
      if @feats_enable[FEAT_CORPS_DRAIN]
        ...
        tmp_hp_before = foe.hit_point
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, hps.size)) if hps.size.positive?
    ...
    def finish_invert_feat
      if @feats_enable[FEAT_INVERT]
        ...
        if owner.current_chara_card.hp / 2 > owner.hit_point
          ...
          duel.second_entrant.healed_event(heal_pt) if owner.hit_point.positive?
        end
      end
    end
    ...
    def use_night_hawk_feat_det_mp_before1
      if @feats_enable[FEAT_NIGHT_HAWK]
        ...
        if (foe.current_chara_card.status[STATE_TARGET][1]).zero?
          ...
          if buffed
            on_buff_event(false,
                          ...
                          foe.current_chara_card.status[STATE_TARGET][1])
          end
    ...
    def use_night_hawk_feat_det_mp_before2
      foe.set_direction(Entrant::DIRECTION_CHARA_CHANGE) if @feats_enable[FEAT_NIGHT_HAWK] && @night_hawk_already_targeted && owner.initiative?
    end
    regist_event UseNightHawkFeatDetMpBefore2Event

    # 追跡する夜鷹を使用
    def use_night_hawk_feat_change
      if @feats_enable[FEAT_NIGHT_HAWK] && @night_hawk_already_targeted && owner.initiative?

        hps = []
        foe.chara_cards.each_with_index do |c, i|
          hps << i if i != foe.current_chara_card_no && (c.status[STATE_TARGET][1]).zero? && foe.hit_points[i].positive?
        end

        foe.chara_change_index = hps[rand(hps.size)] if hps.size.positive?

        foe.chara_change_force = true if hps.size == 1
      end
    ...
    def use_phantom_barrett_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PHANTOM_BARRETT]) if @feats_enable[FEAT_PHANTOM_BARRETT]
    end
    ...
    def finish_phantom_barrett_feat
      if @feats_enable[FEAT_PHANTOM_BARRETT]
        ...
        if duel.tmp_damage.positive?
          ...
          hps = []
          foe.chara_cards.each_with_index do |c, i|
            hps << i if (c.status[STATE_TARGET][1]).positive? && foe.hit_points[i].positive?
          end

          return if hps.size.zero?
          ...
          # パーティダメージ
          attribute_party_damage(foe, hps, dmg, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if hps.size.positive?
    ...
    def finish_one_act_feat
      if @feats_enable[FEAT_ONE_ACT]
        ...
        return if @one_act_card_types.size.zero?
        ...
        if foe.battle_table.size.positive?
          ...
          return if target_type.size.zero?
          ...
          Feat.pow(@feats[FEAT_ONE_ACT]).times do |n|
            foe.discard_table_event(aca[n]) if aca[n]
          end
    ...
    # 終極の烈弾が使用される
    # 有効の場合必殺技IDを返す
    def use_final_barrett_feat; end
    ...
    def finish_final_barrett_feat
      if @feats_enable[FEAT_FINAL_BARRETT]
        ...
        hps = []
        foe.chara_cards.each_with_index do |c, i|
          if (c.status[STATE_TARGET][1]).positive?
            target_count += 1
            hps << i if foe.hit_points[i].positive?
          end
        end
    ...
    def use_grimmdead_feat_calc
      @cc.owner.tmp_power += 1 if @feats_enable[FEAT_GRIMMDEAD] && (owner.direction == Entrant::DIRECTION_BACKWARD)
    end
    regist_event UseGrimmdeadFeatCalcEvent

    # グリムデッドが使用される
    def use_grimmdead_feat
      use_feat_event(@feats[FEAT_GRIMMDEAD]) if @feats_enable[FEAT_GRIMMDEAD]
    end
    regist_event UseGrimmdeadFeatEvent

    # グリムデッドが使用される
    def use_grimmdead_feat_move_before
      @grimmdead_tmp_dist = owner.distance if @feats_enable[FEAT_GRIMMDEAD]
    end
    ...
    def use_grimmdead_feat_move_after
      if @feats_enable[FEAT_GRIMMDEAD]
        ...
        dmg += Feat.pow(@feats[FEAT_GRIMMDEAD]) if dmg.positive?
        foe.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, dmg)) if dmg.positive?
      end
    end
    ...
    def finish_grimmdead_feat
      @feats_enable[FEAT_GRIMMDEAD] = false if @feats_enable[FEAT_GRIMMDEAD]
    end
    ...
    def use_wunderkammer_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_WUNDERKAMMER]) if @feats_enable[FEAT_WUNDERKAMMER]
    end
    regist_event UseWunderkammerFeatEvent

    # ヴンダーカンマーが使用される
    def finish_wunderkammer_feat
      use_feat_event(@feats[FEAT_WUNDERKAMMER]) if @feats_enable[FEAT_WUNDERKAMMER]
    end
    ...
    def use_constraint_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CONSTRAINT]) if @feats_enable[FEAT_CONSTRAINT]
    end
    regist_event UseConstraintFeatEvent

    # コンストレイントが使用終了
    def finish_constraint_feat
      use_feat_event(@feats[FEAT_CONSTRAINT]) if @feats_enable[FEAT_CONSTRAINT]
    end
    ...
    def use_constraint_feat_damage
      if @feats_enable[FEAT_CONSTRAINT]
        ...
        if duel.tmp_damage <= 0
          # 何を禁止するかはビットで指定
          c_num = 1 # いくつ禁止するか
          c_num = case Feat.pow(@feats[FEAT_CONSTRAINT])
                  when 7
                    2
                  when 12
                    3
    ...
    def use_renovate_atrandom_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_RENOVATE_ATRANDOM]) if @feats_enable[FEAT_RENOVATE_ATRANDOM]
    end
    ...
    def use_renovate_atrandom_feat_damage
      if @feats_enable[FEAT_RENOVATE_ATRANDOM]
        ...
        if (@cc.status[STATE_CONTROL][1]).zero?
          # 平常時 １ヒットで抜ける
          catch :exit do
            status_list.each do |s|
              proc_order.each do |i|
                next unless (owner.chara_cards[i].status[s][1]).positive?
    ...
    def finish_backbeard_feat
      use_feat_event(@feats[FEAT_BACKBEARD]) if @feats_enable[FEAT_BACKBEARD]
    end
    ...
    def finish_shadow_stitch_feat
      use_feat_event(@feats[FEAT_SHADOW_STITCH]) if @feats_enable[FEAT_SHADOW_STITCH]
    end
    regist_event FinishShadowStitchFeatEvent

    # 影縫いが使用される(非ダメージ時)
    # 有効の場合必殺技IDを返す
    SHADOW_STITCH_TURNS = [1, 4, 7, 10, 13, 16].freeze
    ...
    def finish_phantomas_feat
      if @feats_enable[FEAT_PHANTOMAS]
        ...
        owner.set_dummy_weapon unless owner.has_weapon
    ...
    def use_three_thunder_feat
      if @feats_enable[FEAT_THREE_THUNDER]
        ...
        owner.hit_points.each_with_index do |hp, i|
          @three_thunder_own_hps << i if hp.positive? && (hp % 3).zero?
        end

        @three_thunder_foe_hps = []
        foe.hit_points.each_with_index do |hp, i|
          @three_thunder_foe_hps << i if hp.positive? && (hp % 3).zero?
        end
      end
    end
    ...
    def finish_three_thunder_feat
      if @feats_enable[FEAT_THREE_THUNDER]
        ...
        d += 2 if (@cc.status[STATE_CONTROL][1]).positive?

        attribute_party_damage(owner, @three_thunder_own_hps, d, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if @three_thunder_own_hps.size.positive?

        attribute_party_damage(foe, @three_thunder_foe_hps, d, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if @three_thunder_foe_hps.size.positive?
      end
    end
    ...
    def use_prime_heal_feat
      if @feats_enable[FEAT_PRIME_HEAL]
        @prime_heal_own_hps = []
        owner.hit_points.each_with_index do |hp, i|
          @prime_heal_own_hps << i if hp.positive? && Prime.prime?(hp)
        end

        @prime_heal_foe_hps = []
        foe.hit_points.each_with_index do |hp, i|
          @prime_heal_foe_hps << i if hp.positive? && Prime.prime?(hp)
        end
      end
    end
    ...
    def finish_prime_heal_feat
      if @feats_enable[FEAT_PRIME_HEAL]
        ...
        @prime_heal_own_hps.each do |i|
          if (owner.chara_cards[i].status[STATE_DARK][1]).positive?
        ...
        @prime_heal_foe_hps.each do |i|
          if (foe.chara_cards[i].status[STATE_DARK][1]).positive?
    ...
    def use_four_comet_feat
      if @feats_enable[FEAT_FOUR_COMET]
        @four_comet_own_hps = []
        owner.hit_points.each_with_index do |hp, i|
          @four_comet_own_hps << i if hp.positive? && (hp % 4).zero?
        end

        @four_comet_foe_hps = []
        foe.hit_points.each_with_index do |hp, i|
          @four_comet_foe_hps << i if hp.positive? && (hp % 4).zero?
        end
      end
    end
    ...
    def finish_four_comet_feat
      if @feats_enable[FEAT_FOUR_COMET]
        ...
        d += 3 if (@cc.status[STATE_CONTROL][1]).positive?

        attribute_party_damage(owner, @four_comet_own_hps, d, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if @four_comet_own_hps.size.positive?

        attribute_party_damage(foe, @four_comet_foe_hps, d, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if @four_comet_foe_hps.size.positive?
      end
    end
    ...
    def use_club_jugg_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CLUB_JUGG]) if @feats_enable[FEAT_CLUB_JUGG]
    end
    ...
    def use_knife_jugg_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_KNIFE_JUGG]) if @feats_enable[FEAT_KNIFE_JUGG]
    end
    ...
    def use_blowing_fire_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BLOWING_FIRE]) if @feats_enable[FEAT_BLOWING_FIRE]
    end
    ...
    def use_balance_ball_feat
      if @feats_enable[FEAT_BALANCE_BALL]
        ...
        when Entrant::DIRECTION_FORWARD
          use_feat_event(@feats[FEAT_BALANCE_BALL])
          return if foe_direction == Entrant::DIRECTION_BACKWARD
        when Entrant::DIRECTION_BACKWARD
          return if foe_direction == Entrant::DIRECTION_FORWARD
        when Entrant::DIRECTION_PEND, Entrant::DIRECTION_STAY, Entrant::DIRECTION_CHARA_CHANGE
          return if foe_direction != Entrant::DIRECTION_FORWARD && foe_direction != Entrant::DIRECTION_BACKWARD
        end
    ...
    def use_bad_milk_feat_recalc
      if @feats_enable[FEAT_BAD_MILK] && (@cc.get_enable_feats(PHASE_ATTACK).keys & RECALC_FEATS).size.positive?
        @cc.owner.tmp_power += (foe.chara_cards[foe.current_chara_card_no].ap - Feat.pow(@feats[FEAT_BAD_MILK])) * 2
        @cc.owner.tmp_power = 0 if @cc.owner.tmp_power.negative?
        owner.point_rewrite_event
      end
    ...
    def finish_change_bad_milk_feat
      if @feats_enable[FEAT_BAD_MILK] && owner.hit_points.count(&:positive?) <= 1 && owner.direction == Entrant::DIRECTION_CHARA_CHANGE
        # 自分ひとりでキャラチェンジしたとき移動方向を制御
        owner.set_direction(Entrant::DIRECTION_STAY)
      end
    end
    ...
    def finish_mira_hp_feat
      if @feats_enable[FEAT_MIRA_HP]
        ...
        @cc.owner.healed_event(heal_pt) if heal_pt.positive?
        owner.damaged_event(-heal_pt, IS_NOT_HOSTILE_DAMAGE) if heal_pt.negative?
    ...
    def use_mira_hp_feat_damage
      if @feats_enable[FEAT_MIRA_HP]
        owner.hit_point_changed_event(@mira_hp_before_hp) if owner.current_hit_point > @mira_hp_before_hp
    ...
    def use_skill_drain_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SKILL_DRAIN]) if @feats_enable[FEAT_SKILL_DRAIN]
    end
    regist_event UseSkillDrainFeatEvent

    # スキルドレインが使用終了
    def finish_skill_drain_feat
      use_feat_event(@feats[FEAT_SKILL_DRAIN]) if @feats_enable[FEAT_SKILL_DRAIN] && owner.initiative
    end
    ...
    def use_skill_drain_feat_damage
      if @feats_enable[FEAT_SKILL_DRAIN] && owner.initiative
        # 相手がだれなのか覚えておく
        @skill_drain_target_chara_index = duel.tmp_damage.positive? ? foe.current_chara_card_no : nil
      end
    end
    ...
    def finish_skill_drain_feat_finish
      if @feats_enable[FEAT_SKILL_DRAIN] && @skill_drain_target_chara_index
        ...
        return if foe.chara_cards[@skill_drain_target_chara_index].get_feat_ids.size.zero?
    ...
    def override_my_feats(index, f_id_src, actor = false)
      ...
      if actor
        if foe_index && @override_feats.size.positive? && @override_feats[3][:foe_index]
          # 現在誰かから借りている場合には返却する
          if (@cc.special_status[SPECIAL_STATE_OVERRIDE_SKILL][1]).positive?
        ...
        @override_feats.each do |_index, _val|
    ...
    def get_other_feats(chara_index, feat_id)
      ...
      foe.chara_cards[chara_index].feat_inventories.each do |fi|
        others[fi.feat.feat_no] = fi.feat_id if fi.feat_id != feat_id
      end
    ...
    def reset_override_my_feats
      ...
      @override_feats.each_key do |index|
        next unless @cc.feat_inventories[index]
        ...
        @feats.delete(f_no_dist)
        @override_feats[index][:other_feats].each_key { |fno| @feats.delete(fno) } if @override_feats.size.positive?
        @feats_enable.delete(f_no_dist)
        @feats[f_no_src] = f_id_src
    ...
    def delete_feat_hook(f_no)
      CHARA_FEAT_EVENT_NO[f_no].each do |fe|
        cap = fe.to_s.split('_').collect!(&:capitalize).join
        func_name = eval(cap).func_name
        hook_func_name = eval(cap).hook_func_name
        eval(hook_func_name.to_s).delete_if { |method_set| method_set[0] == eval("self.method(:#{func_name})") }
      end
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_coffin_feat
      @cc.owner.tmp_power += 10 if @feats_enable[FEAT_COFFIN] && Feat.pow(@feats[FEAT_COFFIN]) == 1
    end
    ...
    def use_dark_eyes_feat
      owner.tmp_power += Feat.pow(@feats[FEAT_DARK_EYES]) if @feats_enable[FEAT_DARK_EYES]
    end
    ...
    def use_dark_eyes_feat_damage
      if @feats_enable[FEAT_DARK_EYES]
        if !owner.initiative && duel.tmp_damage.positive?
          @cc.owner.move_action(1)
          @cc.foe.move_action(1)
        end
      ...
      if @feats_enable[FEAT_CROWS_CLAW]
        ...
        if owner.get_type_point_table_count(ActionCard::BLNK, 1, true) > 1
          ...
          if owner.get_type_point_table_count(ActionCard::BLNK, 2, true) > 1
            @feat_crows_clow_level += 1
            @feat_crows_clow_level += 1 if Feat.pow(@feats[FEAT_CROWS_CLAW]).positive? && owner.get_type_point_table_count(ActionCard::BLNK, 3, true) > 1
          end
        end

        @cc.owner.tmp_power = 5 * (3 + @feat_crows_clow_level) if @feat_crows_clow_level.positive?
      end
    end
    ...
    def finish_crows_claw_feat
      if @feats_enable[FEAT_CROWS_CLAW]
        ...
        @feats_enable[FEAT_DARK_EYES] = true if feat_nos.include?(FEAT_DARK_EYES) && @feat_crows_clow_level > 1
        @feats_enable[FEAT_SUNSET] = true if feat_nos.include?(FEAT_DARK_EYES) && feat_nos.include?(FEAT_SUNSET) && @feat_crows_clow_level > 2
    ...
    # 有効の場合必殺技IDを返す
    def use_mole_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MOLE]) if @feats_enable[FEAT_MOLE]
    end
    regist_event UseMoleFeatEvent

    # 土竜縛符が使用終了
    def finish_mole_feat
      use_feat_event(@feats[FEAT_MOLE]) if @feats_enable[FEAT_MOLE]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_sunset_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SUNSET]) if @feats_enable[FEAT_SUNSET]
    end
    regist_event UseSunsetFeatEvent

    def use_sunset_feat_result
      use_feat_event(@feats[FEAT_SUNSET]) if @feats_enable[FEAT_SUNSET]
    end
    regist_event UseSunsetFeatResultEvent

    # ダメージ参照
    def use_sunset_feat_damage_check
      @feat_sunset_damage = duel.tmp_damage if @feats_enable[FEAT_SUNSET]
    end
    ...
    def use_sunset_feat_const_damage
      if @feats_enable[FEAT_SUNSET]
        ...
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << i if duel.second_entrant.hit_points[i].positive? && i != foe.current_chara_card_no
        end

        d = @feat_sunset_damage > Feat.pow(@feats[FEAT_SUNSET]) ? Feat.pow(@feats[FEAT_SUNSET]) : @feat_sunset_damage
        attribute_party_damage(foe, hps, d, ATTRIBUTE_CONSTANT, TARGET_TYPE_RANDOM) if d.positive?
      end
    end
    ...
    def finish_vine_feat
      use_feat_event(@feats[FEAT_VINE]) if @feats_enable[FEAT_VINE]
    end
    ...
    def use_grape_vine_feat_damage
      @feats_enable[FEAT_GRAPE_VINE] = false if @feats_enable[FEAT_GRAPE_VINE]
    end
    ...
    def finish_thunder_struck_feat_end
      @feats_enable[FEAT_THUNDER_STRUCK] = false if @feats_enable[FEAT_THUNDER_STRUCK]
    end
    ...
    def use_weave_world_feat
      @cc.owner.tmp_power += 5 if @feats_enable[FEAT_WEAVE_WORLD]
    end
    ...
    def use_collection_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_COLLECTION]) if @feats_enable[FEAT_COLLECTION]
    end
    regist_event UseCollectionFeatEvent

    def use_collection_feat_deal
      if @feat_collection_card_list && duel.turn > 1 && @feat_collection_card_list[(duel.turn - 1).to_s]
        owner.battle_table = []
        deal_list = []
        cnt = 0
        @feat_collection_card_list[(duel.turn - 1).to_s].sort_by(&:get_value_max).reverse_each do |c|
          deal_list << c if !duel.deck.exist?(c) && !foe.cards.include?(c) && !owner.cards.include?(c) # 山札・手札になければ引く
    ...
    def check_table_collection_feat_move
      init_collection unless @feat_collection_card_list
      keep_list = @feat_collection_card_list[duel.turn.to_s]
      keep_list = [] if keep_list.nil?
      foe.battle_table.clone.each do |c|
        keep_list << c
      end
      @feat_collection_card_list[duel.turn.to_s] = keep_list
    end
    ...
    def finish_collection_feat
      use_feat_event(@feats[FEAT_COLLECTION]) if @feats_enable[FEAT_COLLECTION] && owner.initiative
    end
    ...
    def use_restriction_feat
      @cc.owner.tmp_power += owner.get_type_table_count(ActionCard::DEF) if @feats_enable[FEAT_RESTRICTION]
    end
    ...
    # ATK++
    def use_dabs_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DABS]) if @feats_enable[FEAT_DABS]
    end
    ...
    def use_vibration_feat
      @cc.owner.tmp_power += duel.deck.get_grave_card_count(4) * Feat.pow(@feats[FEAT_VIBRATION]) if @feats_enable[FEAT_VIBRATION]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_tot_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_TOT]) if @feats_enable[FEAT_TOT]
    end
    regist_event UseTotFeatEvent

    # ToTが使用終了
    def finish_tot_feat
      use_feat_event(@feats[FEAT_TOT]) if @feats_enable[FEAT_TOT]
    end
    ...
    def use_tot_feat_damage
      if @feats_enable[FEAT_TOT]
        ...
        # 最大2枚カードを捨てる
        aca.each do |ac|
          discarded_count += discard(foe, ac) if ac.get_types.include?(target_ac_type)
    ...
    def finish_duck_apple_feat
      if @feats_enable[FEAT_DUCK_APPLE]
        ...
        while type_point < Feat.pow(@feats[FEAT_DUCK_APPLE])
          owner.special_dealed_event(duel.deck.draw_cards_event(1).each do |c|
    ...
    def finish_rampage_feat
      use_feat_event(@feats[FEAT_RAMPAGE]) if @feats_enable[FEAT_RAMPAGE]
    end
    ...
    def check_scratch_fire_feat
      ...
      # テーブルにアクションカードがおかれていてかつ
      if owner.get_battle_table_point(ActionCard::SWD).positive?
    ...
    def use_scratch_fire_feat
      @cc.owner.tmp_power += @cc.owner.table_point_check(ActionCard::SWD) if @feats_enable[FEAT_SCRATCH_FIRE]
    end
    ...
    def finish_blue_ruin_feat
      if @feats_enable[FEAT_BLUE_RUIN]
        ...
        bullets_num = 0
        bullets_num = (1..@cc.owner.table_point_check(ActionCard::ARW)).to_a.sample if @cc.owner.table_point_check(ActionCard::ARW).positive?

        target_indexies = []

        bullets_num.times do |_i|
          hps = []
          foe.hit_points.each_index do |i|
            hps << i if foe.hit_points[i].positive?
          end

          break if hps.size.zero?
        ...
        target_indexies.each do |i|
          ...
          on_buff_event(false,
                        ...
                        foe.chara_cards[i].status[STATE_DARK][1])
        end
    ...
    def use_magnetic_field_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MAGNETIC_FIELD]) if @feats_enable[FEAT_MAGNETIC_FIELD] && !@cc.owner.initiative
    end
    ...
    def final_magnetic_field_feat
      @feats_enable[FEAT_MAGNETIC_FIELD] = false if @feats_enable[FEAT_MAGNETIC_FIELD]
    end
    ...
    def finish_afterglow_feat
      use_feat_event(@feats[FEAT_AFTERGLOW]) if @feats_enable[FEAT_AFTERGLOW]
    end
    ...
    # ヒーリングショックが使用
    def use_healing_schock_feat; end
    ...
    def use_trap_chase_feat_chain
      if @cc.charactor_id == CHARACTOR_ID_RAUL && get_feat_nos.include?(FEAT_TRAP_CHASE)
        # 1枚以上手札がなければ終了
        return if owner.cards.size.zero?
        ...
        ac = nil
        ac = if saca.size.positive?
               saca.min_by { |c| c.get_value_max(ActionCard::SWD) }
             else
               owner.cards.sample
             end
    ...
    def use_trap_chase_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_TRAP_CHASE]) if @feats_enable[FEAT_TRAP_CHASE]
    end
    regist_event UseTrapChaseFeatEvent

    # トラップチェイスが使用終了
    def finish_trap_chase_feat
      use_feat_event(@feats[FEAT_TRAP_CHASE]) if @feats_enable[FEAT_TRAP_CHASE]
    end
    ...
    def use_panic_feat_chain
      if @cc.charactor_id == CHARACTOR_ID_RAUL && get_feat_nos.include?(FEAT_TRAP_CHASE)
        # 1枚以上手札がなければ終了
        return if owner.cards.size.zero? || @feats[FEAT_PANIC].nil?
        ...
        ac = nil
        ac = if aaca.size.positive?
               aaca.min_by { |c| c.get_value_max(ActionCard::ARW) }
             else
               owner.cards.sample
             end
        discard(owner, ac) if ac
    ...
    # 必殺技の状態
    def use_panic_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PANIC]) if @feats_enable[FEAT_PANIC]
    end
    regist_event UsePanicFeatEvent

    # パニックグレネードが使用される
    def finish_panic_feat
      use_feat_event(@feats[FEAT_PANIC]) if @feats_enable[FEAT_PANIC]
    end
    ...
    def use_bullet_counter_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_BULLET_COUNTER]) if @feats_enable[FEAT_BULLET_COUNTER]
    end
    ...
    def finish_bullet_counter_feat
      if @feats_enable[FEAT_BULLET_COUNTER]
        ...
        if duel.tmp_damage <= 0
          hps = []
          foe.chara_cards.each_with_index do |c, i|
            hps << i if ((c.status[STATE_POISON][1]).positive? || (c.status[STATE_POISON2][1]).positive?) && foe.hit_points[i].positive?
          end
          # パーティダメージ
          attribute_party_damage(foe, hps, 3, ATTRIBUTE_CONSTANT, TARGET_TYPE_ALL) if hps.size.positive?
        end
      end
    end
    ...
    def use_bean_storm_feat
      if @feats_enable[FEAT_BEAN_STORM]
        ...
        @cc.owner.tmp_power += 5 if (foe.current_chara_card.status[STATE_BIND][1]).positive?
      end
    end
    ...
    def use_familiar_feat
      if @feats_enable[FEAT_FAMILIAR]
        num = @cc.owner.cards.count(&:joker?)
    ...
    def finish_familiar_feat
      if @feats_enable[FEAT_FAMILIAR]
        ...
        tmp_card = owner.cards.find(&:joker?)
        discard(owner, tmp_card) if tmp_card
    ...
    def use_crown_crown_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CROWN_CROWN]) if @feats_enable[FEAT_CROWN_CROWN]
    end
    regist_event UseCrownCrownFeatEvent

    # クラウンクラウンが使用終了
    def finish_crown_crown_feat
      use_feat_event(@feats[FEAT_CROWN_CROWN]) if @feats_enable[FEAT_CROWN_CROWN]
    end
    ...
    def finish_riddle_box_feat
      use_feat_event(@feats[FEAT_RIDDLE_BOX]) if @feats_enable[FEAT_RIDDLE_BOX]
    end
    ...
    def finish_flutter_sword_dance_feat
      if @feats_enable[FEAT_FLUTTER_SWORD_DANCE]
        ...
        owner.tmp_power = (@cc.status[STATE_PARALYSIS][1]).positive? ? 0 : foe.tmp_power
    ...
    def finish_ritual_of_bravery_feat
      use_feat_event(@feats[FEAT_RITUAL_OF_BRAVERY]) if @feats_enable[FEAT_RITUAL_OF_BRAVERY]
    ...
    def use_hunting_cheetah_feat
      @cc.owner.tmp_power += owner.battle_table.size if @feats_enable[FEAT_HUNTING_CHEETAH]
    end
    regist_event UseHuntingCheetahFeatEvent

    # 狩猟豹の剣が使用される
    def finish_hunting_cheetah_feat
      use_feat_event(@feats[FEAT_HUNTING_CHEETAH]) if @feats_enable[FEAT_HUNTING_CHEETAH]
    end
    ...
    def use_probe_feat_pow
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_PROBE]) if @feats_enable[FEAT_PROBE]
    end
    ...
    def use_probe_feat
      if @feats_enable[FEAT_PROBE]
        ...
        3.times do
          foe.current_chara_card.steal_deal(owner.cards[owner.cards.index(sorted_cards.shift)]) if owner.cards.size.positive?
        end
      end
    end
    ...
    def use_tailoring_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_TAILORING]) if @feats_enable[FEAT_TAILORING]
    end
    ...
    def use_cut_feat
      if @feats_enable[FEAT_CUT]
        multi_num = (@cc.status[STATE_CONTROL][1]).positive? ? 3 : 2
    ...
    # 縫製が使用
    def use_sewing_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SEWING]) + @cc.special_status[SPECIAL_STATE_STUFFED_TOYS][1] * 3 if @feats_enable[FEAT_SEWING]
    end
    ...
    def finish_cancellation_feat
      if @feats_enable[FEAT_CANCELLATION]
        ...
        if (@cc.special_status[SPECIAL_STATE_STUFFED_TOYS][1]).positive?
    ...
    # 有効の場合必殺技IDを返す
    def use_seiho_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SEIHO]) if @feats_enable[FEAT_SEIHO]
    end
    ...
    def finish_seiho_feat
      if @feats_enable[FEAT_SEIHO]
        ...
        owner.cards.each do |c|
          next unless (1..60).cover?(c.id)

          c.rewrite_u_value(c.u_value + 1) if c.u_type == ActionCard::SWD && c.u_value < 4
          c.rewrite_b_value(c.b_value + 1) if c.b_type == ActionCard::SWD && c.b_value < 4
        end
      end
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_dokko_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DOKKO]) if @feats_enable[FEAT_DOKKO]
    end
    regist_event UseDokkoFeatEvent

    # 使用終了
    def finish_dokko_feat
      use_feat_event(@feats[FEAT_DOKKO]) if @feats_enable[FEAT_DOKKO]
    end
    ...
    def use_dokko_feat_damage
      if @feats_enable[FEAT_DOKKO]
        ...
        owner.cards.each do |c|
          ...
          c.rewrite_u_value(6) if c.u_type == ActionCard::SWD && (c.u_value == 4 || c.u_value == 5)
          c.rewrite_b_value(6) if c.b_type == ActionCard::SWD && (c.b_value == 4 || c.b_value == 5)
        end
      end
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_nyoi_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_NYOI]) if @feats_enable[FEAT_NYOI]
    end
    regist_event UseNyoiFeatEvent

    # 使用終了
    def finish_nyoi_feat
      use_feat_event(@feats[FEAT_NYOI]) if @feats_enable[FEAT_NYOI]
    end
    ...
    def use_nyoi_feat_damage
      if @feats_enable[FEAT_NYOI]
        ...
        owner.cards.each do |c|
          ...
          c.rewrite_u_value(9) if c.u_type == ActionCard::SWD && c.u_value == 6
          c.rewrite_b_value(9) if c.b_type == ActionCard::SWD && c.b_value == 6
        end
        if duel.tmp_damage.positive?
          foe.cards.each do |c|
            next unless (1..60).cover?(c.id)

            c.rewrite_u_value(c.u_value - 1) if c.u_value > 1
            c.rewrite_b_value(c.b_value - 1) if c.b_value > 1
          end
        end
      end
    end
    ...
    # 金剛の状態
    def use_kongo_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_KONGO]) if @feats_enable[FEAT_KONGO] && (owner.distance == 3)
    end
    regist_event UseKongoFeatEvent

    # 金剛が使用される
    def finish_kongo_feat
      use_feat_event(@feats[FEAT_KONGO]) if @feats_enable[FEAT_KONGO]
    end
    ...
    def use_kongo_feat_damage
      if @feats_enable[FEAT_KONGO]
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, 8)) if owner.distance == 1
    ...
    def use_carp_lightning_feat
      @cc.owner.tmp_power += (4 + Feat.pow(@feats[FEAT_CARP_LIGHTNING]) * 4) if @feats_enable[FEAT_CARP_LIGHTNING]
    end
    regist_event UseCarpLightningFeatEvent

    # 使用終了
    def finish_carp_lightning_feat
      use_feat_event(@feats[FEAT_CARP_LIGHTNING]) if @feats_enable[FEAT_CARP_LIGHTNING]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_arrest_feat
      @cc.owner.tmp_power += (Feat.pow(@feats[FEAT_ARREST]) * owner.distance) if @feats_enable[FEAT_ARREST]
    end
    regist_event UseArrestFeatEvent

    # 捕縛が使用終了
    def finish_arrest_feat
      use_feat_event(@feats[FEAT_ARREST]) if @feats_enable[FEAT_ARREST]
    end
    ...
    def use_arrest_feat_damage
      if @feats_enable[FEAT_ARREST]
        ...
        if duel.tmp_damage <= 0 && foe.tmp_power.positive?
          lock_cards_num = (duel.tmp_dice_heads_def - duel.tmp_dice_heads_atk)
          card_id_list = foe.cards.map(&:id)
          return if lock_cards_num < 1 || card_id_list.size.zero?
    ...
    def use_quick_draw_feat
      @cc.owner.tmp_power += (Feat.pow(@feats[FEAT_QUICK_DRAW]) + owner.distance * 2) if @feats_enable[FEAT_QUICK_DRAW]
    end
    ...
    def use_gaze_feat
      @cc.owner.tmp_power += (Feat.pow(@feats[FEAT_GAZE]) * foe.battle_table.size) if @feats_enable[FEAT_GAZE]
    end
    ...
    def finish_gaze_feat
      if @feats_enable[FEAT_GAZE]
        ...
        gaze_cnt = 2
        draw_table = []
        gaze_cnt.times do |_n|
          break if tmp_table.size.zero?

          draw_table << tmp_table.shift
        end
        foe.battle_table = []
        lock_id_list = draw_table.map(&:id)
    ...
    def use_monitoring_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_MONITORING]) if @feats_enable[FEAT_MONITORING] && (owner.distance == 1)
    end
    ...
    def finish_monitoring_feat
      if @feats_enable[FEAT_MONITORING]
        ...
        # 2020-10-08 - Issue#181
        if owner.distance >= 2
          set_state(@cc.status[STATE_MOVE_UP], 1, 2)
          on_buff_event(true, owner.current_chara_card_no, STATE_MOVE_UP, @cc.status[STATE_MOVE_UP][0], @cc.status[STATE_MOVE_UP][1])
        end
    ...
    def use_time_lag_draw_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_TIME_LAG_DRAW]) if @feats_enable[FEAT_TIME_LAG_DRAW]
    end
    ...
    def finish_time_lag_buff_feat
      if @feats_enable[FEAT_TIME_LAG_BUFF]
        ...
        owner.hit_points.each_index do |i|
          set_state(owner.chara_cards[i].special_status[SPECIAL_STATE_TIME_LAG_BUFF], Feat.pow(@feats[FEAT_TIME_LAG_BUFF]), 1) if owner.hit_points[i].positive?
        end
      end
    end
    ...
    def use_damage_transfer_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DAMAGE_TRANSFER]) if @feats_enable[FEAT_DAMAGE_TRANSFER]
    end
    ...
    def finish_damage_transfer_feat
      if @feats_enable[FEAT_DAMAGE_TRANSFER]
        ...
        hps = []
        duel.second_entrant.hit_points.each_index do |i|
          hps << [i, duel.second_entrant.hit_points[i], duel.second_entrant.chara_cards[i].hp - duel.second_entrant.hit_points[i]] if duel.second_entrant.hit_points[i].positive?
        end
        if hps.size > 1
          hps.shuffle! if hps.size == 2 && hps[0][1] == hps[1][1] # 残り二人のhpが同じ値ならばランダムに入れ替える
    ...
    def use_cigarette_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_CIGARETTE]) if @feats_enable[FEAT_CIGARETTE]
    end
    ...
    def finish_cigarette_feat
      if @feats_enable[FEAT_CIGARETTE]
        ...
        owner.cards.shuffle.each do |c|
          ...
          break if duel.tmp_damage.zero? || aca.size == 2
        end
    ...
    def use_three_card_feat
      if @feats_enable[FEAT_THREE_CARD]
        add_pt = 0
        add_pt = 14 if owner.get_type_point_table_count(ActionCard::ARW, 2, true) > 2
        add_pt += 7 if owner.get_type_point_table_count(ActionCard::ARW, 1, true) > 2
    ...
    def finish_three_card_feat
      if @feats_enable[FEAT_THREE_CARD]
        ...
        d = 0
        d += 7 if owner.get_type_point_table_count(ActionCard::SPC, 2, true) > 2
        d += 3 if owner.get_type_point_table_count(ActionCard::SPC, 1, true) > 2
        duel.second_entrant.damaged_event(attribute_damage(ATTRIBUTE_CONSTANT, foe, d)) if d.positive?
      end
    end
    ...
    def use_all_in_one_feat_power
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ALL_IN_ONE]) if @feats_enable[FEAT_ALL_IN_ONE]
    end
    ...
    def use_after_fire_bird_feat
      if @feats_enable[FEAT_FIRE_BIRD]
        ...
        owner.healed_event(dmg * Feat.pow(@feats[FEAT_FIRE_BIRD])) if owner.hit_point.positive?
      end
    ...
    def use_brambles_feat
      use_feat_event(@feats[FEAT_BRAMBLES]) if @feats_enable[FEAT_BRAMBLES]
    end
    regist_event UseBramblesFeatEvent

    # 苔蔦が使用される
    def use_brambles_feat_move_before
      @brambles_tmp_dist = owner.distance if @feats_enable[FEAT_BRAMBLES]
    end
    ...
    def finish_brambles_feat
      @feats_enable[FEAT_BRAMBLES] = false if @feats_enable[FEAT_BRAMBLES]
    end
    ...
    def use_franken_tackle_feat
      foe.tmp_power = foe.tmp_power / 2 if @feats_enable[FEAT_FRANKEN_TACKLE] && owner.battle_table.count >= foe.battle_table.count
    end
    ...
    def use_franken_charging_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_FRANKEN_CHARGING]) * @cc.owner.battle_table.count if @feats_enable[FEAT_FRANKEN_CHARGING]
    end
    regist_event UseFrankenChargingFeatEvent

    # フランケン充電が使用終了
    def finish_franken_charging_feat
      use_feat_event(@feats[FEAT_FRANKEN_CHARGING]) if @feats_enable[FEAT_FRANKEN_CHARGING]
    end
    ...
    def use_moving_one_r_feat_attack
      @cc.owner.tmp_power += 10 if @feats_enable[FEAT_MOVING_ONE_R]
    end
    regist_event UseMovingOneRFeatAttackEvent

    # おいしいミルクの効果が発揮される
    def use_moving_one_r_feat_defense
      @cc.owner.tmp_power -= 5 if @feats_enable[FEAT_MOVING_ONE_R]
    end
    ...
    def finish_moving_one_r_feat
      if @feats_enable[FEAT_MOVING_ONE_R]
        ...
        if @moving_one_r_feat_tmp_mv.positive?
    ...
    def finish_turn_moving_one_r_feat
      if @feats_enable[FEAT_MOVING_ONE_R]
        @feats_enable[FEAT_MOVING_ONE_R] = false
        owner.healed_event((@moving_one_r_feat_tmp_hp - owner.hit_point + 1) / 2) if @moving_one_r_feat_tmp_hp > owner.hit_point && owner.hit_point.positive?
        off_transform_sequence(true)
      end
    end
    ...
    def use_arrogant_one_r_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_ARROGANT_ONE_R]) if @feats_enable[FEAT_ARROGANT_ONE_R]
    end
    ...
    def use_eating_one_r_feat
      @cc.owner.tmp_power += (@cc.owner.table_point_check(ActionCard::MOVE) * Feat.pow(@feats[FEAT_EATING_ONE_R])) if @feats_enable[FEAT_EATING_ONE_R]
    end
    ...
    def use_harf_dead_move
      if @feats_enable[FEAT_HARF_DEAD]
        foe.tmp_power -= 2
        foe.tmp_power = 0 if foe.tmp_power.negative?
      end
    end
    regist_event UseHarfDeadMoveEvent
    ...
    def finish_directional_beam_feat
      use_feat_event(@feats[FEAT_DIRECTIONAL_BEAM]) if @feats_enable[FEAT_DIRECTIONAL_BEAM]
    end
    ...
    def use_directional_beam_feat_damage
      if @feats_enable[FEAT_DIRECTIONAL_BEAM]
        duel.tmp_damage += duel.tmp_damage * rand(Feat.pow(@feats[FEAT_DIRECTIONAL_BEAM])) if duel.tmp_damage.positive?
    ...
    def use_heat_seeker_r_feat_damage
      if @feats_enable[FEAT_HEAT_SEEKER_R]
        ...
        if (@cc.status[STATE_DEF_DOWN][1]).positive?
          @cc.status[STATE_DEF_DOWN][1] = 0
          off_buff_event(true, owner.current_chara_card_no, STATE_DEF_DOWN, @cc.status[STATE_DEF_DOWN][0])
        end
    ...
    # 有効の場合必殺技IDを返す
    def use_delta_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_DELTA]) if @feats_enable[FEAT_DELTA]
    end
    ...
    # 有効の場合必殺技IDを返す
    def use_sigma_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_SIGMA]) if @feats_enable[FEAT_SIGMA]
    end
    ...
    def ex_sigma0_feat
      if @feats_enable[FEAT_SIGMA]
        ...
        if spp.positive? && (foe.current_chara_card.status[STATE_BIND][1]).positive?
    ...
    # スタンプが使用される
    def use_stamp_feat
      @cc.owner.tmp_power += Feat.pow(@feats[FEAT_STAMP]) if @feats_enable[FEAT_STAMP]
    end
    ...
    def use_acceleration_feat
      @cc.owner.tmp_power = 0 if @feats_enable[FEAT_ACCELERATION]
    end
    ...
    def finish_foab_feat
      if @feats_enable[FEAT_FOAB]
        ...
        own_dmg = (@cc.special_status[SPECIAL_STATE_AX_GUARD][1]).positive? ? (Feat.pow(@feats[FEAT_FOAB]) / 2).to_i : Feat.pow(@feats[FEAT_FOAB])
    ...
    def finish_white_moon_feat
      if @feats_enable[FEAT_WHITE_MOON]
        f_id = @feats[FEAT_WHITE_MOON]
        use_feat_event(f_id)
        if @cc.owner.get_battle_table_point(ActionCard::SPC) >= owner.hit_point && (@cc.status[STATE_BERSERK][1]).positive?
          duel.first_entrant.damaged_event(owner.hit_point - 1, IS_NOT_HOSTILE_DAMAGE)
        else
          duel.first_entrant.damaged_event(@cc.owner.get_battle_table_point(ActionCard::SPC), IS_NOT_HOSTILE_DAMAGE)
        end
    ...
    def finish_anger_back_feat
      use_feat_event(@feats[FEAT_ANGER_BACK]) if @feats_enable[FEAT_ANGER_BACK]
    end
    ...
    STABLE_TRAP_KINDS = [FEAT_BATAFLY_SLD].freeze
    INSTANT_TRAP_KINDS = [FEAT_BATAFLY_ATK, FEAT_BATAFLY_DEF, FEAT_BATAFLY_MOV, FEAT_CLAYMORE].freeze
    ...
    def open_trap_check
      if @cc.using && @cc.index == owner.current_chara_card_no
        ...
        INSTANT_TRAP_KINDS.each do |kind|
          next unless owner.trap.key?(kind.to_s)

          next unless check_trap_state(owner, kind, owner.distance, TRAP_STATE_READY)
        ...
        owner.trap.each do |kind, status|
          owner.trap_update_event(kind.to_i, status[TRAP_STATUS_DISTANCE], status[TRAP_STATUS_TURN], status[TRAP_STATUS_VISIBILITY]) if (status[TRAP_STATUS_TURN]).positive? && status[TRAP_STATUS_STATE] == TRAP_STATE_READY
        end
      end
    end
    ...
    def progress_trap
      if @cc.using && @cc.index == owner.current_chara_card_no
        @trap_updated = false
        trap = owner.trap
        return if trap.size.zero?

        delete_list = []
        trap.each do |kind, status|
          trap[kind.to_s][TRAP_STATUS_TURN] -= 1

          # ターンを進める。規定ターン経過したものは消す
          delete_list << kind if status[TRAP_STATUS_TURN] < 1

          trap[kind.to_s][TRAP_STATUS_STATE] = TRAP_STATE_READY if status[TRAP_STATUS_STATE] == TRAP_STATE_WAIT

          owner.trap_update_event(kind.to_i, trap[kind.to_s][TRAP_STATUS_DISTANCE], trap[kind.to_s][TRAP_STATUS_TURN], trap[kind.to_s][TRAP_STATUS_VISIBILITY])
        end
    ...
    def set_trap(target, kind, status)
      trap = target.trap
      # 既に出ているものがあれば引っ込める
      target.trap_update_event(kind.to_i, trap[kind.to_s][TRAP_STATUS_DISTANCE], 0, trap[kind.to_s][TRAP_STATUS_VISIBILITY]) if status[TRAP_STATUS_STATE] == TRAP_STATE_WAIT && trap.key?(kind.to_s) && (trap[kind.to_s][TRAP_STATUS_TURN]).positive?
      trap[kind.to_s] = status
    end
    ...
    def set_trap_status(target, kind, status_name, state)
      target.trap[kind.to_s][status_name.to_s] = state if target.trap.key?(kind.to_s)
    end

    def get_trap_status(target, kind, status_name)
      target.trap[kind.to_s][status_name.to_s] if target.trap.key?(kind.to_s)
    end
    ...
    def use_feat(id)
      unless @used_feats[id]
        @cc.owner.duel_bonus_event(DUEL_BONUS_FEAT, @used_feats.size + 1) if @cc&.owner && origin_feat?(id)
        @used_feats[id] = true
      end
    ...
    def off_field_effect(player)
      target = player ? owner : foe
      target.field_status.each_with_index do |val, i|
        next unless (val[1]).positive?
    ...
    def move_feat_forbidden?(feat_no)
      ret = false
      ret = get_feats_list_as_for(owner, PHASE_MOVE).include?(feat_no) if (owner.current_chara_card.status[STATE_DOLL][1]).positive?
      ret
    end
    ...
    def skip_check_feat(feat_no)
      # 移動技禁止状態のチェック
      return true if move_feat_forbidden?(feat_no)
    ...
    def check_feat(feat_no)
      ...
      if @check_feat_range_free
        ...
        in_the_fog_event(true, range) if range.length.positive?
      end
    end
    ...
    def get_feats_range(entrant, phase)
      ...
      range.uniq!
      range.sort! if range.length.positive?
      range
    end
    ...
    def get_battle_table_range(entrant)
      ...
      if target.initiative
        ...
        if (swd_pt + arw_pt).positive?
          range = swd_pt >= arw_pt ? AI_RANGE_SWORD : AI_RANGE_ARROW
        end
      else
        range = if target.get_battle_table_point(ActionCard::DEF).positive?
                  AI_RANGE_ALL
                else
                  AI_RANGE_NOTHING
                end
    ...
    def get_enable_feats(phase = nil, target = owner)
      ret = {}
      if phase.nil?
        ret = @feats_enable.select { |_k, v| v == true }
      else
        feats = get_feats_list_as_for(target, phase)
    ...
    def check_feat_bg(feat_no)
      ...
      if @cc.using
        ...
        if Feat.check_feat(@feats[feat_no], @cc.owner) && @cc.status[STATE_SEAL][1] <= 0 && !skip_check_feat(feat_no)
          @feats_enable[feat_no] = true unless @feats_enable[feat_no]
        else
          # OFFの場合有効カードをリセットする
          @cc.owner.reset_feat_on_cards(feat_no)
          @feats_enable[feat_no] = false if @feats_enable[feat_no]
        end
      else
        @cc.owner.reset_feat_on_cards(feat_no)
      end
    ...
    def check_passive(passive_no)
      !@passives_enable[passive_no] && @cc.special_status[SPECIAL_STATE_CAT][1] <= 0
    end
    ...
    # 相手のカードcを奪う
    def steal_deal(c)
      if foe.cards.size.positive? && !((owner.field_status[Entrant::FIELD_STATUS['AC_LOCK']][1]).positive? || (foe.field_status[Entrant::FIELD_STATUS['AC_LOCK']][1]).positive?)
    ...
    def discard(entrant, c)
      if entrant.cards.size.positive? && !((owner.field_status[Entrant::FIELD_STATUS['AC_LOCK']][1]).positive? || (foe.field_status[Entrant::FIELD_STATUS['AC_LOCK']][1]).positive?)
    ...
    EX_FEATS = { FEAT_RAZORS_EDGE => FEAT_EX_RAZORS_EDGE }.freeze
    ...
    def easing_card_condition(f_no, pow = -1)
      ...
      f_id = @feats[f_no]
      @easing_feat_list ||= {}
      unless @easing_feat_list.key?(f_id)
        # 無ければ登録
    ...
    # 条件緩和用特異メソッド定義
    def create_easing_check_feat(_f_id, _f_no)
      unless singleton_class.instance_methods(false).include?(:check_feat_core)
        def self.check_feat_core(f_id, target, range_free)
          ret = false
          if @easing_feat_list.key?(f_id)
            instance_eval(@easing_feat_list[f_id][:condition])
          else
            Feat.check_feat(f_id, target, range_free)
          end
        end
      end
    end
    ...
    def easing_condition_all_type(condition, num = -1)
      ...
      if ac_cond_list.count { |_i, cond| cond[:value] > 1 }.zero?

        REDUCTION_TYPE_PRIORITY.each do |t|
          next unless ac_cond_list.count { |_i, cond| cond[:type_sign] == t }.positive?
        ...
        ac_cond_list.each do |_i, cond|
          if cond[:type_sign] != 'W'
            ...
            elsif (cond[:value]).zero?
            end
          end
        end
      end
      ac_cond = ac_cond_list.collect do |_i, cond|
